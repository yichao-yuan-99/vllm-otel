...
DEFAULT_INPUT_NAME = "kv-usage-stacked-histogram.json"
DEFAULT_STACK_KV_INPUT_REL_PATH = Path("post-processed/gateway/stack-kv")
DEFAULT_OUTPUT_REL_PATH = Path("post-processed/visualization/gateway-stack-kv")
DEFAULT_MANIFEST_NAME = "figures-manifest.json"
DEFAULT_FIGURE_STEM = "kv-usage-stacked-histogram"
DEFAULT_FORMAT = "png"
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Generate figures from gateway stacked kv-usage histograms "
    ...
    target_group.add_argument(
        "--run-dir",
        default=None,
        help="Run result root directory containing post-processed/gateway/stack-kv/.",
    )
    target_group.add_argument(
        "--root-dir",
        default=None,
        help=(
            ...
            "with post-processed/gateway/stack-kv/kv-usage-stacked-histogram.json "
            "will be processed."
        ),
    )
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional gateway stack-kv input directory. Default: "
...
def _default_stack_kv_input_dir_for_run(run_dir: Path) -> Path:
    return (run_dir / DEFAULT_STACK_KV_INPUT_REL_PATH).resolve()
...
def _run_dir_from_stack_kv_input_dir(stack_kv_input_dir: Path) -> Path | None:
    if stack_kv_input_dir.name != "stack-kv":
        return None
    gateway_dir = stack_kv_input_dir.parent
...
def discover_run_dirs_with_gateway_stack_kv(root_dir: Path) -> list[Path]:
    run_dirs: set[Path] = set()
    for input_path in root_dir.rglob(DEFAULT_INPUT_NAME):
        if not input_path.is_file():
            continue
        stack_kv_input_dir = input_path.parent
        run_dir = _run_dir_from_stack_kv_input_dir(stack_kv_input_dir)
        if run_dir is None:
            continue
...
def _import_matplotlib_pyplot() -> Any:
    try:
        ...
        raise RuntimeError(
            "matplotlib is required to generate gateway stack-kv figures. "
...
def _render_variant_figure(
    *,
    ...
    axis.set_title(
        "KV Usage (Stacked)" + title_suffix,
        loc="left",
        fontweight="semibold",
    )
    axis.set_xlabel("Second From Run Start (s)")
    axis.set_ylabel("Accumulated KV Usage in 1s Bucket")
...
def generate_figures_for_run_dir(
    ...
    *,
    ...
    resolved_stack_kv_input_dir = (
        stack_kv_input_dir or _default_stack_kv_input_dir_for_run(resolved_run_dir)
    ).expanduser().resolve()
    resolved_output_dir = (
        output_dir or _default_output_dir_for_run(resolved_run_dir)
    ).expanduser().resolve()

    if not resolved_stack_kv_input_dir.is_dir():
        raise ValueError(
            f"Missing stack-kv input directory: {resolved_stack_kv_input_dir}"
        )
    if dpi <= 0:
        raise ValueError(f"dpi must be a positive integer: {dpi}")

    input_path = resolved_stack_kv_input_dir / DEFAULT_INPUT_NAME
    if not input_path.is_file():
        raise ValueError(f"Missing stack-kv histogram file: {input_path}")
    ...
    for series_spec in series_specs:
        ...
        for variant_spec in variants:
            ...
            figure_file_name = (
                f"{DEFAULT_FIGURE_STEM}{variant_spec['file_suffix']}.{image_format}"
            )
    ...
    manifest = {
        ...
        "source_stack_kv_input_dir": str(resolved_stack_kv_input_dir),
        "output_dir": str(resolved_output_dir),
        "image_format": image_format,
        "dpi": dpi,
        "multi_profile": bool(histogram_payload.get("multi_profile", False)),
        "port_profile_ids": histogram_payload.get("port_profile_ids"),
        "series_count": len(series_specs),
        "variant_count": len(variants),
        "figure_count": len([entry for entry in figure_entries if entry["figure_generated"]]),
        "skipped_variant_count": len(skipped_figure_ids),
        "skipped_variants": skipped_figure_ids,
...
def _main_run_dir(args: argparse.Namespace) -> int:
    ...
    stack_kv_input_dir = (
        Path(args.stack_kv_input_dir).expanduser().resolve()
        if args.stack_kv_input_dir
        else None
    )
    output_dir = Path(args.output_dir).expanduser().resolve() if args.output_dir else None
    output_path = generate_figures_for_run_dir(
        run_dir,
        stack_kv_input_dir=stack_kv_input_dir,
...
def _main_root_dir(args: argparse.Namespace) -> int:
    if args.stack_kv_input_dir:
        raise ValueError("--stack-kv-input-dir can only be used with --run-dir")
    if args.output_dir:
    ...
    run_dirs = discover_run_dirs_with_gateway_stack_kv(root_dir)
