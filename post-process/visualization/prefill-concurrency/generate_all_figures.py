...
DEFAULT_INPUT_NAME = "prefill-concurrency-timeseries.json"
DEFAULT_OUTPUT_REL_PATH = Path("post-processed/visualization/prefill-concurrency")
DEFAULT_MANIFEST_NAME = "figures-manifest.json"
DEFAULT_FIGURE_STEM = "prefill-concurrency"
DEFAULT_FORMAT = "png"
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Generate prefill-concurrency line-chart figures from extracted "
    ...
    target_group.add_argument(
        "--run-dir",
        default=None,
        help="Run result root directory containing post-processed/prefill-concurrency/.",
    )
    target_group.add_argument(
        "--root-dir",
        default=None,
        help=(
            ...
            "with post-processed/prefill-concurrency/prefill-concurrency-timeseries.json "
            "will be processed."
        ),
    )
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional prefill-concurrency timeseries input path. Default: "
...
def _default_timeseries_input_path_for_run(run_dir: Path) -> Path:
    return (run_dir / "post-processed" / "prefill-concurrency" / DEFAULT_INPUT_NAME).resolve()
...
def discover_run_dirs_with_prefill_concurrency(root_dir: Path) -> list[Path]:
    run_dirs: set[Path] = set()
    for timeseries_path in root_dir.rglob(DEFAULT_INPUT_NAME):
        if not timeseries_path.is_file():
            continue
        if timeseries_path.parent.name != "prefill-concurrency":
            continue
...
def _extract_xy_from_points(raw_points: Any) -> tuple[list[float], list[float]]:
    ...
    for item in raw_points:
        ...
        if time_offset_s is None or concurrency is None:
            continue
        pairs.append((time_offset_s, concurrency))
    if not pairs:
        return [], []
...
def _build_stats_annotation(
    *,
    ...
    tick_ms = _int_or_none(timeseries_payload.get("tick_ms"))

    requests_line = "requests: n/a"
    if request_count is not None:
        if prefill_activity_count is None:
            requests_line = f"activities: ? / {request_count}"
        else:
            requests_line = f"activities: {prefill_activity_count} / {request_count}"
    ...
    return (
        ...
        f"tick: {tick_ms if tick_ms is not None else 'n/a'} ms"
    )
...
def _import_matplotlib_pyplot() -> Any:
    try:
        ...
        raise RuntimeError(
            "matplotlib is required to generate prefill-concurrency figures. "
            "Install it in your environment, for example: pip install matplotlib"
        ) from exc
    return plt


def _render_prefill_concurrency_figure(
    *,
    timeseries_payload: dict[str, Any],
    output_path: Path,
    image_format: str,
    ...
    axis.set_title("Prefill Concurrency Over Time", loc="left", fontweight="semibold")
...
def _main_root_dir(args: argparse.Namespace) -> int:
    ...
    run_dirs = discover_run_dirs_with_prefill_concurrency(root_dir)
