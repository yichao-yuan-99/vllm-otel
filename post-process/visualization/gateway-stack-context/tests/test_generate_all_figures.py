...
def _write_stack_context_histogram(run_dir: Path) -> Path:
    ...
    return stack_context_dir


def test_discover_run_dirs_with_gateway_stack_context_scans_recursively(tmp_path: Path) -> None:
    root_dir = tmp_path / "results"
    good_run = root_dir / "a" / "job-ok"
    bad_run = root_dir / "b" / "job-missing-input"

    _write_stack_context_histogram(good_run)
    bad_stack_context_dir = bad_run / "post-processed" / "gateway" / "stack-context"
    bad_stack_context_dir.mkdir(parents=True)

    discovered = generate_all_figures.discover_run_dirs_with_gateway_stack_context(root_dir)
...
def test_generate_figures_for_run_dir_writes_manifest(tmp_path: Path, monkeypatch) -> None:
    ...
    _write_stack_context_histogram(run_dir)
    ...
    assert any(
        entry["figure_file_name"] == "context-usage-stacked-histogram-smoothed-10s.png"
        for entry in manifest["figures"]
    )
    assert any(
...
def test_generate_figures_for_run_dir_rejects_missing_input_file(tmp_path: Path) -> None:
    ...
    try:
        ...
        assert "Missing stack-context histogram file" in str(exc)
    else:
        raise AssertionError("Expected ValueError when stack-context histogram file is missing")
...
def test_main_root_dir_processes_discovered_runs(monkeypatch, tmp_path: Path) -> None:
    ...
    run_a = root_dir / "a"
    run_b = root_dir / "b"
    _write_stack_context_histogram(run_a)
    _write_stack_context_histogram(run_b)
    ...
    def fake_generate_figures_for_run_dir(
        ...
        *,
        ...
        del stack_context_input_dir, output_dir
...
def test_main_rejects_stack_context_input_dir_for_root_dir(tmp_path: Path) -> None:
    root_dir = tmp_path / "results"
    root_dir.mkdir(parents=True)
    ...
    try:
        generate_all_figures.main(
            [
                ...
                str(tmp_path / "stack-context"),
            ]
        )
    except ValueError as exc:
        assert "--stack-context-input-dir can only be used with --run-dir" in str(exc)
    else:
        raise AssertionError(
            "Expected ValueError when --stack-context-input-dir is used with --root-dir"
        )
