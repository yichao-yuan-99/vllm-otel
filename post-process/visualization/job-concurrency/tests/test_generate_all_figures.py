...
def _write_concurrency_timeseries(run_dir: Path) -> Path:
    processed_dir = run_dir / "post-processed" / "job-concurrency"
    processed_dir.mkdir(parents=True)
    timeseries_path = processed_dir / "job-concurrency-timeseries.json"
...
def test_discover_run_dirs_with_job_concurrency_scans_recursively(tmp_path: Path) -> None:
    root_dir = tmp_path / "results"
    good_run = root_dir / "a" / "job-ok"
    bad_run = root_dir / "b" / "job-missing-timeseries"

    _write_concurrency_timeseries(good_run)
    bad_processed_dir = bad_run / "post-processed" / "job-concurrency"
    bad_processed_dir.mkdir(parents=True)

    discovered = generate_all_figures.discover_run_dirs_with_job_concurrency(root_dir)
...
def test_generate_figure_for_run_dir_writes_manifest(tmp_path: Path, monkeypatch) -> None:
    ...
    _write_concurrency_timeseries(run_dir)

    def fake_render_concurrency_figure(
        *,
    ...
    monkeypatch.setattr(
        ...
        "_render_concurrency_figure",
        fake_render_concurrency_figure,
    )
    ...
    assert manifest["figure_file_name"] == "job-concurrency.png"
    assert manifest["image_format"] == "png"
    assert manifest["dpi"] == 150
    assert manifest["sample_count"] == 3
    assert manifest["avg_concurrency"] == 2.0
    assert manifest["max_concurrency"] == 3.0
...
def test_generate_figure_for_run_dir_writes_profile_specific_figures(
    ...
    processed_dir = run_dir / "post-processed" / "job-concurrency"
    processed_dir.mkdir(parents=True)
    (processed_dir / "job-concurrency-timeseries.json").write_text(
        json.dumps(
            {
                "source_type": "replay",
                "replay_count": 2,
                "jobs_with_valid_range_count": 2,
                "total_duration_s": 3.0,
                "concurrency_points": [{"second": 0, "concurrency": 1}],
                "multi_profile": True,
    ...
    assert any(
        figure["figure_file_name"] == "job-concurrency.png"
...
def test_main_root_dir_processes_discovered_runs(monkeypatch, tmp_path: Path) -> None:
    ...
    run_a = root_dir / "a"
    run_b = root_dir / "b"
    _write_concurrency_timeseries(run_a)
    _write_concurrency_timeseries(run_b)
    ...
    def fake_generate_figure_for_run_dir(
        ...
        *,
        ...
        return run_dir / "post-processed" / "visualization" / "job-concurrency" / "figures-manifest.json"
