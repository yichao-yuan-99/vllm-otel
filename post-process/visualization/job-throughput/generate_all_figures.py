...
DEFAULT_INPUT_NAME = "job-throughput-timeseries.json"
DEFAULT_OUTPUT_REL_PATH = Path("post-processed/visualization/job-throughput")
DEFAULT_MANIFEST_NAME = "figures-manifest.json"
DEFAULT_FIGURE_STEM = "job-throughput"
DEFAULT_EXCLUDING_CANCELLED_FIGURE_STEM = "job-throughput-excluding-cancelled"
DEFAULT_FORMAT = "png"
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate job-throughput line-chart figures from extracted throughput timeseries."
    )
    target_group = parser.add_mutually_exclusive_group(required=True)
    target_group.add_argument(
        "--run-dir",
        default=None,
        help="Run result root directory containing post-processed/job-throughput/.",
    )
    target_group.add_argument(
        "--root-dir",
        default=None,
        help=(
            ...
            "with post-processed/job-throughput/job-throughput-timeseries.json "
            "will be processed."
        ),
    )
    parser.add_argument(
        ...
        help=(
            ...
            "<run-dir>/post-processed/job-throughput/"
...
def _default_timeseries_input_path_for_run(run_dir: Path) -> Path:
    return (run_dir / "post-processed" / "job-throughput" / DEFAULT_INPUT_NAME).resolve()
...
def discover_run_dirs_with_job_throughput(root_dir: Path) -> list[Path]:
    run_dirs: set[Path] = set()
    for timeseries_path in root_dir.rglob(DEFAULT_INPUT_NAME):
        if not timeseries_path.is_file():
            continue
        if timeseries_path.parent.name != "job-throughput":
            continue
...
def _extract_xy_from_points(raw_points: Any) -> tuple[list[float], list[float]]:
    ...
    for item in raw_points:
        ...
        throughput_jobs_per_s = _float_or_none(item.get("throughput_jobs_per_s"))
        if time_s is None or throughput_jobs_per_s is None:
            continue
        pairs.append((time_s, throughput_jobs_per_s))
    if not pairs:
        return [], []
...
def _build_stats_annotation(
    *,
    ...
    replay_count = _int_or_none(series_payload.get("replay_count"))
    ...
    if replay_count is not None:
        if finished_replay_count is None:
            jobs_line = f"jobs: ? / {replay_count}"
        else:
            jobs_line = f"jobs: {finished_replay_count} / {replay_count}"
...
def _import_matplotlib_pyplot() -> Any:
    try:
        ...
        raise RuntimeError(
            "matplotlib is required to generate job-throughput figures. "
...
def _render_throughput_figure(
    *,
    ...
    axis.plot(x_values, y_values, color="#0F766E", linewidth=2.2, alpha=0.97)
    axis.fill_between(x_values, y_values, color="#99F6E4", alpha=0.25)
    ...
    axis.set_ylabel("Throughput (jobs/s)")

    subtitle_parts: list[str] = []
    source_type = series_payload.get("source_type")
    ...
    variants = [
        {
            ...
            "finished_count_key": "finished_replay_count",
            "figure_title": "Job Throughput Over Time",
            "figure_variant_label": "all finished statuses",
        }
    ]
    if isinstance(timeseries_payload.get("throughput_points_excluding_cancelled"), list):
        variants.append(
            {
                "variant_id": "excluding-cancelled",
                "figure_stem": DEFAULT_EXCLUDING_CANCELLED_FIGURE_STEM,
                "points_key": "throughput_points_excluding_cancelled",
                "finished_count_key": "finished_replay_count_excluding_cancelled",
                "figure_title": "Job Throughput Over Time",
                "figure_variant_label": "excluding cancelled",
            }
        )
    return variants
...
def generate_figure_for_run_dir(
    ...
    *,
    ...
    manifest = {
        ...
        "avg_throughput_jobs_per_s": (
            primary_figure["avg_throughput_jobs_per_s"] if primary_figure else None
        ),
        "min_throughput_jobs_per_s": (
            primary_figure["min_throughput_jobs_per_s"] if primary_figure else None
        ),
        "max_throughput_jobs_per_s": (
            primary_figure["max_throughput_jobs_per_s"] if primary_figure else None
        ),
        ...
        "finished_replay_count": _int_or_none(timeseries_payload.get("finished_replay_count")),
        "finished_replay_count_excluding_cancelled": _int_or_none(
            timeseries_payload.get("finished_replay_count_excluding_cancelled")
        ),
        "cancelled_finished_replay_count": _int_or_none(
            timeseries_payload.get("cancelled_finished_replay_count")
        ),
...
def _main_root_dir(args: argparse.Namespace) -> int:
    ...
    run_dirs = discover_run_dirs_with_job_throughput(root_dir)
