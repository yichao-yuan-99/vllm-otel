...
DEFAULT_INPUT_NAME = "power-summary.json"
DEFAULT_OUTPUT_REL_PATH = Path("post-processed/visualization/power")
DEFAULT_MANIFEST_NAME = "figures-manifest.json"
DEFAULT_FIGURE_STEM = "gpu-power-over-time"
DEFAULT_FORMAT = "png"
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate GPU power-over-time figures from extracted power summary."
    )
    target_group = parser.add_mutually_exclusive_group(required=True)
    target_group.add_argument(
        "--run-dir",
        default=None,
        help="Run result root directory containing post-processed/power/.",
    )
    target_group.add_argument(
        "--root-dir",
        default=None,
        help=(
            ...
            "with post-processed/power/power-summary.json will be processed."
        ),
    )
    parser.add_argument(
        "--power-input",
        default=None,
        help=(
            "Optional power summary input path. Default: "
            "<run-dir>/post-processed/power/"
...
def _default_power_input_path_for_run(run_dir: Path) -> Path:
    return (run_dir / "post-processed" / "power" / DEFAULT_INPUT_NAME).resolve()
...
def discover_run_dirs_with_power_summary(root_dir: Path) -> list[Path]:
    run_dirs: set[Path] = set()
    for power_summary_path in root_dir.rglob(DEFAULT_INPUT_NAME):
        if not power_summary_path.is_file():
            continue
        if power_summary_path.parent.name != "power":
            continue
        if power_summary_path.parent.parent.name != "post-processed":
            continue
        run_dirs.add(power_summary_path.parent.parent.parent.resolve())
...
def _extract_xy(power_summary_payload: dict[str, Any]) -> tuple[list[float], list[float]]:
    ...
    for point in raw_points:
        ...
        if time_offset_s is None or power_w is None:
            continue
        pairs.append((time_offset_s, power_w))
    if not pairs:
        return [], []
...
def _build_stats_annotation(
    *,
    ...
    total_energy_kwh = _float_or_none(power_summary_payload.get("total_energy_kwh"))
    total_energy_j = _float_or_none(power_summary_payload.get("total_energy_j"))
    return (
        f"power log: {'yes' if power_log_found else 'no'}\n"
        f"samples: {summary_stats['sample_count']}\n"
        f"avg: {_format_stat_value(_float_or_none(summary_stats.get('avg')))} W\n"
        f"min: {_format_stat_value(_float_or_none(summary_stats.get('min')))} W\n"
        f"max: {_format_stat_value(_float_or_none(summary_stats.get('max')))} W\n"
...
def _import_matplotlib_pyplot() -> Any:
    try:
        ...
        raise RuntimeError(
            "matplotlib is required to generate power figures. "
            "Install it in your environment, for example: pip install matplotlib"
        ) from exc
    return plt


def _render_power_figure(
    *,
    power_summary_payload: dict[str, Any],
    output_path: Path,
    image_format: str,
    ...
    x_values, y_values = _extract_xy(power_summary_payload)
    if not x_values:
        return False
    ...
    axis.plot(x_values, y_values, color="#B45309", linewidth=2.2, alpha=0.97)
    axis.fill_between(x_values, y_values, color="#FCD34D", alpha=0.25)
    ...
    if peak_time_s is not None and max_value is not None:
        ...
        axis.annotate(
            f"Peak {max_value:.3g}W @ {peak_time_s:.3g}s",
...
def generate_figure_for_run_dir(
    ...
    *,
    ...
    resolved_power_input_path = (
        power_input_path or _default_power_input_path_for_run(resolved_run_dir)
    ).expanduser().resolve()
    resolved_output_dir = (
        output_dir or _default_output_dir_for_run(resolved_run_dir)
    ).expanduser().resolve()

    if not resolved_power_input_path.is_file():
        raise ValueError(f"Missing power summary file: {resolved_power_input_path}")
    if dpi <= 0:
        raise ValueError(f"dpi must be a positive integer: {dpi}")

    power_summary_payload = _load_json(resolved_power_input_path)
    if not isinstance(power_summary_payload, dict):
        raise ValueError(f"Power summary JSON must be an object: {resolved_power_input_path}")
    ...
    manifest = {
        ...
        "source_power_summary_path": str(resolved_power_input_path),
        ...
        "power_sample_count": _int_or_none(power_summary_payload.get("power_sample_count")),
        "total_energy_j": _float_or_none(power_summary_payload.get("total_energy_j")),
        "total_energy_kwh": _float_or_none(power_summary_payload.get("total_energy_kwh")),
        "source_type": power_summary_payload.get("source_type"),
...
def _main_run_dir(args: argparse.Namespace) -> int:
    ...
    power_input_path = Path(args.power_input).expanduser().resolve() if args.power_input else None
    output_dir = Path(args.output_dir).expanduser().resolve() if args.output_dir else None
    output_path = generate_figure_for_run_dir(
        run_dir,
        power_input_path=power_input_path,
...
def _main_root_dir(args: argparse.Namespace) -> int:
    if args.power_input:
        raise ValueError("--power-input can only be used with --run-dir")
    if args.output_dir:
    ...
    run_dirs = discover_run_dirs_with_power_summary(root_dir)
