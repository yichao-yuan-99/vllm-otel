...
DEFAULT_CONFIG_PATH = REPO_ROOT / "gateway_lite" / "config.toml"
...
def load_runtime_settings(
    ...
    *,
    ...
    return GatewayRuntimeSettings(
        ...
        service_name="vllm-gateway-lite",
        artifact_compression=_parse_optional_str(
            gateway_table.get("artifact_compression"),
            "gateway.artifact_compression",
        ) or "none",
        upstream_api_key=_parse_optional_str(
            gateway_table.get("upstream_api_key"),
            "gateway.upstream_api_key",
        ),
    )
