...
def load_generator_module() -> object:
    ...
    spec = importlib.util.spec_from_file_location(
        "generate_sweep_qps_docker_power_clean_freq_ctrl_linespace_multi_experiment",
        module_path,
    )
...
def test_main_generates_sweep_qps_docker_power_clean_freq_ctrl_linespace_multi_bundle(
    tmp_path: Path,
) -> None:
    ...
    assert replay_qps025["output_dir"].endswith(
        "results/replay/sweep-qps-docker-power-clean-freq-ctrl-linespace-multi/"
    ...
    assert manifest["gpu_indices"] == [2, 3]
    ...
    assert manifest["qps_points"][0]["freq_controller_log_dir"].endswith(
        "results/replay/sweep-qps-docker-power-clean-freq-ctrl-linespace-multi/"
        f"dataset-a/agent-a/split/rest/qps0_25/{batch_timestamp}/freq-control-linespace-multi"
    )
    ...
    assert "[sweep-qps-docker-power-clean-freq-ctrl-linespace-multi]" in run_script_text
    ...
    assert (
        ...
        "\"${replay_output_dir}/freq-control-linespace-multi\""
        in run_script_text
    )
    assert "\"${ZEUS_POWER_READER_BIN}\" --output-dir \"${power_output_dir}\" --gpu-indices \"${POWER_GPU_INDICES[@]}\"" in run_script_text
    ...
    assert "\"--gpu-indices\" \"${POWER_GPU_INDICES[@]}\"" in run_script_text
    assert "for gpu_index in \"${POWER_GPU_INDICES[@]}\"; do" in run_script_text
...
def test_main_supports_power_gpu_indices_alias_and_custom_threshold(
    tmp_path: Path,
) -> None:
    ...
    assert manifest["qps_points"][0]["freq_controller_log_dir"].endswith(
        "results/replay/sweep-qps-docker-power-clean-freq-ctrl-linespace-multi/"
        f"dataset-a/agent-a/split/exclude-unranked/qps0_5/{batch_timestamp}/freq-control-linespace-multi"
    )
...
def test_main_rejects_duplicate_gpu_indices(tmp_path: Path) -> None:
    module, _, source_run_dir = prepare_module_and_source_run(tmp_path)
    output_config_dir = tmp_path / "generated"
