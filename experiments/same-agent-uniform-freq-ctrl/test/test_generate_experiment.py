from __future__ import annotations

import importlib.util
import json
import re
import stat
import sys
import tomllib
from pathlib import Path


def load_generator_module() -> object:
    module_path = (Path(__file__).resolve().parents[1] / "generate_experiment.py").resolve()
    spec = importlib.util.spec_from_file_location(
        "generate_same_agent_uniform_freq_ctrl_experiment",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise RuntimeError(f"Unable to load module from {module_path}")
    module = importlib.util.module_from_spec(spec)
    sys.modules["generate_same_agent_uniform_freq_ctrl_experiment"] = module
    spec.loader.exec_module(module)
    return module


def resolve_single_output_batch_dir(output_config_root: Path) -> Path:
    candidates = sorted(path for path in output_config_root.iterdir() if path.is_dir())
    assert len(candidates) == 1
    batch_dir = candidates[0]
    assert re.fullmatch(r"\d{8}T\d{6}Z", batch_dir.name) is not None
    return batch_dir


def write_minimal_model_config(path: Path) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(
        "\n".join(
            [
                "schema_version = 1",
                "",
                "[models.test_model]",
                'served_model_name = "Test Served Model"',
                'vllm_model_name = "org/test-model"',
            ]
        )
        + "\n",
        encoding="utf-8",
    )


def write_single_trail_plan(
    plan_path: Path,
    *,
    single_trail: str | None = "profile-2/run_beta",
    source_job_dir: str = "/remote/work/results/model-a/dataset-a/agent-a/run-1",
    is_derived: bool = False,
) -> None:
    plan_path.parent.mkdir(parents=True, exist_ok=True)
    compile_options = {
        "exclude_unranked_trails": False,
        "clean": False,
    }
    if single_trail is not None:
        compile_options["single_trail"] = single_trail

    plan_payload = {
        "compile_options": compile_options,
        "source_job_dir": source_job_dir,
        "replay_target": {
            "model": "Test Served Model",
        },
        "launch_policy": {
            "strategy": "config_ordered",
            "max_concurrent": 1,
            "pattern": {"name": "eager"},
            "pattern_args": {},
        },
        "workers": [],
    }
    if is_derived:
        plan_payload["is_derived"] = True
        plan_payload["derived_single_plan"] = {
            "seed": 7,
            "size": 500,
            "source_plan_path": "/remote/work/results/model-a/dataset-a/agent-a/run-1/replay-plan.trail-profile-2_run_beta.json",
        }
    plan_path.write_text(
        json.dumps(plan_payload, indent=2, ensure_ascii=True) + "\n",
        encoding="utf-8",
    )


def test_main_generates_same_agent_uniform_freq_ctrl_bundle(tmp_path: Path) -> None:
    module = load_generator_module()
    repo_root = tmp_path / "repo"
    module.REPO_ROOT = repo_root
    module.MODEL_CONFIG_PATH = repo_root / "configs" / "model_config.toml"
    write_minimal_model_config(module.MODEL_CONFIG_PATH)

    source_plan_path = tmp_path / "input-plan" / "replay-plan.trail-profile-2_run_beta.json"
    write_single_trail_plan(source_plan_path, is_derived=True)

    output_config_dir = tmp_path / "generated"
    exit_code = module.main(
        [
            "--source-plan",
            str(source_plan_path),
            "--randomize-seed",
            "11",
            "--qps",
            "0.25",
            "--time-constraint-s",
            "600",
            "--target-model",
            "test_model",
            "--port-profile",
            "3",
            "--gpu-id",
            "2",
            "--output-config-dir",
            str(output_config_dir),
        ]
    )
    assert exit_code == 0

    batch_dir = resolve_single_output_batch_dir(output_config_dir)
    batch_timestamp = batch_dir.name
    expected_plan_copy = batch_dir / "plan" / source_plan_path.name

    replay_config = tomllib.loads((batch_dir / "replay.toml").read_text(encoding="utf-8"))
    replay = replay_config["replay"]
    assert replay["randomize_seed"] == 11
    assert replay["time_constraint_s"] == 600.0
    assert replay["port_profile_id"] == 3
    assert replay["launch_policy_override"]["pattern"]["name"] == "uniform"
    assert replay["launch_policy_override"]["pattern_args"]["rate"] == 0.25
    assert "seed" not in replay["launch_policy_override"]
    assert replay["plan"] == module.path_for_config(expected_plan_copy)
    assert replay["output_dir"].endswith(
        "results/replay/same-agent-uniform-freq-ctrl/"
        f"dataset-a/agent-a/trail/profile-2_run_beta/qps0_25/{batch_timestamp}"
    )

    manifest = json.loads((batch_dir / "manifest.json").read_text(encoding="utf-8"))
    assert manifest["status"] == "ok"
    assert manifest["source_plan"] == str(source_plan_path)
    assert manifest["source_plan_copy"] == str(expected_plan_copy)
    assert manifest["source_plan_is_derived"] is True
    assert manifest["source_job_dir"] == "/remote/work/results/model-a/dataset-a/agent-a/run-1"
    assert manifest["source_dataset_lineage"] == "dataset-a/agent-a"
    assert manifest["source_trail_name"] == "profile-2/run_beta"
    assert manifest["source_trail_slug"] == "profile-2_run_beta"
    assert manifest["source_plan_model"] == "Test Served Model"
    assert manifest["target_model"] == "test_model"
    assert manifest["launch_pattern"] == "uniform"
    assert manifest["qps"] == 0.25
    assert manifest["qps_slug"] == "qps0_25"
    assert manifest["gpu_id"] == 2
    assert manifest["power_gpu_indices"] == [2]
    assert manifest["freq_controller_lower_bound"] == 369364.0
    assert manifest["freq_controller_upper_bound"] == 395784.0
    assert manifest["replay_output_root_base_dir"].endswith(
        "results/replay/same-agent-uniform-freq-ctrl/"
        "dataset-a/agent-a/trail/profile-2_run_beta"
    )
    assert manifest["replay_output_dir"].endswith(
        "results/replay/same-agent-uniform-freq-ctrl/"
        f"dataset-a/agent-a/trail/profile-2_run_beta/qps0_25/{batch_timestamp}"
    )
    assert manifest["power_output_dir"].endswith(
        "results/replay/same-agent-uniform-freq-ctrl/"
        f"dataset-a/agent-a/trail/profile-2_run_beta/qps0_25/{batch_timestamp}/power"
    )
    assert manifest["freq_controller_log_dir"].endswith(
        "results/replay/same-agent-uniform-freq-ctrl/"
        f"dataset-a/agent-a/trail/profile-2_run_beta/qps0_25/{batch_timestamp}/freq-control"
    )

    run_script_path = batch_dir / "run_replay.sh"
    run_script_text = run_script_path.read_text(encoding="utf-8")
    assert "[same-agent-uniform-freq-ctrl]" in run_script_text
    assert "SOURCE_TRAIL_NAME=profile-2/run_beta" in run_script_text
    assert "\"${ZEUS_POWER_READER_BIN}\" --output-dir \"${POWER_OUTPUT_DIR}\" --gpu-indices \"${GPU_ID}\"" in run_script_text
    assert "FREQ_CONTROLLER_LOG_DIR=\"${REPLAY_OUTPUT_DIR}/freq-control\"" in run_script_text
    assert "FREQ_CONTROLLER_BIN" in run_script_text
    assert "FREQ_CONTROLLER_CONFIG_VALUE" in run_script_text
    assert "DEFAULT_FREQ_CONTROLLER_LOWER_BOUND=369364.0" in run_script_text
    assert "DEFAULT_FREQ_CONTROLLER_UPPER_BOUND=395784.0" in run_script_text
    assert "\"--lower-bound\" \"${FREQ_CONTROLLER_LOWER_BOUND_VALUE}\"" in run_script_text
    assert "\"--upper-bound\" \"${FREQ_CONTROLLER_UPPER_BOUND_VALUE}\"" in run_script_text
    assert "\"--port-profile-id\" \"${PORT_PROFILE_ID_VALUE}\"" in run_script_text
    assert "\"--gpu-index\" \"${GPU_ID}\"" in run_script_text
    assert "\"${PYTHON_BIN}\" -m replayer replay" in run_script_text
    assert run_script_path.stat().st_mode & stat.S_IXUSR


def test_main_applies_output_suffix_and_derived_power_gpu_id(tmp_path: Path) -> None:
    module = load_generator_module()
    repo_root = tmp_path / "repo"
    module.REPO_ROOT = repo_root
    module.MODEL_CONFIG_PATH = repo_root / "configs" / "model_config.toml"
    write_minimal_model_config(module.MODEL_CONFIG_PATH)

    source_plan_path = tmp_path / "input-plan" / "replay-plan.trail-profile-2_run_beta.json"
    write_single_trail_plan(source_plan_path, is_derived=True)

    output_config_dir = tmp_path / "generated"
    exit_code = module.main(
        [
            "--source-plan",
            str(source_plan_path),
            "--randomize-seed",
            "11",
            "--qps",
            "0.5",
            "--time-constraint-s",
            "600",
            "--target-model",
            "test_model",
            "--port-profile",
            "3",
            "--gpu-id",
            "2",
            "--output-suffix",
            "lmcache",
            "--freq-controller-lower-bound",
            "100",
            "--freq-controller-upper-bound",
            "200",
            "--output-config-dir",
            str(output_config_dir),
        ]
    )
    assert exit_code == 0

    batch_dir = resolve_single_output_batch_dir(output_config_dir)
    batch_timestamp = batch_dir.name
    replay_config = tomllib.loads((batch_dir / "replay.toml").read_text(encoding="utf-8"))
    assert replay_config["replay"]["output_dir"].endswith(
        "results/replay/same-agent-uniform-freq-ctrl/"
        f"dataset-a/agent-a/trail-lmcache/profile-2_run_beta/qps0_5/{batch_timestamp}"
    )

    manifest = json.loads((batch_dir / "manifest.json").read_text(encoding="utf-8"))
    assert manifest["output_suffix"] == "lmcache"
    assert manifest["replay_output_trail_dir_name"] == "trail-lmcache"
    assert manifest["gpu_id"] == 2
    assert manifest["power_gpu_indices"] == [2]
    assert manifest["freq_controller_lower_bound"] == 100.0
    assert manifest["freq_controller_upper_bound"] == 200.0

    run_script_text = (batch_dir / "run_replay.sh").read_text(encoding="utf-8")
    assert "\"${ZEUS_POWER_READER_BIN}\" --output-dir \"${POWER_OUTPUT_DIR}\" --gpu-indices \"${GPU_ID}\"" in run_script_text
    assert "POWER_GPU_INDICES" not in run_script_text
    assert "DEFAULT_FREQ_CONTROLLER_LOWER_BOUND=100.0" in run_script_text
    assert "DEFAULT_FREQ_CONTROLLER_UPPER_BOUND=200.0" in run_script_text


def test_main_rejects_original_single_trail_plan(tmp_path: Path) -> None:
    module = load_generator_module()
    repo_root = tmp_path / "repo"
    module.REPO_ROOT = repo_root
    module.MODEL_CONFIG_PATH = repo_root / "configs" / "model_config.toml"
    write_minimal_model_config(module.MODEL_CONFIG_PATH)

    source_plan_path = tmp_path / "input-plan" / "replay-plan.trail-profile-2_run_beta.json"
    write_single_trail_plan(source_plan_path, is_derived=False)

    output_config_dir = tmp_path / "generated"
    exit_code = module.main(
        [
            "--source-plan",
            str(source_plan_path),
            "--randomize-seed",
            "11",
            "--qps",
            "0.25",
            "--time-constraint-s",
            "600",
            "--target-model",
            "test_model",
            "--port-profile",
            "3",
            "--gpu-id",
            "2",
            "--output-config-dir",
            str(output_config_dir),
        ]
    )
    assert exit_code == 1
