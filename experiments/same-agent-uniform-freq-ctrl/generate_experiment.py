#!/usr/bin/env python3
"""Generate one same-agent uniform replay bundle with freq-controller."""
...
EXPERIMENT_DIR_NAME = "same-agent-uniform-freq-ctrl"
...
def write_run_script(
    ...
    *,
    ...
    lines = [
        ...
        f"QPS_VALUE={_shell_quote(qps_display)}",
        f"QPS_SLUG={_shell_quote(qps_slug)}",
        ...
        "",
        f"REPLAY_CONFIG_REF={_shell_quote(replay_config_relpath)}",
        f"REPLAY_OUTPUT_REF={_shell_quote(path_for_config(replay_output_dir))}",
        "",
        "POWER_READER_PID=\"\"",
        "FREQ_CONTROLLER_PID=\"\"",
        ...
        "fi",
        "",
        f"echo \"[{EXPERIMENT_LOG_TAG}] starting replay qps=${{QPS_VALUE}} slug=${{QPS_SLUG}} output=${{REPLAY_OUTPUT_REF}}\"",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        description=(
            "Generate a same-agent uniform single-QPS replay bundle that launches "
            "freq-controller before replay and keeps power logging enabled."
        ),
    )
    parser.add_argument(
        "--source-plan",
    ...
    parser.add_argument(
        ...
        required=True,
        type=float,
        help="Uniform launch rate in requests per second.",
    )
    parser.add_argument(
        "--time-constraint-s",
        required=True,
    ...
    parser.add_argument(
        ...
        type=parse_non_negative_int,
        help=(
            "GPU index used by both freq-controller and zeus-power-reader."
        ),
    )
    parser.add_argument(
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        summary = {
            ...
            "power_output_dir": path_for_config((replay_output_dir / "power").resolve()),
            "freq_controller_log_dir": path_for_config(
                (replay_output_dir / DEFAULT_FREQ_CONTROL_LOG_DIR_NAME).resolve()
            ),
