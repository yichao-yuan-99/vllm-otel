...
def write_replay_config(path: Path, *, replay_payload: dict[str, Any]) -> None:
    lines: list[str] = [
        "# Auto-generated by experiments/sweep-qps-docker-power-clean/generate_experiment.py",
        "",
    ]
...
def write_run_script(
    ...
    *,
    ...
    lines = [
        ...
        "trap 'echo \"[sweep-qps-docker-power-clean] interrupted\" >&2; exit 130' INT TERM",
        "",
        "run_one_qps() {",
        "  local qps_value=\"$1\"",
        "  local qps_slug=\"$2\"",
        ...
        "",
        "  echo \"[sweep-qps-docker-power-clean] qps=${qps_value} slug=${qps_slug} output=${replay_output_ref} port_profile=${PORT_PROFILE_ID_VALUE}\"",
        ...
        "",
        (
            f"echo \"[sweep-qps-docker-power-clean] target_model={target_model} split={split} "
    ...
    lines.extend(
        [
            "",
            f"echo \"[sweep-qps-docker-power-clean] completed {len(qps_jobs)} qps points\"",
            "",
        ]
    )
