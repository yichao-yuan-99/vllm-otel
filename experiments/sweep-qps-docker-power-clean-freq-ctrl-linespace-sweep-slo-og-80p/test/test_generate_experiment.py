...
def load_generator_module() -> object:
    ...
    spec = importlib.util.spec_from_file_location(
        "generate_sweep_qps_docker_power_clean_freq_ctrl_linespace_sweep_slo_og_80p_experiment",
        module_path,
    )
...
def test_main_generates_gateway_only_slo_sweep_qps_slo_bundle(tmp_path: Path) -> None:
    ...
    assert replay_qps025_slo8["output_dir"].endswith(
        "results/replay/sweep-qps-docker-power-clean-freq-ctrl-linespace-sweep-slo-og-80p/"
    ...
    assert "[sweep-qps-docker-power-clean-freq-ctrl-linespace-sweep-slo-og-80p]" in run_script_text
