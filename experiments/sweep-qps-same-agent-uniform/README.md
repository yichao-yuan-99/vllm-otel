# Sweep QPS Same Agent Uniform

This workflow is the fixed-interval sibling of
`experiments/sweep-qps-same-agent/`.
...
- `results/replay/sweep-qps-same-agent-uniform/<dataset-lineage>/trail/<safe-source-trail>/<qps>/<timestamp>/`
...
```bash
python3 experiments/sweep-qps-same-agent-uniform/generate_experiment.py \
...
```bash
bash experiments/sweep-qps-same-agent-uniform/generated/<timestamp>/run_replay.sh
```

Override port profile at runtime:

```bash
bash experiments/sweep-qps-same-agent-uniform/generated/<timestamp>/run_replay.sh 3
```
