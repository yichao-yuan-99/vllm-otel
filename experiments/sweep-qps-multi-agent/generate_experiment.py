#!/usr/bin/env python3
"""Generate one multi-plan sweep-QPS + power logging replay experiment bundle.

This helper does all of the following:
1) Validates each selected replay plan exists and was compiled with --single-trail.
2) Validates each plan's replay target matches the requested target model.
3) Materializes per-plan per-QPS replay TOMLs + a runnable 2D sweep script with power logging.
"""
...
DEFAULT_OUTPUT_CONFIG_DIR = (
    REPO_ROOT / "experiments" / "sweep-qps-multi-agent" / "generated"
)
DEFAULT_REPLAY_OUTPUT_ROOT = Path("results") / "replay" / "sweep-qps-multi-agent"
...
class SourcePlanEntry:
    ...
    source_plan_copy_path: Path
    source_job_dir_text: str | None
    source_dataset_lineage: Path | None
    source_trail_name: str
    source_trail_slug: str
    batch_trail_slug: str
    source_plan_model: str
    replay_output_root_base_dir: Path


@dataclass(frozen=True)
class MatrixReplayJob:
...
def build_unique_slug(base_slug: str, used_slugs: set[str]) -> str:
    candidate = base_slug
    suffix = 2
...
def format_plan_copy_name(index: int, original_name: str) -> str:
    return f"plan-{index:02d}.{original_name}"
...
def write_replay_config(path: Path, *, replay_payload: dict[str, Any]) -> None:
    lines: list[str] = [
        "# Auto-generated by experiments/sweep-qps-multi-agent/generate_experiment.py",
        "",
    ]
...
def write_run_script(
    ...
    *,
    ...
    matrix_jobs: list[MatrixReplayJob],
    qps_count: int,
) -> None:
    ...
    lines = [
        ...
        "trap 'echo \"[sweep-qps-multi-agent] interrupted\" >&2; exit 130' INT TERM",
        "",
        "run_one_job() {",
        "  local batch_trail_slug=\"$1\"",
        "  local source_trail_name=\"$2\"",
        "  local qps_value=\"$3\"",
        "  local qps_slug=\"$4\"",
        ...
        "",
        "  echo \"[sweep-qps-multi-agent] trail=${source_trail_name} batch_trail_slug=${batch_trail_slug} qps=${qps_value} slug=${qps_slug} output=${replay_output_ref} port_profile=${PORT_PROFILE_ID_VALUE}\"",
        ...
        "",
        (
            f"echo \"[sweep-qps-multi-agent] target_model={target_model} "
            f"trail_points={source_plan_count} "
            f"qps_points={qps_count} "
            f"total_runs={len(matrix_jobs)} "
            f"power_gpu_indices={power_gpu_indices_payload} "
    ...
    for job in matrix_jobs:
        lines.append(
            "run_one_job "
            f"{_shell_quote(job.batch_trail_slug)} "
            f"{_shell_quote(job.source_trail_name)} "
    ...
    lines.extend(
        [
            "",
            f"echo \"[sweep-qps-multi-agent] completed {len(matrix_jobs)} total runs\"",
            "",
        ]
    )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        description=(
            "Generate a 2D sweep-QPS replay bundle with power logging from a list of "
            "precompiled single-trail replay plans and emit one sweep entrypoint script."
        ),
    )
    parser.add_argument(
        "--source-plan",
        ...
        required=True,
        help=(
            "Repeatable. Path to a replay plan compiled with "
    ...
    parser.add_argument(
        ...
        help=(
            "Replay output root. Each plan keeps its own lineage and is written under "
            "<dataset-lineage>/trail/<batch-trail-slug>/<qps>/<timestamp> below "
            "results/replay/sweep-qps-multi-agent/. If a plan's lineage cannot be "
            "derived, that plan falls back to trail/<batch-trail-slug>/..."
        ),
    )
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        source_plan_paths = [
            ...
            for path_text in args.source_plan
        ]
        if not source_plan_paths:
            raise ValueError("at least one --source-plan is required")
        for source_plan_path in source_plan_paths:
        ...
        used_batch_trail_slugs: set[str] = set()
        source_plan_entries: list[SourcePlanEntry] = []
        for index, source_plan_path in enumerate(source_plan_paths):
            source_trail_name = _extract_compile_single_trail_from_plan(source_plan_path)
            if source_trail_name is None:
                raise ValueError(
                    f"--source-plan {source_plan_path} must be a replay plan compiled with --single-trail"
                )
            ...
            batch_trail_slug = build_unique_slug(source_trail_slug, used_batch_trail_slugs)
            source_plan_model = _validate_plan_model_matches_target(
                plan_path=source_plan_path,
                target_model_spec=target_model_spec,
            )

            source_plan_copy_path = (
                plan_copy_dir / format_plan_copy_name(index, source_plan_path.name)
            ).resolve()
            ...
            replay_output_root_base = (
                replay_output_root_base / "trail" / batch_trail_slug
            ).resolve()

            source_plan_entries.append(
                SourcePlanEntry(
                    index=index,
                    source_plan_path=source_plan_path,
                    source_plan_copy_path=source_plan_copy_path,
                    source_job_dir_text=source_job_dir_text,
                    source_dataset_lineage=source_dataset_lineage,
                    source_trail_name=source_trail_name,
                    source_trail_slug=source_trail_slug,
                    batch_trail_slug=batch_trail_slug,
                    source_plan_model=source_plan_model,
                    replay_output_root_base_dir=replay_output_root_base,
                )
            )

        matrix_jobs: list[MatrixReplayJob] = []
        for plan_entry in source_plan_entries:
            for qps in qps_values:
                qps_slug = format_qps_slug(qps)
                replay_output_dir = (
                    plan_entry.replay_output_root_base_dir / qps_slug / batch_timestamp
                ).resolve()
                replay_payload = {
                    "plan": path_for_config(plan_entry.source_plan_copy_path),
                ...
                replay_config_path = (
                    batch_dir / "trail" / plan_entry.batch_trail_slug / qps_slug / "replay.toml"
                ).resolve()
                ...
                matrix_jobs.append(
                    MatrixReplayJob(
                        source_plan_index=plan_entry.index,
                        source_trail_name=plan_entry.source_trail_name,
                        batch_trail_slug=plan_entry.batch_trail_slug,
                        qps=qps,
        ...
        write_run_script(
            ...
            source_plan_count=len(source_plan_entries),
        ...
        plan_summary = [
            {
                "source_plan_index": entry.index,
                "source_plan": str(entry.source_plan_path),
                "source_plan_copy": str(entry.source_plan_copy_path),
                "source_job_dir": entry.source_job_dir_text,
                "source_dataset_lineage": (
                    str(entry.source_dataset_lineage)
                    if entry.source_dataset_lineage is not None
                    else None
                ),
                "source_trail_name": entry.source_trail_name,
                "source_trail_slug": entry.source_trail_slug,
                "batch_trail_slug": entry.batch_trail_slug,
                "source_plan_model": entry.source_plan_model,
                "replay_output_root_base_dir": path_for_config(entry.replay_output_root_base_dir),
            }
            for entry in source_plan_entries
        ]
        matrix_summary = [
            {
                "source_plan_index": job.source_plan_index,
                "source_trail_name": job.source_trail_name,
                "batch_trail_slug": job.batch_trail_slug,
                "qps": job.qps,
        ...
        summary = {
            ...
            "trail_points": plan_summary,
            "matrix_points": matrix_summary,
