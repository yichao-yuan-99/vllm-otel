...
def load_generator_module() -> object:
    ...
    spec = importlib.util.spec_from_file_location(
        "generate_sweep_qps_docker_power_clean_gateway_multi_freq_ctrl_linesapce_experiment",
        module_path,
    )
...
def test_main_generates_sweep_qps_docker_power_clean_gateway_multi_freq_ctrl_linesapce_bundle(
    tmp_path: Path,
) -> None:
    ...
    assert replay_qps025["output_dir"].endswith(
        "results/replay/sweep-qps-docker-power-clean-gateway_multi-freq-ctrl-linesapce/"
    ...
    assert "[sweep-qps-docker-power-clean-gateway_multi-freq-ctrl-linesapce]" in run_script_text
    ...
    assert (
        '  freq_controller_log_root_dir="${replay_output_dir}/freq-control-linespace"'
        in run_script_text
    )
...
def test_main_supports_repeated_port_profile_flags_custom_threshold_output_suffix_and_additional_suffix(
    ...
    assert replay_config["replay"]["output_dir"].endswith(
        "results/replay/sweep-qps-docker-power-clean-gateway_multi-freq-ctrl-linesapce-lmcache/"
