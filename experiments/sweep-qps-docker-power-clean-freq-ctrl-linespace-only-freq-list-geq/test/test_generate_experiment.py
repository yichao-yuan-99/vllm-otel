...
def load_generator_module() -> object:
    ...
    spec = importlib.util.spec_from_file_location(
        (
            ...
            "freq_ctrl_linespace_only_freq_list_geq_experiment"
        ),
        module_path,
    )
...
def test_main_generates_sweep_qps_docker_power_clean_freq_ctrl_linespace_bundle(
    tmp_path: Path,
) -> None:
    ...
    assert replay_qps025["output_dir"].endswith(
        "results/replay/"
        "sweep-qps-docker-power-clean-freq-ctrl-linespace-only-freq-list-geq-915/"
    ...
    assert manifest["only_freq_list_geq"] == 915
    ...
    assert "DEFAULT_FREQ_CONTROLLER_ONLY_FREQ_LIST_GEQ=915" in run_script_text
    assert (
        "local freq_controller_log_dir="
        "\"${replay_output_dir}/freq-control-linespace\""
        in run_script_text
    )
...
def test_main_supports_additional_suffix_and_custom_threshold(tmp_path: Path) -> None:
    ...
    assert replay_config["replay"]["output_dir"].endswith(
        "results/replay/"
        "sweep-qps-docker-power-clean-freq-ctrl-linespace-only-freq-list-geq-915-threshold-200/"
