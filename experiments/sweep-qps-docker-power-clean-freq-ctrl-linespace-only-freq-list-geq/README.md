# Sweep QPS Docker Power Clean Freq Ctrl Linespace Only Freq List Geq

This workflow is the `freq-controller-linespace` variant of
`experiments/sweep-qps-docker-power-clean` that always generates a runner with
`--only-freq-list-geq`.
...
- threshold: `395784`
- generated frequency cutoff: `--only-freq-list-geq <required>`

Default replay output layout:

- default threshold (`395784`):
  `results/replay/sweep-qps-docker-power-clean-freq-ctrl-linespace-only-freq-list-geq-<only-freq-list-geq>/<dataset>/<agent>/split/<split>/<qps>/<timestamp>/`
- non-default threshold:
  `results/replay/sweep-qps-docker-power-clean-freq-ctrl-linespace-only-freq-list-geq-<only-freq-list-geq>-threshold-<threshold>/<dataset>/<agent>/split/<split>/<qps>/<timestamp>/`
...
```bash
python3 experiments/sweep-qps-docker-power-clean-freq-ctrl-linespace-only-freq-list-geq/generate_experiment.py \
...
If you do not pass `--replay-output-root`, the generated replay output root is
automatically suffixed with the selected `--only-freq-list-geq` value, and also
with `-threshold-<threshold>` when `--freq-controller-threshold` is non-default.
...
```bash
bash experiments/sweep-qps-docker-power-clean-freq-ctrl-linespace-only-freq-list-geq/generated/<timestamp>/run_replay.sh
```

Override port profile at runtime:

```bash
bash experiments/sweep-qps-docker-power-clean-freq-ctrl-linespace-only-freq-list-geq/generated/<timestamp>/run_replay.sh 3
```
...
- `FREQ_CONTROLLER_ONLY_FREQ_LIST_GEQ=...`
...
such as the frequency list or threshold. The generated runner already passes
`--only-freq-list-geq`, and `FREQ_CONTROLLER_ONLY_FREQ_LIST_GEQ` can override
that generated default at runtime.
...
python3 post-process/run_all.py \
  --root-dir results/replay/sweep-qps-docker-power-clean-freq-ctrl-linespace-only-freq-list-geq-915
```
...
```bash
python3 post-process/run_all.py \
  --root-dir results/replay/sweep-qps-docker-power-clean-freq-ctrl-linespace-only-freq-list-geq-915-threshold-200
```
...
```bash
.venv/bin/pytest experiments/sweep-qps-docker-power-clean-freq-ctrl-linespace-only-freq-list-geq/test -q
```
