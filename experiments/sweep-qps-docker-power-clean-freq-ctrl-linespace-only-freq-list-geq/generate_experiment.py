#!/usr/bin/env python3
"""Generate one local sweep-QPS + power logging + linespace-controller bundle."""
...
DEFAULT_REPLAY_OUTPUT_ROOT_NAME = EXPERIMENT_DIR_NAME
...
def default_replay_output_root_for_params(
    *,
    threshold: float,
    only_freq_list_geq: int,
) -> Path:
    root_name = f"{DEFAULT_REPLAY_OUTPUT_ROOT_NAME}-{only_freq_list_geq}"
    if threshold != DEFAULT_FREQ_CONTROLLER_THRESHOLD:
        root_name = (
            f"{root_name}-threshold-"
...
def write_run_script(
    ...
    *,
    ...
    lines = [
        ...
        f"DEFAULT_FREQ_CONTROLLER_ONLY_FREQ_LIST_GEQ={only_freq_list_geq}",
        (
            "FREQ_CONTROLLER_ONLY_FREQ_LIST_GEQ_VALUE="
            "\"${FREQ_CONTROLLER_ONLY_FREQ_LIST_GEQ:-"
            "${DEFAULT_FREQ_CONTROLLER_ONLY_FREQ_LIST_GEQ}}\""
        ),
        "",
        "POWER_READER_PID=\"\"",
        "FREQ_CONTROLLER_PID=\"\"",
        ...
        (
            ...
            f"slug=${{qps_slug}} gpu_id={gpu_id} "
            "threshold=${FREQ_CONTROLLER_THRESHOLD_VALUE} "
            "only_freq_list_geq=${FREQ_CONTROLLER_ONLY_FREQ_LIST_GEQ_VALUE} "
        ...
        "    \"--threshold\" \"${FREQ_CONTROLLER_THRESHOLD_VALUE}\"",
        "    \"--only-freq-list-geq\" \"${FREQ_CONTROLLER_ONLY_FREQ_LIST_GEQ_VALUE}\"",
        ...
        (
            ...
            "freq-controller-linespace failed to stay alive after startup\" >&2"
        ),
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        description=(
            ...
            "freq-controller-linespace by looking up an existing compiled clean "
    ...
    parser.add_argument(
        ...
        help="GPU index used by both freq-controller-linespace and zeus-power-reader.",
    )
    parser.add_argument(
        ...
        help=(
            "Default freq-controller-linespace context threshold. "
    ...
    parser.add_argument(
        ...
        type=parse_non_negative_int,
        help=(
            "Pass --only-freq-list-geq through to freq-controller-linespace "
            "for every generated QPS run."
        ),
    )
    parser.add_argument(
        "--output-config-dir",
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "results/replay/"
            "sweep-qps-docker-power-clean-freq-ctrl-linespace-only-freq-list-geq-"
            "<only-freq-list-geq>-threshold-<threshold>/. "
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        only_freq_list_geq = int(args.only_freq_list_geq)
        ...
        if _cli_option_provided(argv, "--replay-output-root"):
            ...
            replay_output_root = default_replay_output_root_for_params(
