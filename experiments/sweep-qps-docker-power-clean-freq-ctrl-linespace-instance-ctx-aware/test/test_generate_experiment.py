...
def load_generator_module() -> object:
    ...
    spec = importlib.util.spec_from_file_location(
        "generate_sweep_qps_docker_power_clean_freq_ctrl_linespace_instance_ctx_aware_experiment",
        module_path,
    )
...
def test_main_generates_sweep_qps_docker_power_clean_freq_ctrl_linespace_instance_ctx_aware_bundle(
    tmp_path: Path,
) -> None:
    ...
    assert (
        ...
        "\"${replay_output_dir}/freq-control-linespace-instance\""
        in run_script_text
    )
...
def test_main_supports_additional_suffix_custom_threshold_and_ctx_aware_thresholds(
    tmp_path: Path,
) -> None:
    ...
    assert replay_config["replay"]["output_dir"].endswith(
        "results/replay/sweep-qps-docker-power-clean-freq-ctrl-linespace-instance-ctx-aware-200/"
    ...
    assert manifest["qps_points"][0]["freq_controller_log_dir"].endswith(
        "results/replay/sweep-qps-docker-power-clean-freq-ctrl-linespace-instance-ctx-aware-200/"
        f"dataset-a/agent-a/split/exclude-unranked/qps0_5/{batch_timestamp}/freq-control-linespace-instance"
    )
