#!/usr/bin/env python3
"""Generate one local sweep-QPS + sweep-SLO + power logging bundle."""
...
EXPERIMENT_DIR_NAME = "sweep-qps-docker-power-clean-freq-ctrl-linespace-sweep-slo"
...
def parse_qps_list(raw: str) -> list[float]:
    return parse_positive_float_list(raw, field_name="--qps-list")
...
def format_qps_slug(qps: float) -> str:
    return _format_positive_value_slug(prefix="qps", value=qps)
    ...
    *,
    ...
    lines = [
        ...
        (
            ...
            "qps_slug=${qps_slug} target_slo=${target_slo_value} "
            "target_slo_slug=${target_slo_slug} gpu_id=${GPU_ID} "
            "threshold=${FREQ_CONTROLLER_THRESHOLD_VALUE} output=${replay_output_ref} "
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        description=(
            "Generate a sweep-QPS + sweep-SLO replay bundle with power logging and "
            "freq-controller-linespace-slo by looking up an existing compiled "
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "results/replay/sweep-qps-docker-power-clean-freq-ctrl-linespace-sweep-slo/. "
