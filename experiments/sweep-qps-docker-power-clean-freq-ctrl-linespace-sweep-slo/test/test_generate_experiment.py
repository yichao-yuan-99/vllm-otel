...
def load_generator_module() -> object:
    ...
    spec = importlib.util.spec_from_file_location(
        "generate_sweep_qps_docker_power_clean_freq_ctrl_linespace_sweep_slo_experiment",
        module_path,
    )
...
def test_main_generates_sweep_qps_slo_bundle(tmp_path: Path) -> None:
    ...
    assert "[sweep-qps-docker-power-clean-freq-ctrl-linespace-sweep-slo]" in run_script_text
...
def test_main_supports_additional_suffix_and_custom_threshold(tmp_path: Path) -> None:
    ...
    assert manifest["qps_points"][0]["slo_points"][0]["freq_controller_log_dir"].endswith(
        "results/replay/sweep-qps-docker-power-clean-freq-ctrl-linespace-sweep-slo/"
        f"dataset-a/agent-a/split/exclude-unranked/qps0_5/slo6_5/{batch_timestamp}/freq-control-linespace"
    )
