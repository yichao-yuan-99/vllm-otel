#!/usr/bin/env python3
"""Generate one local single-max-concurrency clean replay experiment bundle.
...
3) Materializes replay config + runnable script for one max-concurrency target.
"""
...
DEFAULT_OUTPUT_CONFIG_DIR = (
    REPO_ROOT / "experiments" / "single-maxcon-clean" / "local" / "generated"
)
DEFAULT_REPLAY_OUTPUT_ROOT = Path("results") / "replay" / "single-maxcon-clean"
...
def write_replay_config(path: Path, *, replay_payload: dict[str, Any]) -> None:
    lines: list[str] = [
        "# Auto-generated by experiments/single-maxcon-clean/local/generate_experiment.py",
        "",
    ]
...
def write_run_script(
    ...
    *,
    ...
    content = (
        ...
        f"echo \"[single-maxcon-clean] target_model={target_model} split={split} "
        f"max_concurrent={max_concurrent} port_profile=${{PORT_PROFILE_ID_VALUE}}\"\n"
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        description=(
            "Generate a local single-max-concurrency clean replay bundle by "
            "looking up an existing compiled clean split plan and emitting a "
            "one-command replay entrypoint script."
        ),
    )
    parser.add_argument(
        "--source-run-dir",
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "results/replay/single-maxcon-clean/. dataset-lineage is inferred "
