...
DEFAULT_OUTPUT_CONFIG_DIR = (
    REPO_ROOT
    / "experiments"
    / "sweep-qps-docker-power-clean-gateway_multi-lowest_usage"
    / "generated"
)
DEFAULT_REPLAY_OUTPUT_ROOT = (
    Path("results") / "replay" / "sweep-qps-docker-power-clean-gateway_multi-lowest_usage"
)
MODEL_CONFIG_PATH = REPO_ROOT / "configs" / "model_config.toml"
EXPERIMENT_DIR_NAME = "sweep-qps-docker-power-clean-gateway_multi-lowest_usage"
EXPERIMENT_LOG_TAG = EXPERIMENT_DIR_NAME
DEFAULT_ASSIGNMENT_POLICY = "lowest_usage"
    ...
    *,
    ...
    lines = [
        ...
        (
            ...
            "output=${replay_output_ref} port_profiles=${PORT_PROFILE_IDS_CSV_NORMALIZED} "
            "control_port_profile=${CONTROL_PORT_PROFILE_ID}\""
        ),
        ...
        (
            ...
            "assignment_policy=${TARGET_ASSIGNMENT_POLICY} "
            "port_profiles=${PORT_PROFILE_IDS_CSV_NORMALIZED} "
            "control_port_profile=${CONTROL_PORT_PROFILE_ID} "
            "gateway=${GATEWAY_BASE_URL_RESOLVED}\""
        ),
        "",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        description=(
            ...
            "split plan, forcing assignment policy lowest_usage, and emitting "
            "one sweep entrypoint script."
        ),
    )
    ...
    parser.add_argument(
        ...
        default="0",
        help=(
            "Comma-separated GPU indices passed to zeus-power-reader, matching "
            "the full gateway_multi backend GPU set, for example: 2,3."
        ),
    )
    parser.add_argument(
        "--output-config-dir",
