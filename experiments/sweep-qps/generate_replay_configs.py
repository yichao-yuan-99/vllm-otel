#!/usr/bin/env python3
"""Generate replay config files for sweep-qps experiments.
...
def derive_replay_output_base(
    ...
    *,
    ...
    return replay_root / Path(*lineage_without_run_dir) / "sweep-qps"
...
def write_replay_config(path: Path, *, replay_payload: dict[str, Any]) -> None:
    lines: list[str] = [
        "# Auto-generated by experiments/sweep-qps/generate_replay_configs.py",
        "",
    ]
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        description="Generate replay config files for sweep-qps runs.",
    )
    parser.add_argument(
        "--source-run-dir",
        required=True,
    ...
    parser.add_argument(
        "--qps-list",
        required=True,
        help="Comma-separated Poisson rates (requests per second), e.g. '1.2,1.5,1.7'",
    )
    parser.add_argument(
        "--poisson-seed",
        required=True,
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Source lineage and sweep-qps/<rate> are appended under this root."
        ),
    )
    parser.add_argument(
        "--port-profile-id",
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        for qps in qps_values:
            ...
            replay_output_dir = (REPO_ROOT / replay_output_base / qps_slug).resolve()
            ...
            config_path = output_config_dir / f"replay.{qps_slug}.toml"
