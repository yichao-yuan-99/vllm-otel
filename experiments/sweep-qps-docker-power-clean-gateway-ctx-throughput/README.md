# Sweep QPS Docker Power Clean Gateway Ctx Throughput
...
Unlike `experiments/sweep-qps-docker-power-clean-gateway-ctx/`, this variant
...
```bash
python3 experiments/sweep-qps-docker-power-clean-gateway-ctx-throughput/generate_experiment.py \
...
```bash
bash experiments/sweep-qps-docker-power-clean-gateway-ctx-throughput/generated/<timestamp>/run_replay.sh
```

Override port profile at runtime:

```bash
bash experiments/sweep-qps-docker-power-clean-gateway-ctx-throughput/generated/<timestamp>/run_replay.sh 3
```
...
- `results/replay/sweep-qps-docker-power-clean-gateway-ctx-throughput/<dataset>/<agent>/split/<split>/<qps>/<timestamp>/`
