...
def load_generator_module() -> object:
    ...
    spec = importlib.util.spec_from_file_location(
        "generate_sweep_qps_same_agent_experiment",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise RuntimeError(f"Unable to load module from {module_path}")
    module = importlib.util.module_from_spec(spec)
    sys.modules["generate_sweep_qps_same_agent_experiment"] = module
...
def test_main_generates_sweep_qps_same_agent_bundle(tmp_path: Path) -> None:
    ...
    replay_config_qps025 = tomllib.loads((batch_dir / "qps0_25" / "replay.toml").read_text(encoding="utf-8"))
    ...
    assert replay_qps025["output_dir"].endswith(
        "results/replay/sweep-qps-same-agent/"
        f"dataset-a/agent-a/trail/profile-2_run_beta/qps0_25/{batch_timestamp}"
    )

    replay_config_qps03 = tomllib.loads((batch_dir / "qps0_3" / "replay.toml").read_text(encoding="utf-8"))
    ...
    assert "[sweep-qps-same-agent]" in run_script_text
...
def test_main_applies_output_suffix_to_trail_dir(tmp_path: Path) -> None:
    module = load_generator_module()
    repo_root = tmp_path / "repo"
    module.REPO_ROOT = repo_root
    module.MODEL_CONFIG_PATH = repo_root / "configs" / "model_config.toml"
    ...
    batch_dir = resolve_single_output_batch_dir(output_config_dir)
    batch_timestamp = batch_dir.name
    replay_config = tomllib.loads((batch_dir / "qps0_25" / "replay.toml").read_text(encoding="utf-8"))
