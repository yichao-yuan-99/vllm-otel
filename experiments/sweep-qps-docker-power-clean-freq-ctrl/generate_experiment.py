#!/usr/bin/env python3
"""Generate one local sweep-QPS + power logging + freq-controller replay bundle."""
...
DEFAULT_OUTPUT_CONFIG_DIR = (
    REPO_ROOT / "experiments" / "sweep-qps-docker-power-clean-freq-ctrl" / "generated"
)
DEFAULT_REPLAY_OUTPUT_ROOT = (
    Path("results") / "replay" / "sweep-qps-docker-power-clean-freq-ctrl"
)
MODEL_CONFIG_PATH = REPO_ROOT / "configs" / "model_config.toml"
EXPERIMENT_DIR_NAME = "sweep-qps-docker-power-clean-freq-ctrl"
...
def write_replay_config(path: Path, *, replay_payload: dict[str, Any]) -> None:
    lines: list[str] = [
        "# Auto-generated by experiments/sweep-qps-docker-power-clean-freq-ctrl/generate_experiment.py",
        "",
    ]
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        description=(
            "Generate a sweep-QPS replay bundle with power logging and freq-controller "
            "by looking up an existing compiled clean split plan and emitting one sweep "
            "entrypoint script."
        ),
    )
