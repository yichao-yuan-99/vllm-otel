#!/usr/bin/env python3
"""Generate one local sweep-QPS + power logging + freq-controller-seg replay bundle."""
...
DEFAULT_OUTPUT_CONFIG_DIR = (
    REPO_ROOT / "experiments" / "sweep-qps-docker-power-clean-freq-ctrl-seg" / "generated"
)
DEFAULT_REPLAY_OUTPUT_ROOT = (
    Path("results") / "replay" / "sweep-qps-docker-power-clean-freq-ctrl-seg"
)
MODEL_CONFIG_PATH = REPO_ROOT / "configs" / "model_config.toml"
EXPERIMENT_DIR_NAME = "sweep-qps-docker-power-clean-freq-ctrl-seg"
...
DEFAULT_FREQ_CONTROL_LOG_DIR_NAME = "freq-control-seg"
...
def write_replay_config(path: Path, *, replay_payload: dict[str, Any]) -> None:
    lines: list[str] = [
        "# Auto-generated by experiments/sweep-qps-docker-power-clean-freq-ctrl-seg/generate_experiment.py",
        "",
    ]
...
def write_run_script(
    ...
    *,
    ...
    lines = [
        ...
        "FREQ_CONTROLLER_BIN=\"${FREQ_CONTROLLER_BIN:-freq-controller-seg}\"",
        ...
        "",
        f"  echo \"[{EXPERIMENT_LOG_TAG}] split={split} qps=${{qps_value}} slug=${{qps_slug}} gpu_id={gpu_id} lower_bound=${{FREQ_CONTROLLER_LOWER_BOUND_VALUE}} upper_bound=${{FREQ_CONTROLLER_UPPER_BOUND_VALUE}} output=${{replay_output_ref}} port_profile=${{PORT_PROFILE_ID_VALUE}}\"",
        ...
        f"    echo \"[{EXPERIMENT_LOG_TAG}] freq-controller-seg failed to stay alive after startup\" >&2",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        description=(
            ...
            "freq-controller-seg by looking up an existing compiled clean split "
            "plan and emitting one sweep entrypoint script."
        ),
    )
    ...
    parser.add_argument(
        ...
        help="GPU index used by both freq-controller-seg and zeus-power-reader.",
    )
    parser.add_argument(
        ...
        help=(
            "Default freq-controller-seg lower context bound. "
    ...
    parser.add_argument(
        ...
        help=(
            "Default freq-controller-seg upper context bound. "
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "results/replay/sweep-qps-docker-power-clean-freq-ctrl-seg/. "
