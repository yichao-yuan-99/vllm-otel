...
def load_generator_module() -> object:
    ...
    spec = importlib.util.spec_from_file_location(
        "generate_sweep_qps_docker_power_clean_freq_ctrl_seg_experiment",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise RuntimeError(f"Unable to load module from {module_path}")
    module = importlib.util.module_from_spec(spec)
    sys.modules["generate_sweep_qps_docker_power_clean_freq_ctrl_seg_experiment"] = module
...
def test_main_generates_sweep_qps_docker_power_clean_freq_ctrl_seg_bundle(
    tmp_path: Path,
) -> None:
    ...
    assert manifest["qps_points"][0]["freq_controller_log_dir"].endswith(
        "results/replay/sweep-qps-docker-power-clean-freq-ctrl-seg/"
        f"dataset-a/agent-a/split/rest/qps0_25/{batch_timestamp}/freq-control-seg"
    )
    ...
    assert "[sweep-qps-docker-power-clean-freq-ctrl-seg]" in run_script_text
    assert "GPU_ID=2" in run_script_text
    assert "FREQ_CONTROLLER_BIN=\"${FREQ_CONTROLLER_BIN:-freq-controller-seg}\"" in run_script_text
    assert "local freq_controller_log_dir=\"${replay_output_dir}/freq-control-seg\"" in run_script_text
...
def test_main_supports_additional_suffix_and_custom_bounds(tmp_path: Path) -> None:
    ...
    assert manifest["qps_points"][0]["freq_controller_log_dir"].endswith(
        "results/replay/sweep-qps-docker-power-clean-freq-ctrl-seg/"
        f"dataset-a/agent-a/split/exclude-unranked/qps0_5/{batch_timestamp}/freq-control-seg"
    )
