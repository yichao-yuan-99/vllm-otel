# Sweep QPS Docker Power Clean Gateway Multi Lowest Profile LEQ Reloc 2 Freq Ctrl Linesapce Instance Ctx Aware

This workflow is the `gateway_multi` + `lowest_profile_leq_reloc_2` + independent
`freq-controller-linespace-instance` + ctx-aware variant of
`experiments/sweep-qps-docker-power-clean-gateway_multi-freq-ctrl-linesapce-instance-ctx-aware`.
...
- sends `balanced_usage_threshold_tokens` to `POST /policy` so the run does
  not depend on the gateway default
- relies on the gateway's built-in relocation rule, so after assignment the
  policy may reassign active agents to the other backend with the lowest
  non-zero ongoing context usage during later requests when that usage is at
  most half of the current backend's
...
- `lowest_profile_leq_reloc_2 balanced_usage_threshold_tokens = 263856`
...
```bash
python3 experiments/sweep-qps-docker-power-clean-gateway_multi-lowest_profile_leq_reloc_2-freq-ctrl-linesapce-instance-ctx-aware/generate_experiment.py \
...
```bash
bash experiments/sweep-qps-docker-power-clean-gateway_multi-lowest_profile_leq_reloc_2-freq-ctrl-linesapce-instance-ctx-aware/generated/<timestamp>/run_replay.sh
```

Override the gateway_multi port-profile selection at runtime:

```bash
bash experiments/sweep-qps-docker-power-clean-gateway_multi-lowest_profile_leq_reloc_2-freq-ctrl-linesapce-instance-ctx-aware/generated/<timestamp>/run_replay.sh 0,1,2,3
```
...
- `results/replay/sweep-qps-docker-power-clean-gateway_multi-lowest_profile_leq_reloc_2-freq-ctrl-linesapce-instance-ctx-aware/<dataset>/<agent>/split/<split>/<qps>/<timestamp>/`

With `--output-suffix <suffix>`, the top-level replay directory name becomes:

- `results/replay/sweep-qps-docker-power-clean-gateway_multi-lowest_profile_leq_reloc_2-freq-ctrl-linesapce-instance-ctx-aware-<suffix>/<dataset>/<agent>/split/<split>/<qps>/<timestamp>/`
...
```bash
.venv/bin/pytest experiments/sweep-qps-docker-power-clean-gateway_multi-lowest_profile_leq_reloc_2-freq-ctrl-linesapce-instance-ctx-aware/test -q
```
