...
def load_generator_module() -> object:
    ...
    spec = importlib.util.spec_from_file_location(
        (
            "generate_sweep_qps_docker_power_clean_"
            "gateway_multi_lowest_profile_leq_reloc_2_freq_ctrl_linesapce_instance_ctx_aware_experiment"
        ),
        module_path,
    )
...
def test_main_generates_sweep_qps_docker_power_clean_gateway_multi_lowest_profile_leq_reloc_2_freq_ctrl_linesapce_instance_ctx_aware_bundle(
    tmp_path: Path,
) -> None:
    ...
    assert replay_qps025["output_dir"].endswith(
        "results/replay/sweep-qps-docker-power-clean-gateway_multi-lowest_profile_leq_reloc_2-freq-ctrl-linesapce-instance-ctx-aware/"
    ...
    assert manifest["assignment_policy"] == "lowest_profile_leq_reloc_2"
    assert (
        manifest["balanced_usage_threshold_tokens"]
        == module.DEFAULT_BALANCED_USAGE_THRESHOLD_TOKENS
    )
    ...
    assert manifest["freq_controller_backend_pairings"] == [
        {"port_profile_id": 3, "gpu_index": 2},
        {"port_profile_id": 4, "gpu_index": 3},
    ]
    ...
    assert manifest["qps_points"][0]["freq_controller_log_dir"].endswith(
        "results/replay/sweep-qps-docker-power-clean-gateway_multi-lowest_profile_leq_reloc_2-freq-ctrl-linesapce-instance-ctx-aware/"
        f"dataset-a/agent-a/split/rest/qps0_25/{batch_timestamp}/freq-control-linespace-instance"
    )
    assert (
        manifest["qps_points"][0]["freq_controller_backend_log_dirs"][0]["freq_controller_log_dir"]
    ).endswith(
        "results/replay/sweep-qps-docker-power-clean-gateway_multi-lowest_profile_leq_reloc_2-freq-ctrl-linesapce-instance-ctx-aware/"
        f"dataset-a/agent-a/split/rest/qps0_25/{batch_timestamp}/freq-control-linespace-instance/profile-3"
    )
    assert (
        manifest["qps_points"][0]["freq_controller_backend_log_dirs"][1]["freq_controller_log_dir"]
    ).endswith(
        "results/replay/sweep-qps-docker-power-clean-gateway_multi-lowest_profile_leq_reloc_2-freq-ctrl-linesapce-instance-ctx-aware/"
        f"dataset-a/agent-a/split/rest/qps0_25/{batch_timestamp}/freq-control-linespace-instance/profile-4"
    )
    ...
    assert (
        "[sweep-qps-docker-power-clean-gateway_multi-lowest_profile_leq_reloc_2-freq-ctrl-linesapce-instance-ctx-aware]"
        in run_script_text
    )
    ...
    assert "TARGET_ASSIGNMENT_POLICY=lowest_profile_leq_reloc_2" in run_script_text
    assert (
        f"DEFAULT_BALANCED_USAGE_THRESHOLD_TOKENS={module.DEFAULT_BALANCED_USAGE_THRESHOLD_TOKENS}"
        in run_script_text
    )
    assert (
        'BALANCED_USAGE_THRESHOLD_TOKENS_VALUE="${BALANCED_USAGE_THRESHOLD_TOKENS:-${DEFAULT_BALANCED_USAGE_THRESHOLD_TOKENS}}"'
        in run_script_text
    )
    assert (
        'FREQ_CONTROLLER_BIN="${FREQ_CONTROLLER_BIN:-freq-controller-linespace-instance}"'
        in run_script_text
    )
    ...
    assert f"DEFAULT_FREQ_CONTROLLER_THRESHOLD={module.DEFAULT_FREQ_CONTROLLER_THRESHOLD}" in run_script_text
    assert "POWER_GPU_INDICES=(2 3)" in run_script_text
    assert "validate_backend_pairing" in run_script_text
    assert "port profile count must match power GPU count" in run_script_text
    ...
    assert "stop_freq_controllers" in run_script_text
    ...
    assert "backend_pairings=${BACKEND_PAIRINGS_DISPLAY}" in run_script_text
    assert (
        '  freq_controller_log_root_dir="${replay_output_dir}/freq-control-linespace-instance"'
        in run_script_text
    )
    assert 'freq_controller_log_dir="${freq_controller_log_root_dir}/profile-${profile_id}"' in run_script_text
    assert '"--threshold" "${FREQ_CONTROLLER_THRESHOLD_VALUE}"' in run_script_text
    assert '"--port-profile-id" "${profile_id}"' in run_script_text
    assert '"--gpu-index" "${gpu_index}"' in run_script_text
...
def test_main_supports_repeated_port_profile_flags_custom_threshold_output_suffix_and_additional_suffix(
    tmp_path: Path,
) -> None:
    ...
    assert replay_config["replay"]["output_dir"].endswith(
        "results/replay/sweep-qps-docker-power-clean-gateway_multi-lowest_profile_leq_reloc_2-freq-ctrl-linesapce-instance-ctx-aware-lmcache/"
    ...
    assert manifest["assignment_policy"] == "lowest_profile_leq_reloc_2"
    assert manifest["balanced_usage_threshold_tokens"] == 400000
    ...
    assert manifest["freq_controller_backend_pairings"] == [
        {"port_profile_id": 5, "gpu_index": 0},
        {"port_profile_id": 6, "gpu_index": 1},
    ]
    assert (
        manifest["qps_points"][0]["freq_controller_backend_log_dirs"][0]["freq_controller_log_dir"]
    ).endswith(
        "results/replay/sweep-qps-docker-power-clean-gateway_multi-lowest_profile_leq_reloc_2-freq-ctrl-linesapce-instance-ctx-aware-lmcache/"
        f"dataset-a/agent-a/split/exclude-unranked/qps0_5/{batch_timestamp}/freq-control-linespace-instance/profile-5"
    )
    ...
    assert "TARGET_ASSIGNMENT_POLICY=lowest_profile_leq_reloc_2" in run_script_text
    assert "DEFAULT_BALANCED_USAGE_THRESHOLD_TOKENS=400000" in run_script_text
...
def test_main_rejects_mismatched_port_profile_and_gpu_counts(tmp_path: Path) -> None:
    module, source_run_dir = prepare_module_and_source_run(tmp_path)
    output_config_dir = tmp_path / "generated"
