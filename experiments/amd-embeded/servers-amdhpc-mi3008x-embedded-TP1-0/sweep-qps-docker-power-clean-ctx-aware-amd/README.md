# Embedded AMD HPC MI3008X Embedded TP1 Profile 0 Sweep QPS Power Clean Ctx-Aware AMD
...
```bash
python3 experiments/amd-embeded/servers-amdhpc-mi3008x-embedded-TP1-0/sweep-qps-docker-power-clean-ctx-aware-amd/generate_experiment.py \
...
```bash
bash experiments/amd-embeded/servers-amdhpc-mi3008x-embedded-TP1-0/sweep-qps-docker-power-clean-ctx-aware-amd/generated/<timestamp>/run_replay.sh
```

Explicitly pass the fixed `mi3008x` port profile:

```bash
bash experiments/amd-embeded/servers-amdhpc-mi3008x-embedded-TP1-0/sweep-qps-docker-power-clean-ctx-aware-amd/generated/<timestamp>/run_replay.sh 0
```
...
```bash
bash experiments/amd-embeded/servers-amdhpc-mi3008x-embedded-TP1-0/sweep-qps-docker-power-clean-ctx-aware-amd/generated/<timestamp>/submit_embedded_tp1.sh
```
...
python3 servers/servers-amdhpc-mi3008x-embedded-TP1-0/launch.py submit \
  ...
  -e ./experiments/amd-embeded/servers-amdhpc-mi3008x-embedded-TP1-0/sweep-qps-docker-power-clean-ctx-aware-amd/generated/<timestamp>/run_replay.sh
```

Default replay output layout:

- `results/replay/amd-embeded/servers-amdhpc-mi3008x-embedded-TP1-0/sweep-qps-docker-power-clean-ctx-aware-amd/<dataset>/<agent>/split/<split>/<qps>/<timestamp>/profile-0/`
...
```bash
pytest experiments/amd-embeded/servers-amdhpc-mi3008x-embedded-TP1-0/sweep-qps-docker-power-clean-ctx-aware-amd/test -q
```
