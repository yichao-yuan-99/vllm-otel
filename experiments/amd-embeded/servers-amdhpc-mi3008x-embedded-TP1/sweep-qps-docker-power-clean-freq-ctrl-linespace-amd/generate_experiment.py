#!/usr/bin/env python3
"""Generate one dedicated mi3008x embedded TP1 AMD freq-control sweep bundle."""
...
_BASE_GENERATOR_MODULE_NAME = (
    "generate_sweep_qps_docker_power_clean_experiment_for_mi3008x_embedded_tp1_freq_ctrl_linespace_amd"
)
...
def validate_mi3008x_port_profile(value: int, *, field_name: str) -> int:
    parsed = int(value)
    if parsed < MI3008X_MIN_PORT_PROFILE_ID or parsed > MI3008X_MAX_PORT_PROFILE_ID:
        raise ValueError(
            f"{field_name} must be between "
            f"{MI3008X_MIN_PORT_PROFILE_ID} and {MI3008X_MAX_PORT_PROFILE_ID}"
        )
    return parsed
...
def ensure_supported_qps_point_count(qps_values: list[float]) -> None:
    if len(qps_values) > MAX_QPS_POINTS:
        raise ValueError(
            f"--qps-list supports at most {MAX_QPS_POINTS} QPS values for mi3008x embedded TP1"
        )
