#!/usr/bin/env python3
"""Generate one dedicated mi2508x embedded TP2 AMD freq-control sweep bundle."""
...
DEFAULT_OUTPUT_CONFIG_DIR = (
    ...
    / "experiments"
    / "amd-embeded"
    / "servers-amdhpc-mi2508x-embedded-TP2"
    / MI2508X_SUBDIR
    / EXPERIMENT_DIR_NAME
    / "generated"
)
...
EXPERIMENT_PATH = (
    "experiments/amd-embeded/servers-amdhpc-mi2508x-embedded-TP2/"
    f"{MI2508X_SUBDIR}/"
...
_BASE_GENERATOR_MODULE_NAME = (
    "generate_sweep_qps_docker_power_clean_experiment_for_mi2508x_embedded_tp2_freq_ctrl_linespace_amd"
)
...
def parse_optional_gpu_index(raw: str | None) -> str | None:
    ...
    if normalized not in GPU_PAIR_BY_PORT_PROFILE.values():
        raise ValueError(
            "--gpu-index must be one of 0,1 2,3 4,5 6,7 for mi2508x embedded TP2"
        )
    return normalized
...
def ensure_supported_qps_point_count(qps_values: list[float]) -> None:
    if len(qps_values) > MAX_QPS_POINTS:
        raise ValueError(
            f"--qps-list supports at most {MAX_QPS_POINTS} QPS values for mi2508x embedded TP2"
        )
...
def write_run_script(
    ...
    *,
    ...
    lines = [
        ...
        "  fi",
        (
            f"  if (( normalized_value < {MI2508X_MIN_PORT_PROFILE_ID} "
            f"|| normalized_value > {MI2508X_MAX_PORT_PROFILE_ID} )); then"
        ),
        (
            f"    echo \"[{EXPERIMENT_LOG_TAG}] error: supported mi2508x port "
            f"profile ids are {MI2508X_MIN_PORT_PROFILE_ID}..{MI2508X_MAX_PORT_PROFILE_ID}\" >&2"
        ),
        "    exit 1",
        "  fi",
        ...
        "    0,1|2,3|4,5|6,7) ;;",
        (
            f"    *) echo \"[{EXPERIMENT_LOG_TAG}] error: supported mi2508x gpu "
            "pairs are 0,1 2,3 4,5 6,7\" >&2; exit 1 ;;"
        ),
        "  esac",
        "  if [[ \"${normalized_value}\" != \"${expected_value}\" ]]; then",
        (
            f"    echo \"[{EXPERIMENT_LOG_TAG}] error: this mi2508x workflow "
            "expects gpu pair ${expected_value} for port profile "
...
def build_embedded_tp2_submit_command(*, run_script_path: Path, target_model: str) -> str:
    return shlex.join(
        [
            "python3",
...
def write_submit_script(
    ...
    *,
    ...
    lines = [
        ...
        "PYTHON_BIN=\"${PYTHON_BIN:-python3}\"",
        (
            "EMBEDDED_TP2_LAUNCH_SCRIPT="
            "\"${EMBEDDED_TP2_LAUNCH_SCRIPT:-servers/servers-amdhpc-mi2508x-embedded-TP2/launch.py}\""
        ),
        f"TARGET_MODEL={_shell_quote(target_model)}",
        "RUN_SCRIPT_PATH=\"${SCRIPT_DIR}/run_replay.sh\"",
        "",
        "cd \"${REPO_ROOT}\"",
        "exec \"${PYTHON_BIN}\" \"${EMBEDDED_TP2_LAUNCH_SCRIPT}\" submit \\",
        "  -m \"${TARGET_MODEL}\" \\",
        "  -e \"${RUN_SCRIPT_PATH}\"",
        "",
        (
            "# Equivalent raw command: "
            f"{build_embedded_tp2_submit_command(run_script_path=run_script_path, target_model=target_model)}"
        ),
        "",
    ]
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        description=(
            "Generate a dedicated mi2508x embedded TP2 AMD sweep-QPS replay bundle "
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "under results/replay/amd-embeded/servers-amdhpc-mi2508x-embedded-TP2/"
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        submit_script_path = (batch_dir / "submit_embedded_tp2.sh").resolve()
        write_submit_script(
            submit_script_path,
            run_script_path=run_script_path,
            target_model=target_model,
        )
        embedded_tp2_submit_command = build_embedded_tp2_submit_command(
            run_script_path=run_script_path,
            target_model=target_model,
        )
        ...
        summary = {
            ...
            "embedded_tp2_submit_command": embedded_tp2_submit_command,
