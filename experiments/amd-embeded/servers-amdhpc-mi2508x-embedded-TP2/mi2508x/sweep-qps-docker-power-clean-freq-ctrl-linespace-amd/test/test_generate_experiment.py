...
def load_generator_module() -> object:
    ...
    spec = importlib.util.spec_from_file_location(
        "generate_amd_mi2508x_embedded_tp2_sweep_qps_freq_ctrl_linespace_amd_experiment",
        module_path,
    )
...
def test_main_generates_mi2508x_embedded_tp2_freq_ctrl_bundle(tmp_path: Path) -> None:
    ...
    assert replay_qps025["output_dir"].endswith(
        "results/replay/amd-embeded/servers-amdhpc-mi2508x-embedded-TP2/"
    ...
    assert "service_mode" not in manifest
    ...
    assert manifest["submit_command_default"].endswith("submit_embedded_tp2.sh")
    assert "servers/servers-amdhpc-mi2508x-embedded-TP2/launch.py submit" in manifest[
        "embedded_tp2_submit_command"
    ]

    run_script_path = batch_dir / "run_replay.sh"
    run_script_text = run_script_path.read_text(encoding="utf-8")
    assert (
        "[amd-embeded-servers-amdhpc-mi2508x-embedded-TP2-mi2508x-"
    ...
    submit_script_path = batch_dir / "submit_embedded_tp2.sh"
    submit_script_text = submit_script_path.read_text(encoding="utf-8")
    assert 'PYTHON_BIN="${PYTHON_BIN:-python3}"' in submit_script_text
    assert (
        'EMBEDDED_TP2_LAUNCH_SCRIPT="${EMBEDDED_TP2_LAUNCH_SCRIPT:-'
        'servers/servers-amdhpc-mi2508x-embedded-TP2/launch.py}"'
    ) in submit_script_text
    assert 'RUN_SCRIPT_PATH="${SCRIPT_DIR}/run_replay.sh"' in submit_script_text
    assert 'exec "${PYTHON_BIN}" "${EMBEDDED_TP2_LAUNCH_SCRIPT}" submit \\' in submit_script_text
