...
def load_generator_module() -> object:
    ...
    spec = importlib.util.spec_from_file_location(
        "generate_amd_interactive_mi3001x_sweep_qps_power_clean_amd_experiment",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise RuntimeError(f"Unable to load module from {module_path}")
    module = importlib.util.module_from_spec(spec)
    sys.modules["generate_amd_interactive_mi3001x_sweep_qps_power_clean_amd_experiment"] = (
        module
    )
...
def test_main_generates_mi3001x_interactive_amd_power_clean_bundle(tmp_path: Path) -> None:
    ...
    assert replay_qps025["output_dir"].endswith(
        "results/replay/amd-embeded/servers-amdhpc-interactive-mi3001x-embedded-TP1/"
    ...
    assert (
        ...
        == "servers/servers-amdhpc-interactive-mi3001x-embedded-TP1/README.md"
    )
    assert (
    ...
    assert "interactive_start_services_script" not in manifest
    assert "interactive_power_daemon_required" not in manifest
    assert "interactive_power_daemon_command_example" not in manifest
    assert manifest["interactive_power_socket_env_example"].endswith(
    ...
    assert "interactive_embedded_tp1_normalize_port_profile_id" in run_script_text
    assert "interactive_embedded_tp1_normalize_gpu_index" in run_script_text
    assert "interactive_embedded_tp1_wait_for_services" in run_script_text
    assert 'export AMD_SMI_POWER_SOCKET_PATH="${AMD_SMI_POWER_SOCKET_PATH_VALUE}"' in run_script_text
    assert "resolve_profile_ports() {" not in run_script_text
    assert "wait_for_interactive_services() {" not in run_script_text
    assert 'replay_output_dir="${replay_output_base_dir}/profile-${PORT_PROFILE_ID_VALUE}"' in run_script_text
    assert 'local power_output_dir="${replay_output_dir}/power"' in run_script_text
    assert (
        '"${AMD_POWER_READER_BIN}" --output-dir "${power_output_dir}" '
        '--gpu-indices "${GPU_INDEX_VALUE}" --socket-path '
        '"${AMD_SMI_POWER_SOCKET_PATH_VALUE}" &'
    ) in run_script_text
    ...
    assert not submit_script_path.exists()


def test_main_supports_additional_suffix(tmp_path: Path) -> None:
