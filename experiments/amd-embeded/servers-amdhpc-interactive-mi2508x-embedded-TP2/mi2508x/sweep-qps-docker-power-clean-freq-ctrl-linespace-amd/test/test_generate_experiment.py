...
def load_generator_module() -> object:
    ...
    spec = importlib.util.spec_from_file_location(
        "generate_amd_interactive_mi2508x_tp2_sweep_qps_freq_ctrl_linespace_amd_experiment",
        module_path,
    )
...
def test_main_generates_interactive_mi2508x_tp2_freq_ctrl_bundle(tmp_path: Path) -> None:
    ...
    assert replay_qps025["output_dir"].endswith(
        "results/replay/amd-embeded/servers-amdhpc-interactive-mi2508x-embedded-TP2/"
    ...
    assert replay_config_qps03["replay"]["port_profile_id"] == 1
    ...
    assert manifest["max_qps_points"] == 4
    ...
    assert manifest["gpu_index_runtime_default"] == "match-port-profile-gpu-pair"
    ...
    assert manifest["service_mode"] == "interactive"
    assert manifest["assumes_external_service_stack"] is True
    assert manifest["service_preflight"] is True
    assert (
        manifest["interactive_client_script"]
        == "servers/servers-amdhpc-interactive-mi2508x-embedded-TP2/client.py"
    )
    assert (
        manifest["interactive_env_helper_script"]
        == "servers/servers-amdhpc-interactive-mi2508x-embedded-TP2/experiment-env.sh"
    )
    assert (
        manifest["interactive_launcher_script"]
        == "servers/servers-amdhpc-interactive-mi2508x-embedded-TP2/start-services.sh"
    )
    assert (
        manifest["interactive_start_services_command"]
        == "python3 servers/servers-amdhpc-interactive-mi2508x-embedded-TP2/client.py start"
    )
    assert (
        manifest["interactive_stop_services_command"]
        == "python3 servers/servers-amdhpc-interactive-mi2508x-embedded-TP2/client.py stop"
    )
    assert (
        manifest["interactive_foreground_start_services_command"]
        == "bash servers/servers-amdhpc-interactive-mi2508x-embedded-TP2/start-services.sh"
    )
    assert manifest["interactive_power_daemon_enabled"] is True
    assert manifest["interactive_power_daemon_managed_by_launcher"] is True
    assert "submit_script" not in manifest
    assert "submit_command_default" not in manifest
    assert "embedded_tp1_submit_command" not in manifest
    ...
    assert manifest["qps_points"][0]["assigned_gpu_index"] == "0,1"
    assert manifest["qps_points"][0]["replay_output_dir"].endswith(
        "results/replay/amd-embeded/servers-amdhpc-interactive-mi2508x-embedded-TP2/"
        "mi2508x/sweep-qps-docker-power-clean-freq-ctrl-linespace-amd/"
        f"dataset-a/agent-a/split/rest/qps0_25/{batch_timestamp}/profile-0"
    )
    assert manifest["qps_points"][0]["power_output_dir"].endswith(
        "results/replay/amd-embeded/servers-amdhpc-interactive-mi2508x-embedded-TP2/"
        "mi2508x/sweep-qps-docker-power-clean-freq-ctrl-linespace-amd/"
        f"dataset-a/agent-a/split/rest/qps0_25/{batch_timestamp}/profile-0/power"
    )
    assert manifest["qps_points"][0]["freq_controller_log_dir"].endswith(
        "results/replay/amd-embeded/servers-amdhpc-interactive-mi2508x-embedded-TP2/"
        "mi2508x/sweep-qps-docker-power-clean-freq-ctrl-linespace-amd/"
        f"dataset-a/agent-a/split/rest/qps0_25/{batch_timestamp}/profile-0/freq-control-linespace-amd"
    )
    assert manifest["qps_points"][1]["assigned_port_profile"] == 1
    assert manifest["qps_points"][1]["assigned_gpu_index"] == "2,3"
    ...
    assert (
        'INTERACTIVE_ENV_HELPER="${INTERACTIVE_ENV_HELPER:-'
        'servers/servers-amdhpc-interactive-mi2508x-embedded-TP2/experiment-env.sh}"'
    ) in run_script_text
    assert 'source "${INTERACTIVE_ENV_HELPER_PATH}"' in run_script_text
    assert "interactive_embedded_tp2_normalize_port_profile_id" in run_script_text
    assert "interactive_embedded_tp2_normalize_gpu_index" in run_script_text
    assert "interactive_embedded_tp2_wait_for_services" in run_script_text
    assert "interactive_embedded_tp2_resolve_gateway_base_url" in run_script_text
    ...
    assert 'IFS=\',\' read -r -a GPU_INDEX_LIST <<<"${GPU_INDEX_VALUE}"' in run_script_text
    assert 'IFS=\',\' read -r -a GPU_INDEX_ARGS <<<"${GPU_INDEX_VALUE}"' in run_script_text
    ...
    assert (
        ...
        '"${GPU_INDEX_ARGS[@]}" --socket-path "${AMD_SMI_POWER_SOCKET_PATH_VALUE}" &'
    ) in run_script_text
    assert '"${RESET_GPU_CORE_FREQ_BIN}" --gpu-index "${gpu_index}"' in run_script_text
    assert 'SELECTED_REPLAY_CONFIG_REF=qps0_25/replay.toml' in run_script_text
    assert 'SELECTED_REPLAY_CONFIG_REF=qps0_3/replay.toml' in run_script_text
    assert run_script_path.stat().st_mode & stat.S_IXUSR


def test_main_supports_output_suffix_and_nonzero_default_port_profile(
    tmp_path: Path,
) -> None:
    ...
    assert replay_config["replay"]["output_dir"].endswith(
        "results/replay/amd-embeded/servers-amdhpc-interactive-mi2508x-embedded-TP2/"
        "mi2508x/sweep-qps-docker-power-clean-freq-ctrl-linespace-amd-lmcache/"
...
def test_main_rejects_more_than_four_qps_points(tmp_path: Path) -> None:
    module, source_run_dir = prepare_module_and_source_run(tmp_path)
    output_config_dir = tmp_path / "generated"
...
def test_main_rejects_mismatched_gpu_pair(tmp_path: Path) -> None:
    module, source_run_dir = prepare_module_and_source_run(tmp_path)
    output_config_dir = tmp_path / "generated"
