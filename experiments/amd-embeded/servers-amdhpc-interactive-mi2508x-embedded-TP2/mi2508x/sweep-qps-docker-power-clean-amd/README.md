# Interactive AMD HPC Embedded TP2 MI2508X Sweep QPS Power Clean AMD
...
embedded AMD workflows, but assumes you have already started the interactive
service stack on an `mi2508x` allocation with
...
`servers/servers-amdhpc-interactive-mi2508x-embedded-TP2/experiment-env.sh`
and waits for the selected profile before it starts replaying. New generated
...
```bash
python3 experiments/amd-embeded/servers-amdhpc-interactive-mi2508x-embedded-TP2/mi2508x/sweep-qps-docker-power-clean-amd/generate_experiment.py \
...
```bash
bash experiments/amd-embeded/servers-amdhpc-interactive-mi2508x-embedded-TP2/mi2508x/sweep-qps-docker-power-clean-amd/generated/<timestamp>/run_replay.sh
```

Explicitly select a port profile:

```bash
bash experiments/amd-embeded/servers-amdhpc-interactive-mi2508x-embedded-TP2/mi2508x/sweep-qps-docker-power-clean-amd/generated/<timestamp>/run_replay.sh 2
```
...
- `results/replay/amd-embeded/servers-amdhpc-interactive-mi2508x-embedded-TP2/mi2508x/sweep-qps-docker-power-clean-amd/<dataset>/<agent>/split/<split>/<qps>/<timestamp>/profile-<port_profile_id>/`
...
```bash
pytest experiments/amd-embeded/servers-amdhpc-interactive-mi2508x-embedded-TP2/mi2508x/sweep-qps-docker-power-clean-amd/test -q
```
