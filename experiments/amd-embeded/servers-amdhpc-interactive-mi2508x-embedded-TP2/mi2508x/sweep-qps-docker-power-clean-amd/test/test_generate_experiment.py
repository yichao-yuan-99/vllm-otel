...
def load_generator_module() -> object:
    ...
    spec = importlib.util.spec_from_file_location(
        "generate_amd_interactive_mi2508x_tp2_sweep_qps_power_clean_amd_experiment",
        module_path,
    )
...
def test_main_generates_interactive_mi2508x_tp2_power_bundle(tmp_path: Path) -> None:
    ...
    assert (
        manifest["interactive_setup_readme"]
        == "servers/servers-amdhpc-interactive-mi2508x-embedded-TP2/README.md"
    )
    assert (
    ...
    assert manifest["qps_points"][1]["power_output_dir"].endswith(
        "results/replay/amd-embeded/servers-amdhpc-interactive-mi2508x-embedded-TP2/"
        "mi2508x/sweep-qps-docker-power-clean-amd/"
        f"dataset-a/agent-a/split/rest/qps0_3/{batch_timestamp}/profile-1/power"
    )
    ...
    assert 'GPU_INDEX_VALUE="${SELECTED_ASSIGNED_GPU_INDEX}"' in run_script_text
    assert 'SELECTED_ASSIGNED_GPU_INDEX=0,1' in run_script_text
    assert 'SELECTED_ASSIGNED_GPU_INDEX=2,3' in run_script_text
...
def test_main_supports_nonzero_default_port_profile_and_matching_gpu_pair(
    tmp_path: Path,
) -> None:
    ...
    assert manifest["selected_plan_copy"] == str(expected_plan_copy)
    assert manifest["run_command_with_port_profile"].endswith("run_replay.sh 2")
