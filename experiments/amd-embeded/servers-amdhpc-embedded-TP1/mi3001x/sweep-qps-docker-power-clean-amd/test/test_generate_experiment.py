...
def load_generator_module() -> object:
    ...
    spec = importlib.util.spec_from_file_location(
        "generate_amd_embedded_tp1_sweep_qps_power_clean_amd_experiment",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise RuntimeError(f"Unable to load module from {module_path}")
    module = importlib.util.module_from_spec(spec)
    sys.modules["generate_amd_embedded_tp1_sweep_qps_power_clean_amd_experiment"] = (
        module
    )
...
def test_main_generates_mi3001x_amd_power_clean_bundle(tmp_path: Path) -> None:
    ...
    assert replay_qps025["output_dir"].endswith(
        "results/replay/amd-embeded/servers-amdhpc-embedded-TP1/"
    ...
    assert "servers/servers-amdhpc-embedded-TP1/launch.py submit" in manifest[
    ...
    assert "[amd-embeded-servers-amdhpc-embedded-TP1-mi3001x-sweep-qps-docker-power-clean-amd]" in run_script_text
    ...
    assert "  -p mi3001x \\" in submit_script_text
