...
def load_generator_module() -> object:
    ...
    spec = importlib.util.spec_from_file_location(
        "generate_amd_mi2104x_embedded_tp1_sweep_qps_power_clean_amd_experiment",
        module_path,
    )
...
def test_main_generates_mi2104x_amd_power_clean_bundle(tmp_path: Path) -> None:
    ...
    assert (
        "[amd-embeded-servers-amdhpc-mi2104x-embedded-TP1-sweep-qps-docker-power-clean-amd]"
        in run_script_text
    )
...
def test_main_supports_nonzero_default_port_profile_and_additional_suffix(
    tmp_path: Path,
) -> None:
