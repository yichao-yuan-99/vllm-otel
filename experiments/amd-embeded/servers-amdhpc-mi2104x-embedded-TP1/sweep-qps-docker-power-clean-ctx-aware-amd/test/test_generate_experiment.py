...
def load_generator_module() -> object:
    ...
    spec = importlib.util.spec_from_file_location(
        "generate_amd_mi2104x_embedded_tp1_sweep_qps_power_clean_ctx_aware_amd_experiment",
        module_path,
    )
...
def test_main_generates_mi2104x_amd_power_clean_ctx_aware_bundle(tmp_path: Path) -> None:
    ...
    assert replay_qps025["output_dir"].endswith(
        "results/replay/amd-embeded/servers-amdhpc-mi2104x-embedded-TP1/"
    ...
    assert "servers/servers-amdhpc-mi2104x-embedded-TP1/launch.py submit" in manifest[
    ...
    assert (
        "[amd-embeded-servers-amdhpc-mi2104x-embedded-TP1-sweep-qps-docker-power-clean-ctx-aware-amd]"
        in run_script_text
    )
    ...
    assert "0)" in run_script_text
    assert "1)" in run_script_text
...
def test_main_supports_nonzero_default_port_profile_additional_suffix_and_custom_ctx_aware_thresholds(
    tmp_path: Path,
) -> None:
    ...
    assert 'SELECTED_REPLAY_CONFIG_REF=qps0_5/replay.toml' in run_script_text
