#!/usr/bin/env python3
"""Generate one local max-concurrency sweep clean replay experiment bundle.

This helper does all of the following:
1) Looks up existing clean replay plans for the requested split mode.
2) Validates selected plans match the requested target model.
3) Materializes per-concurrency replay TOMLs + runnable sweep script.
"""
...
DEFAULT_OUTPUT_CONFIG_DIR = (
    REPO_ROOT / "experiments" / "sweep-maxcon-clean" / "generated"
)
DEFAULT_REPLAY_OUTPUT_ROOT = Path("results") / "replay" / "sweep-maxcon-clean"
...
def write_replay_config(path: Path, *, replay_payload: dict[str, Any]) -> None:
    lines: list[str] = [
        "# Auto-generated by experiments/sweep-maxcon-clean/generate_experiment.py",
        "",
    ]
...
def write_run_script(
    ...
    *,
    ...
    lines = [
        ...
        "  fi",
        "",
        "  echo \"[sweep-maxcon-clean] max_concurrent=${max_concurrent_value} slug=${max_concurrent_slug} output=${replay_output_ref} port_profile=${PORT_PROFILE_ID_VALUE}\"",
        ...
        "",
        (
            f"echo \"[sweep-maxcon-clean] target_model={target_model} split={split} "
    ...
    lines.extend(
        [
            "",
            f"echo \"[sweep-maxcon-clean] completed {len(concurrency_jobs)} concurrency points\"",
            "",
        ]
    )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        description=(
            "Generate a clean max-concurrency sweep replay bundle by looking up an "
            "existing compiled clean split plan and emitting one sweep entrypoint script."
        ),
    )
    parser.add_argument(
        "--source-run-dir",
    ...
    parser.add_argument(
        ...
        default="token_usage",
        help=(
            "Split grouping metric used for clean plan lookup "
            "(default: token_usage)."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "<dataset-lineage>/split/<split>/c<max-concurrent>/<timestamp> under "
            "results/replay/sweep-maxcon-clean/. dataset-lineage is inferred "
            "from --source-run-dir by dropping the first (<model>) and last "
            "(<run-dir>) path segments."
        ),
    )
    return parser
