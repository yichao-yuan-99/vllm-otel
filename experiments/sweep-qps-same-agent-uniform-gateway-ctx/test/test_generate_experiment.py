...
def load_generator_module() -> object:
    ...
    spec = importlib.util.spec_from_file_location(
        "generate_sweep_qps_same_agent_uniform_gateway_ctx_experiment",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise RuntimeError(f"Unable to load module from {module_path}")
    module = importlib.util.module_from_spec(spec)
    sys.modules["generate_sweep_qps_same_agent_uniform_gateway_ctx_experiment"] = module
...
def test_main_generates_sweep_qps_same_agent_uniform_gateway_ctx_bundle(
    tmp_path: Path,
) -> None:
    ...
    assert replay_qps025["output_dir"].endswith(
        "results/replay/sweep-qps-same-agent-uniform-gateway-ctx/"
    ...
    assert "[sweep-qps-same-agent-uniform-gateway-ctx]" in run_script_text
