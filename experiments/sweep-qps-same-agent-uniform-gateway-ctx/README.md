# Sweep QPS Same Agent Uniform Gateway Ctx
...
- `results/replay/sweep-qps-same-agent-uniform-gateway-ctx/<dataset-lineage>/trail/<safe-source-trail>/<qps>/<timestamp>/`
...
```bash
python3 experiments/sweep-qps-same-agent-uniform-gateway-ctx/generate_experiment.py \
...
```bash
bash experiments/sweep-qps-same-agent-uniform-gateway-ctx/generated/<timestamp>/run_replay.sh
```

Override port profile at runtime:

```bash
bash experiments/sweep-qps-same-agent-uniform-gateway-ctx/generated/<timestamp>/run_replay.sh 3
```
