#!/usr/bin/env python3
"""Generate one local sweep-QPS + power logging + fixed-freq replay bundle.
...
3) Materializes per-QPS replay TOMLs + runnable sweep script with power logging.
4) Locks GPU core clocks to one fixed range for the entire sweep.
"""
...
DEFAULT_OUTPUT_CONFIG_DIR = (
    REPO_ROOT / "experiments" / "sweep-qps-docker-power-clean-fixed-freq" / "generated"
)
DEFAULT_REPLAY_OUTPUT_ROOT = (
    Path("results") / "replay" / "sweep-qps-docker-power-clean-fixed-freq"
)
MODEL_CONFIG_PATH = REPO_ROOT / "configs" / "model_config.toml"
EXPERIMENT_DIR_NAME = "sweep-qps-docker-power-clean-fixed-freq"
EXPERIMENT_LOG_TAG = EXPERIMENT_DIR_NAME
FIXED_CORE_MIN_MHZ = 345

SPLIT_ALIASES = {
    # "full" is kept as a compatibility alias; in clean mode it maps to the
    # clean split-union plan (exclude-unranked).
...
def format_frequency_slug(max_freq_mhz: int, *, min_freq_mhz: int = FIXED_CORE_MIN_MHZ) -> str:
    return f"core-{min_freq_mhz}-{max_freq_mhz}"
...
def write_replay_config(path: Path, *, replay_payload: dict[str, Any]) -> None:
    lines: list[str] = [
        "# Auto-generated by experiments/sweep-qps-docker-power-clean-fixed-freq/generate_experiment.py",
        "",
    ]
...
def write_run_script(
    ...
    *,
    ...
    lines = [
        ...
        f"FIXED_CORE_MIN_MHZ={FIXED_CORE_MIN_MHZ}",
        f"FIXED_CORE_MAX_MHZ={fixed_freq_mhz}",
        ...
        (
            ...
            "freq=${FIXED_CORE_MIN_MHZ}:${FIXED_CORE_MAX_MHZ} "
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        description=(
            "Generate a sweep-QPS replay bundle with power logging and a fixed GPU "
            "core clock by looking up an existing compiled clean split plan and "
            "emitting one sweep entrypoint script."
        ),
    )
    ...
    parser.add_argument(
        ...
        default="0",
        help=(
            "Comma-separated GPU indices passed to zeus-power-reader and locked "
            "via set-gpu-core-freq/reset-gpu-core-freq, for example: 0 or 0,1 "
            "(default: 0)."
        ),
    )
    parser.add_argument(
        ...
        type=int,
        help=(
            "Fixed maximum core clock in MHz. The generated runner locks each "
            f"selected GPU to {FIXED_CORE_MIN_MHZ}:<freq> for the entire sweep."
        ),
    )
    parser.add_argument(
        "--output-config-dir",
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        fixed_freq_mhz = int(args.freq)
        if fixed_freq_mhz < FIXED_CORE_MIN_MHZ:
            raise ValueError(
                f"--freq must be >= {FIXED_CORE_MIN_MHZ} so max clock is not below the fixed minimum"
            )
        ...
        freq_slug = format_frequency_slug(fixed_freq_mhz)
        ...
        summary = {
            ...
            "power_gpu_indices": power_gpu_indices,
            "fixed_core_min_mhz": FIXED_CORE_MIN_MHZ,
            "fixed_core_max_mhz": fixed_freq_mhz,
            "freq_mhz": fixed_freq_mhz,
            "freq_slug": freq_slug,
            "additional_suffix": additional_suffix,
