...
def load_generator_module() -> object:
    ...
    spec = importlib.util.spec_from_file_location(
        "generate_single_maxcon_experiment",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise RuntimeError(f"Unable to load module from {module_path}")
    module = importlib.util.module_from_spec(spec)
    sys.modules["generate_single_maxcon_experiment"] = module
...
def test_main_generates_single_maxcon_bundle(tmp_path: Path) -> None:
    module = load_generator_module()
    repo_root = tmp_path / "repo"
    results_root = repo_root / "results"
    source_run_dir = results_root / "suite" / "dataset" / "agent" / "run-1"
    ...
    selected_plan_path = source_run_dir / "replay-plan.exclude-unranked.json"
    ...
    expected_plan_copy = batch_dir / "plan" / selected_plan_path.name
    assert expected_plan_copy.read_text(encoding="utf-8") == selected_plan_path.read_text(
        encoding="utf-8"
    )
