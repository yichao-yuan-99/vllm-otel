#!/usr/bin/env python3
"""Generate one local single-max-concurrency replay experiment bundle.
...
DEFAULT_OUTPUT_CONFIG_DIR = (
    REPO_ROOT / "experiments" / "single-maxcon" / "local" / "generated"
)
DEFAULT_REPLAY_OUTPUT_ROOT = Path("results") / "replay" / "single-maxcon" / "split"
...
def write_replay_config(path: Path, *, replay_payload: dict[str, Any]) -> None:
    lines: list[str] = [
        "# Auto-generated by experiments/single-maxcon/local/generate_experiment.py",
        "",
    ]
...
def write_run_script(
    ...
    *,
    ...
    content = (
        ...
        f"echo \"[single-maxcon] target_model={target_model} split={split} "
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        description=(
            "Generate a local single-max-concurrency replay bundle by looking up "
            "an existing compiled plan and emitting a one-command replay "
            "entrypoint script."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "<split>/<max-concurrent>/<timestamp> under results/replay/single-maxcon/split/."
        ),
    )
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        replay_output_dir = (
            replay_output_root / split / max_concurrent_slug / batch_timestamp
        ).resolve()
        replay_payload = {
