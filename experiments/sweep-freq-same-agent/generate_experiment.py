#!/usr/bin/env python3
"""Generate one derived single-trail frequency-sweep replay experiment bundle.

This helper does all of the following:
1) Validates the selected replay plan exists, is derived, and came from --single-trail.
2) Validates the plan's replay target matches the requested target model.
3) Materializes per-frequency replay TOMLs + a runnable sweep script with power logging.
4) Supports either fixed-QPS patterned replay or fixed max-concurrency replay.
"""
...
EXPERIMENT_DIR_NAME = "sweep-freq-same-agent"
...
class LaunchSpec:
    ...
    display_value: str
    target_slug: str
    replay_launch_policy_override: dict[str, Any]
    launch_seed: int | None
    qps: float | None
...
def build_launch_spec(args: argparse.Namespace) -> LaunchSpec:
    raw_qps = getattr(args, "qps", None)
    raw_max_concurrent = getattr(args, "max_concurrent", None)
    raw_launch_seed = (
        getattr(args, LAUNCH_SEED_DEST, None)
        if LAUNCH_SEED_DEST is not None
        else None
    )

    if raw_qps is not None:
        qps = parse_positive_float(str(raw_qps), field_name="--qps")
        if LAUNCH_SEED_DEST is not None and LAUNCH_SEED_OPTION is not None:
            if raw_launch_seed is None:
                raise ValueError(f"{LAUNCH_SEED_OPTION} is required with --qps")
            launch_seed = parse_non_negative_int(str(raw_launch_seed))
        else:
            launch_seed = None
        ...
        if launch_seed is not None:
            replay_launch_policy_override["seed"] = launch_seed
        return LaunchSpec(
            mode="qps",
            label="qps",
            display_value=format(qps, ".12g"),
            target_slug=format_qps_slug(qps),
            replay_launch_policy_override=replay_launch_policy_override,
            launch_seed=launch_seed,
            qps=qps,
            max_concurrent=None,
        )

    if raw_max_concurrent is not None:
        if (
            LAUNCH_SEED_DEST is not None
            and LAUNCH_SEED_OPTION is not None
            and raw_launch_seed is not None
        ):
            raise ValueError(f"{LAUNCH_SEED_OPTION} cannot be used with --max-concurrent")
        max_concurrent = parse_positive_int(
            str(raw_max_concurrent),
            field_name="--max-concurrent",
        )
        return LaunchSpec(
            mode="max_concurrent",
            label="max_concurrent",
            display_value=str(max_concurrent),
            target_slug=format_max_concurrent_slug(max_concurrent),
            replay_launch_policy_override={
    ...
    raise ValueError("either --qps or --max-concurrent is required")
...
def write_run_script(
    ...
    *,
    ...
    lines = [
        ...
        f"LAUNCH_MODE_NAME={_shell_quote(launch_spec.mode)}",
        f"LAUNCH_LABEL_NAME={_shell_quote(launch_spec.label)}",
        f"LAUNCH_VALUE={_shell_quote(launch_spec.display_value)}",
        f"TARGET_MODEL_NAME={_shell_quote(target_model)}",
        ...
        "",
        f"  echo \"[{EXPERIMENT_LOG_TAG}] trail=${{SOURCE_TRAIL_NAME}} ${{LAUNCH_LABEL_NAME}}=${{LAUNCH_VALUE}} freq=${{min_mhz}}:${{max_mhz}} slug=${{freq_slug}} output=${{replay_output_ref}} port_profile=${{PORT_PROFILE_ID_VALUE}}\"",
        ...
        (
            "echo "
            f"\"[{EXPERIMENT_LOG_TAG}] target_model=${{TARGET_MODEL_NAME}} "
            "trail=${SOURCE_TRAIL_NAME} "
            "launch_mode=${LAUNCH_MODE_NAME} "
            "${LAUNCH_LABEL_NAME}=${LAUNCH_VALUE} "
            f"gpu_id={gpu_id} "
            f"freq_points={len(frequency_jobs)} "
    ...
    lines.extend(
        [
            "",
            f"echo \"[{EXPERIMENT_LOG_TAG}] completed {len(frequency_jobs)} frequency points\"",
            "",
        ]
    )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        description=(
            "Generate a frequency-sweep replay bundle from one precompiled derived "
            "single-trail replay plan and emit one sweep entrypoint script."
        ),
    )
    parser.add_argument(
        "--source-plan",
    ...
    launch_group.add_argument("--qps", type=float, help=f"{LAUNCH_PATTERN_LABEL} launch rate.")
    launch_group.add_argument(
        "--max-concurrent",
        type=int,
        help="Replay max concurrency override.",
    )
    if LAUNCH_SEED_OPTION is not None and LAUNCH_SEED_DEST is not None:
        parser.add_argument(
            ...
            help=(
                f"{LAUNCH_PATTERN_LABEL} launch seed. Required with --qps and rejected "
                "with --max-concurrent."
            ),
        )
    parser.add_argument(
    ...
    parser.add_argument(
        "--freq-list",
        required=True,
        help="Semicolon-separated core clock ranges in '<min>:<max>' format.",
    )
    parser.add_argument(
        "--gpu-id",
        required=True,
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "<dataset-lineage>/trail/<safe-source-trail>/<launch-target>/<timestamp>/<freq-slug> "
            f"under results/replay/{EXPERIMENT_DIR_NAME}/, where <launch-target> is "
            "qps... or c.... If dataset-lineage cannot be derived from the plan, the "
            "path starts at trail/<safe-source-trail>/..."
        ),
    )
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        launch_spec = build_launch_spec(args)
        ...
        replay_output_base_dir = replay_output_root
        if source_dataset_lineage is not None:
            replay_output_base_dir = replay_output_base_dir / source_dataset_lineage
        replay_output_base_dir = (
            replay_output_base_dir
            / "trail"
            / source_trail_slug
            / launch_spec.target_slug
            / batch_timestamp
        ).resolve()
        ...
        for frequency_range in frequency_ranges:
            ...
            replay_payload = {
                ...
                "launch_policy_override": launch_spec.replay_launch_policy_override,
            }
            replay_config_path = (
        ...
        summary = {
            ...
            "launch_label": launch_spec.label,
            "launch_value": launch_spec.display_value,
            "launch_target_slug": launch_spec.target_slug,
            "randomize_seed": int(args.randomize_seed),
            "qps": launch_spec.qps,
            "qps_slug": (
                launch_spec.target_slug if launch_spec.mode == "qps" else None
            ),
            "max_concurrent": launch_spec.max_concurrent,
            "max_concurrent_slug": (
                launch_spec.target_slug
                if launch_spec.mode == "max_concurrent"
                else None
            ),
            "time_constraint_s": time_constraint_s,
        ...
        if LAUNCH_SEED_MANIFEST_KEY is not None:
            summary[LAUNCH_SEED_MANIFEST_KEY] = launch_spec.launch_seed
