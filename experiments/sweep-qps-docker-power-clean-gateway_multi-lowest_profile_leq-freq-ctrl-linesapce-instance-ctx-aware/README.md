# Sweep QPS Docker Power Clean Gateway Multi Lowest Profile LEQ Freq Ctrl Linesapce Instance Ctx Aware

This workflow is the `gateway_multi` + `lowest_profile_leq` + independent
...
- sets the public `gateway_multi` assignment policy to `lowest_profile_leq`
...
- `lowest_profile_leq balanced_usage_threshold_tokens = 263856`
...
```bash
python3 experiments/sweep-qps-docker-power-clean-gateway_multi-lowest_profile_leq-freq-ctrl-linesapce-instance-ctx-aware/generate_experiment.py \
...
```bash
bash experiments/sweep-qps-docker-power-clean-gateway_multi-lowest_profile_leq-freq-ctrl-linesapce-instance-ctx-aware/generated/<timestamp>/run_replay.sh
```

Override the gateway_multi port-profile selection at runtime:

```bash
bash experiments/sweep-qps-docker-power-clean-gateway_multi-lowest_profile_leq-freq-ctrl-linesapce-instance-ctx-aware/generated/<timestamp>/run_replay.sh 0,1,2,3
```
...
- `results/replay/sweep-qps-docker-power-clean-gateway_multi-lowest_profile_leq-freq-ctrl-linesapce-instance-ctx-aware/<dataset>/<agent>/split/<split>/<qps>/<timestamp>/`

With `--output-suffix <suffix>`, the top-level replay directory name becomes:

- `results/replay/sweep-qps-docker-power-clean-gateway_multi-lowest_profile_leq-freq-ctrl-linesapce-instance-ctx-aware-<suffix>/<dataset>/<agent>/split/<split>/<qps>/<timestamp>/`
...
```bash
.venv/bin/pytest experiments/sweep-qps-docker-power-clean-gateway_multi-lowest_profile_leq-freq-ctrl-linesapce-instance-ctx-aware/test -q
```
