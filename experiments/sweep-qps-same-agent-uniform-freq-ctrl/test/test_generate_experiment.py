...
def load_generator_module() -> object:
    ...
    spec = importlib.util.spec_from_file_location(
        "generate_sweep_qps_same_agent_uniform_freq_ctrl_experiment",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise RuntimeError(f"Unable to load module from {module_path}")
    module = importlib.util.module_from_spec(spec)
    sys.modules["generate_sweep_qps_same_agent_uniform_freq_ctrl_experiment"] = module
...
def test_main_generates_sweep_qps_same_agent_uniform_freq_ctrl_bundle(
    tmp_path: Path,
) -> None:
    ...
    assert replay_qps025["output_dir"].endswith(
        "results/replay/sweep-qps-same-agent-uniform-freq-ctrl/"
    ...
    assert "[sweep-qps-same-agent-uniform-freq-ctrl]" in run_script_text
    assert "SOURCE_TRAIL_NAME=profile-2/run_beta" in run_script_text
    assert "GPU_ID=2" in run_script_text
    assert "local freq_controller_log_dir=\"${replay_output_dir}/freq-control\"" in run_script_text
...
def test_main_applies_output_suffix_and_custom_bounds(tmp_path: Path) -> None:
    module = load_generator_module()
    repo_root = tmp_path / "repo"
    module.REPO_ROOT = repo_root
    module.MODEL_CONFIG_PATH = repo_root / "configs" / "model_config.toml"
    ...
    assert manifest["qps_points"][0]["freq_controller_log_dir"].endswith(
        "results/replay/sweep-qps-same-agent-uniform-freq-ctrl/"
        f"dataset-a/agent-a/trail-lmcache/profile-2_run_beta/qps0_5/{batch_timestamp}/freq-control"
    )
