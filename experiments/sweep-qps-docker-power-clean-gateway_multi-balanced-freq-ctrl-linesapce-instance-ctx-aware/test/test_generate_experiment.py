...
def load_generator_module() -> object:
    ...
    spec = importlib.util.spec_from_file_location(
        (
            ...
            "gateway_multi_balanced_freq_ctrl_linesapce_instance_ctx_aware_experiment"
        ),
        module_path,
    )
...
def test_main_generates_sweep_qps_docker_power_clean_gateway_multi_balanced_freq_ctrl_linesapce_instance_ctx_aware_bundle(
    tmp_path: Path,
) -> None:
    ...
    assert replay_qps025["output_dir"].endswith(
        "results/replay/sweep-qps-docker-power-clean-gateway_multi-balanced-freq-ctrl-linesapce-instance-ctx-aware/"
    ...
    assert manifest["assignment_policy"] == "balanced"
    assert (
    ...
    assert "TARGET_ASSIGNMENT_POLICY=balanced" in run_script_text
    assert (
...
def test_main_supports_repeated_port_profile_flags_custom_threshold_output_suffix_and_additional_suffix(
    ...
    assert replay_config["replay"]["output_dir"].endswith(
        "results/replay/sweep-qps-docker-power-clean-gateway_multi-balanced-freq-ctrl-linesapce-instance-ctx-aware-lmcache/"
