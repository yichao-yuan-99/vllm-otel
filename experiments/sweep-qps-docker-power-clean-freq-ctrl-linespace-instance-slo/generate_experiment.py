#!/usr/bin/env python3
"""Generate one local sweep-QPS + power logging + linespace-instance-slo bundle."""
...
DEFAULT_REPLAY_OUTPUT_ROOT_NAME = (
    "sweep-qps-docker-power-clean-freq-ctrl-linespace-instance-slo"
)
...
DEFAULT_FREQ_CONTROL_LOG_DIR_NAME = "freq-control-linespace-instance-slo"
DEFAULT_FREQ_CONTROLLER_BIN = "freq-controller-linespace-instance-slo"
...
def format_target_slo_path_component(target_slo: float) -> str:
    text = f"{target_slo:.12f}".rstrip("0").rstrip(".")
    return text or "0"
...
def write_replay_config(path: Path, *, replay_payload: dict[str, Any]) -> None:
    lines: list[str] = [
        (
            "# Auto-generated by "
            "experiments/sweep-qps-docker-power-clean-freq-ctrl-linespace-instance-slo/"
...
def write_run_script(
    ...
    *,
    ...
    lines = [
        ...
        f"DEFAULT_TARGET_SLO={target_slo}",
        ...
        "}",
        "",
        "format_target_slo_path_component() {",
        "  local target_slo_value=\"$1\"",
        "  \"${PYTHON_BIN}\" - \"${target_slo_value}\" <<'PY'",
        "import sys",
        ...
        "  fi",
        "  if [[ -n \"${FREQ_CONTROLLER_TARGET_SLO:-}\" ]]; then",
        "    local replay_timestamp=\"\"",
        "    local replay_qps_dir=\"\"",
        "    local target_slo_path_component=\"\"",
        "    replay_timestamp=\"$(basename \"${replay_output_dir}\")\"",
        "    replay_qps_dir=\"$(dirname \"$(dirname \"${replay_output_dir}\")\")\"",
        "    target_slo_path_component=\"$(format_target_slo_path_component \"${FREQ_CONTROLLER_TARGET_SLO_VALUE}\")\"",
        "    replay_output_dir=\"${replay_qps_dir}/${target_slo_path_component}/${replay_timestamp}\"",
        "  fi",
        ...
        (
            ...
            "slug=${qps_slug} gpu_id=${GPU_ID} "
            "threshold=${FREQ_CONTROLLER_THRESHOLD_VALUE} "
            "target_slo=${FREQ_CONTROLLER_TARGET_SLO_VALUE} "
        ...
        "    \"--threshold\" \"${FREQ_CONTROLLER_THRESHOLD_VALUE}\"",
        "    \"--throughput-target\" \"${FREQ_CONTROLLER_TARGET_SLO_VALUE}\"",
        ...
        (
            ...
            "freq-controller-linespace-instance-slo failed to stay alive after startup\" >&2"
        ),
        ...
        (
            ...
            f"qps_points={len(qps_jobs)} gpu_id={gpu_id} "
            "threshold=${FREQ_CONTROLLER_THRESHOLD_VALUE} "
            "target_slo=${FREQ_CONTROLLER_TARGET_SLO_VALUE} "
            "port_profile=${PORT_PROFILE_ID_VALUE}\""
        ),
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        description=(
            "Generate a sweep-QPS replay bundle with power logging and "
            "freq-controller-linespace-instance-slo by looking up an existing compiled clean "
            "split plan and emitting one sweep entrypoint script."
        ),
    )
    ...
    parser.add_argument(
        ...
        type=float,
        help=(
            "Bundle-wide output-throughput SLO target in token/s passed to "
    ...
    parser.add_argument(
        ...
        help="GPU index used by both freq-controller-linespace-instance-slo and zeus-power-reader.",
    )
    parser.add_argument(
        ...
        help=(
            "Default freq-controller-linespace-instance-slo context threshold. "
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "results/replay/sweep-qps-docker-power-clean-freq-ctrl-linespace-instance-slo-<threshold>/. "
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        target_slo_value = parse_positive_float(
            str(args.target_slo),
            field_name="--target-slo",
        )
        target_slo_path_component = format_target_slo_path_component(target_slo_value)
        ...
        summary = {
            ...
            "freq_controller_threshold": threshold_value,
            "target_slo": target_slo_value,
            "target_slo_path_component": target_slo_path_component,
