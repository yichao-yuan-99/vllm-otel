#!/usr/bin/env python3
"""Generate grouped exclude-unranked sweep-qps bundles using sbatch-orchestrator.

For each target QPS, this script creates:
...
DEFAULT_PLAN_FILE_NAME = "replay-plan.exclude-unranked.json"
DEFAULT_OUTPUT_CONFIG_DIR = (
    REPO_ROOT / "experiments" / "sweep-qps-local" / "group" / "exclude-unranked" / "generated"
)
...
def write_replay_config(path: Path, *, replay_payload: dict[str, Any]) -> None:
    lines: list[str] = [
        "# Auto-generated by experiments/sweep-qps-local/group/exclude-unranked/generate_replay_configs.py",
        "",
    ]
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        description=(
            "Generate grouped exclude-unranked sweep-qps bundles for sbatch-orchestrator "
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "<utc-timestamp>/source-lineage/sweep-qps-local/group/exclude-unranked/<qps> "
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.plan_path:
            ...
            plan_path = (source_run_dir / DEFAULT_PLAN_FILE_NAME).resolve()
        ...
        summary = {
            ...
            "plan_path": str(plan_path),
            "plan_variant": "exclude-unranked",
            "exclude_unranked_plan": True,
