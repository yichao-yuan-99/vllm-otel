#!/usr/bin/env python3
"""Generate local-mode split-group sweep-qps experiment bundles.

For each split group (top/rest) and target QPS, this script creates one
experiment subdirectory containing:
...
DEFAULT_OUTPUT_CONFIG_DIR = REPO_ROOT / "experiments" / "sweep-qps-local" / "split" / "generated"
...
def write_replay_config(path: Path, *, replay_payload: dict[str, Any]) -> None:
    lines: list[str] = [
        "# Auto-generated by experiments/sweep-qps-local/split/generate_replay_configs.py",
        "",
    ]
...
def write_submit_all_script(path: Path, *, sbatch_relpaths: list[str]) -> None:
    lines = [
        "#!/usr/bin/env bash",
        "set -euo pipefail",
        "",
    ...
    for sbatch_relpath in sbatch_relpaths:
        lines.append(f"sbatch \"${{SCRIPT_DIR}}/{sbatch_relpath}\"")
    lines.append("")
    content = "\n".join(lines)
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(content, encoding="utf-8")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "<utc-timestamp>/source-lineage/sweep-qps-local/split/<group>/<qps> "
    ...
    try:
        ...
        for split_group_name in SPLIT_GROUPS:
            ...
            for qps in qps_values:
                ...
                submit_relpath = f"{split_group_name}/{qps_slug}/sbatch.sh"
                experiment_record = {
