...
def test_main_generates_only_selected_split_group(tmp_path: Path) -> None:
    module = load_generator_module()
    repo_root = tmp_path / "repo"
    results_root = repo_root / "results"
    source_run_dir = results_root / "model-a" / "dataset-b" / "agent-c" / "run-1"
    ...
    batch_dir = resolve_output_batch_dir(output_config_dir)
    top_qps_01_dir = batch_dir / "top" / "qps0_1"
    assert top_qps_01_dir.exists()
    assert (top_qps_01_dir / "replay.toml").exists()
    assert (top_qps_01_dir / "run_local_replay.sh").exists()
    assert (top_qps_01_dir / "gateway-config.toml").exists()
    assert (top_qps_01_dir / "sbatch.sh").exists()
    assert not (batch_dir / "rest").exists()

    submit_all_content = (batch_dir / "submit_all.sh").read_text(encoding="utf-8")
    assert 'sbatch "${SCRIPT_DIR}/top/qps0_1/sbatch.sh"' in submit_all_content
    assert 'sbatch "${SCRIPT_DIR}/rest/' not in submit_all_content
    ...
    expected_generated_copy_dir = (expected_replay_timestamp_dir / "generated").resolve()
    assert (expected_generated_copy_dir / "top" / "qps0_1" / "replay.toml").exists()
    assert not (expected_generated_copy_dir / "rest").exists()

    manifest = json.loads((batch_dir / "manifest.json").read_text(encoding="utf-8"))
    assert manifest["split_groups"] == ["top"]
    assert list(manifest["generated_experiments_by_group"]) == ["top"]
    assert list(manifest["plan_paths"]) == ["top"]
    assert manifest["plan_paths"]["top"] == str(split_top_plan_path)
    assert str(split_rest_plan_path) not in json.dumps(manifest)
    assert len(manifest["generated_experiments"]) == 1
    assert manifest["generated_experiments"][0]["group"] == "top"
    assert manifest["generated_experiments"][0]["plan_path"] == str(split_top_plan_path)
...
def test_derive_default_split_plan_paths_prefers_unsuffixed_pair(tmp_path: Path) -> None:
    module = load_generator_module()
    source_run_dir = tmp_path / "run-1"
    source_run_dir.mkdir(parents=True)
    ...
    suffixed_top_path = (source_run_dir / "replay-plan.token.top.v2.json").resolve()
    suffixed_rest_path = (source_run_dir / "replay-plan.token.rest.v2.json").resolve()
    ...
    assert resolved["top"] == unsuffixed_top_path
    assert resolved["rest"] == unsuffixed_rest_path
