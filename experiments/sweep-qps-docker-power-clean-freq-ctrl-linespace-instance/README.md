...
This workflow is the `freq-controller-linespace-instance` variant of
...
wraps each replay with both Zeus power logging and `freq-controller-linespace-instance`.

Compared to `sweep-qps-docker-power-clean`:

- each QPS run also starts and stops `freq-controller-linespace-instance`
- linespace-instance controller logs are written to `<replay_output_dir>/freq-control-linespace-instance/`
- the generated runner resets GPU core clocks after each QPS point
- `--gpu-id` is used for both `freq-controller-linespace-instance` and
  `zeus-power-reader`
...
Because `freq-controller-linespace-instance` waits for gateway `job_active=true`, it
...
- default threshold (`395784`):
  `results/replay/sweep-qps-docker-power-clean-freq-ctrl-linespace-instance/<dataset>/<agent>/split/<split>/<qps>/<timestamp>/`
- non-default threshold:
  `results/replay/sweep-qps-docker-power-clean-freq-ctrl-linespace-instance-<threshold>/<dataset>/<agent>/split/<split>/<qps>/<timestamp>/`
...
```bash
python3 experiments/sweep-qps-docker-power-clean-freq-ctrl-linespace-instance/generate_experiment.py \
...
```bash
bash experiments/sweep-qps-docker-power-clean-freq-ctrl-linespace-instance/generated/<timestamp>/run_replay.sh
```

Override port profile at runtime:

```bash
bash experiments/sweep-qps-docker-power-clean-freq-ctrl-linespace-instance/generated/<timestamp>/run_replay.sh 3
```
...
the linespace-instance controller logs through the
`freq-control-linespace-instance` flow.

```bash
python3 post-process/run_all.py \
  --root-dir results/replay/sweep-qps-docker-power-clean-freq-ctrl-linespace-instance
```
...
```bash
python3 post-process/run_all.py \
  --root-dir results/replay/sweep-qps-docker-power-clean-freq-ctrl-linespace-instance-200
```

Each run produces:

- `post-processed/freq-control-linespace-instance/freq-control-summary.json`
- `post-processed/visualization/freq-control-linespace-instance/freq-control-timeline.png`
...
```bash
.venv/bin/pytest experiments/sweep-qps-docker-power-clean-freq-ctrl-linespace-instance/test -q
```
