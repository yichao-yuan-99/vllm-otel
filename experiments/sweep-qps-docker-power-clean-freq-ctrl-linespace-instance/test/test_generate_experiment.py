...
def load_generator_module() -> object:
    ...
    spec = importlib.util.spec_from_file_location(
        "generate_sweep_qps_docker_power_clean_freq_ctrl_linespace_instance_experiment",
        module_path,
    )
...
def test_main_generates_sweep_qps_docker_power_clean_freq_ctrl_linespace_instance_bundle(
    tmp_path: Path,
) -> None:
    ...
    assert "[sweep-qps-docker-power-clean-freq-ctrl-linespace-instance]" in run_script_text
...
def test_main_supports_additional_suffix_and_custom_threshold(tmp_path: Path) -> None:
    ...
    assert replay_config["replay"]["output_dir"].endswith(
        "results/replay/sweep-qps-docker-power-clean-freq-ctrl-linespace-instance-200/"
