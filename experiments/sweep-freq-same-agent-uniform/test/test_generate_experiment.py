...
def load_generator_module() -> object:
    ...
    spec = importlib.util.spec_from_file_location(
        "generate_sweep_freq_same_agent_uniform_experiment",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise RuntimeError(f"Unable to load module from {module_path}")
    module = importlib.util.module_from_spec(spec)
    sys.modules["generate_sweep_freq_same_agent_uniform_experiment"] = module
...
def test_main_generates_sweep_freq_same_agent_uniform_qps_bundle(tmp_path: Path) -> None:
    ...
    assert replay_payload["launch_policy_override"]["pattern"]["name"] == "uniform"
    ...
    assert replay_payload["output_dir"].endswith(
        "results/replay/sweep-freq-same-agent-uniform/"
    ...
    assert "poisson_seed" not in manifest
    ...
    assert "[sweep-freq-same-agent-uniform]" in run_script_text
...
def test_main_generates_sweep_freq_same_agent_uniform_maxcon_bundle(tmp_path: Path) -> None:
    module = load_generator_module()
    repo_root = tmp_path / "repo"
    module.REPO_ROOT = repo_root
    module.MODEL_CONFIG_PATH = repo_root / "configs" / "model_config.toml"
