#!/usr/bin/env python3
"""Generate a fixed-interval uniform same-agent frequency sweep bundle."""
...
_BASE_GENERATOR_PATH = (
    _THIS_FILE.parents[1] / "sweep-freq-same-agent" / "generate_experiment.py"
).resolve()


def _load_base_module() -> Any:
    spec = importlib.util.spec_from_file_location(
        "generate_sweep_freq_same_agent_base",
        _BASE_GENERATOR_PATH,
    )
    if spec is None or spec.loader is None:
        raise RuntimeError(f"Unable to load base generator from {_BASE_GENERATOR_PATH}")
    module = importlib.util.module_from_spec(spec)
    sys.modules["generate_sweep_freq_same_agent_base"] = module
...
_BASE_MODULE.configure_experiment_variant(
    experiment_dir_name="sweep-freq-same-agent-uniform",
