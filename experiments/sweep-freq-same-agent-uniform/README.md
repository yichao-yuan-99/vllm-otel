# Sweep Freq Same Agent Uniform

This workflow is the fixed-interval sibling of
`experiments/sweep-freq-same-agent/`.
...
In QPS mode, this bundle uses a `uniform` replay launch pattern, which in this
repository means a fixed inter-launch interval of exactly `1 / qps`. In
max-concurrency mode, the replay override is the same as the Poisson sibling.
...
- `results/replay/sweep-freq-same-agent-uniform/<dataset-lineage>/trail/<safe-source-trail>/<launch-target>/<timestamp>/<freq-slug>/`
...
```bash
python3 experiments/sweep-freq-same-agent-uniform/generate_experiment.py \
...
```bash
bash experiments/sweep-freq-same-agent-uniform/generated/<timestamp>/run_replay.sh
```

Override port profile at runtime:

```bash
bash experiments/sweep-freq-same-agent-uniform/generated/<timestamp>/run_replay.sh 3
```
