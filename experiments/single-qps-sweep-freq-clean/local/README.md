# Single QPS Sweep Frequency Clean (Local)
...
```bash
python3 experiments/single-qps-sweep-freq-clean/local/generate_experiment.py \
...
```bash
bash experiments/single-qps-sweep-freq-clean/local/generated/<timestamp>/run_replay.sh
```

Override port profile at runtime:

```bash
bash experiments/single-qps-sweep-freq-clean/local/generated/<timestamp>/run_replay.sh 3
```
