...
def write_replay_config(path: Path, *, replay_payload: dict[str, Any]) -> None:
    lines: list[str] = [
        "# Auto-generated by experiments/single-qps-sweep-freq-clean/local/generate_experiment.py",
        "",
    ]
...
def write_run_script(
    ...
    *,
    ...
    lines = [
        ...
        "      echo \"[single-qps-sweep-freq-clean] warning: failed to reset GPU ${GPU_ID} core clocks\" >&2",
        "    fi",
        "    GPU_FREQ_LOCK_ACTIVE=0",
        "  fi",
        "}",
        ...
        "trap 'echo \"[single-qps-sweep-freq-clean] interrupted\" >&2; exit 130' INT TERM",
        "",
        "run_one_frequency() {",
        "  local min_mhz=\"$1\"",
        "  local max_mhz=\"$2\"",
    ...
    lines.extend(
        [
            "",
            f"echo \"[single-qps-sweep-freq-clean] completed {len(frequency_jobs)} frequency points\"",
            "",
        ]
    )
