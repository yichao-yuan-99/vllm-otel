...
def load_generator_module() -> object:
    ...
    spec = importlib.util.spec_from_file_location(
        "generate_sweep_qps_docker_power_clean_gateway_multi_lowest_usage_freq_ctrl_linespace_experiment",
        module_path,
    )
...
def test_main_generates_sweep_qps_docker_power_clean_gateway_multi_lowest_usage_freq_ctrl_linespace_bundle(
    tmp_path: Path,
) -> None:
    ...
    assert replay_qps025["output_dir"].endswith(
        "results/replay/sweep-qps-docker-power-clean-gateway_multi-lowest_usage_freq_ctrl_linespace/"
    ...
    assert manifest["qps_points"][0]["freq_controller_log_dir"].endswith(
        "results/replay/sweep-qps-docker-power-clean-gateway_multi-lowest_usage_freq_ctrl_linespace/"
        f"dataset-a/agent-a/split/rest/qps0_25/{batch_timestamp}/freq-control-linespace"
    )
    assert (
        manifest["qps_points"][0]["freq_controller_backend_log_dirs"][0]["freq_controller_log_dir"]
    ).endswith(
        "results/replay/sweep-qps-docker-power-clean-gateway_multi-lowest_usage_freq_ctrl_linespace/"
        f"dataset-a/agent-a/split/rest/qps0_25/{batch_timestamp}/freq-control-linespace/profile-3"
    )
    assert (
        manifest["qps_points"][0]["freq_controller_backend_log_dirs"][1]["freq_controller_log_dir"]
    ).endswith(
        "results/replay/sweep-qps-docker-power-clean-gateway_multi-lowest_usage_freq_ctrl_linespace/"
        f"dataset-a/agent-a/split/rest/qps0_25/{batch_timestamp}/freq-control-linespace/profile-4"
    )
    ...
    assert "[sweep-qps-docker-power-clean-gateway_multi-lowest_usage_freq_ctrl_linespace]" in run_script_text
    ...
    assert 'FREQ_CONTROLLER_BIN="${FREQ_CONTROLLER_BIN:-freq-controller-linespace}"' in run_script_text
...
def test_main_supports_repeated_port_profile_flags_custom_threshold_output_suffix_and_additional_suffix(
    ...
    assert replay_config["replay"]["output_dir"].endswith(
        "results/replay/sweep-qps-docker-power-clean-gateway_multi-lowest_usage_freq_ctrl_linespace-lmcache/"
    ...
    assert (
        ...
        "results/replay/sweep-qps-docker-power-clean-gateway_multi-lowest_usage_freq_ctrl_linespace-lmcache/"
        f"dataset-a/agent-a/split/exclude-unranked/qps0_5/{batch_timestamp}/freq-control-linespace/profile-5"
    )
