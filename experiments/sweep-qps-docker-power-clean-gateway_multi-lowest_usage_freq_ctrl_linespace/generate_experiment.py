#!/usr/bin/env python3
"""Generate one local sweep-QPS + power logging + gateway_multi replay bundle."""
...
DEFAULT_OUTPUT_CONFIG_DIR = (
    REPO_ROOT
    / "experiments"
    / "sweep-qps-docker-power-clean-gateway_multi-lowest_usage_freq_ctrl_linespace"
    / "generated"
)
...
EXPERIMENT_DIR_NAME = "sweep-qps-docker-power-clean-gateway_multi-lowest_usage_freq_ctrl_linespace"
    ...
    *,
    ...
    if len(port_profile_ids) != len(gpu_indices):
        raise ValueError(
            ...
            "--port-profile when using independent freq-controller-linespace"
        )
    return [
    ...
    *,
    ...
    lines = [
        ...
        (
            ...
            "power GPU count for independent freq-controller-linespace\" >&2"
        ),
        "    exit 1",
        "  fi",
        ...
        (
            f"    echo \"[{EXPERIMENT_LOG_TAG}] starting freq-controller-linespace "
        ...
        (
            ...
            "freq-controller-linespace failed to stay alive after startup "
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        description=(
            ...
            "split plan, enabling ctx-aware mode, and starting one "
            "freq-controller-linespace instance per backend GPU."
        ),
    )
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "for independent freq-controller-linespace, for example: 2,3."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "results/replay/"
            "sweep-qps-docker-power-clean-gateway_multi-lowest_usage_freq_ctrl_linespace-lmcache/."
        ),
    )
    return parser
