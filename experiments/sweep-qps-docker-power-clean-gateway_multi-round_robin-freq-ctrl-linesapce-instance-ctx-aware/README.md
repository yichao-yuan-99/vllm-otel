# Sweep QPS Docker Power Clean Gateway Multi Round Robin Freq Ctrl Linesapce Instance Ctx Aware

This workflow is the `gateway_multi` + explicit `round_robin` + independent
...
- sends an explicit `POST /policy` update so the run does not depend on the
  gateway default
...
```bash
python3 experiments/sweep-qps-docker-power-clean-gateway_multi-round_robin-freq-ctrl-linesapce-instance-ctx-aware/generate_experiment.py \
...
```bash
bash experiments/sweep-qps-docker-power-clean-gateway_multi-round_robin-freq-ctrl-linesapce-instance-ctx-aware/generated/<timestamp>/run_replay.sh
```

Override the gateway_multi port-profile selection at runtime:

```bash
bash experiments/sweep-qps-docker-power-clean-gateway_multi-round_robin-freq-ctrl-linesapce-instance-ctx-aware/generated/<timestamp>/run_replay.sh 0,1,2,3
```
...
- `results/replay/sweep-qps-docker-power-clean-gateway_multi-round_robin-freq-ctrl-linesapce-instance-ctx-aware/<dataset>/<agent>/split/<split>/<qps>/<timestamp>/`

With `--output-suffix <suffix>`, the top-level replay directory name becomes:

- `results/replay/sweep-qps-docker-power-clean-gateway_multi-round_robin-freq-ctrl-linesapce-instance-ctx-aware-<suffix>/<dataset>/<agent>/split/<split>/<qps>/<timestamp>/`
...
```bash
.venv/bin/pytest experiments/sweep-qps-docker-power-clean-gateway_multi-round_robin-freq-ctrl-linesapce-instance-ctx-aware/test -q
```
