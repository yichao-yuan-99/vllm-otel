...
def load_generator_module() -> object:
    ...
    spec = importlib.util.spec_from_file_location(
        "generate_sweep_qps_docker_power_clean_freq_ctrl_linespace_sweep_slo_dual_experiment",
        module_path,
    )
...
def test_main_generates_dual_sweep_qps_slo_bundle(tmp_path: Path) -> None:
    ...
    assert manifest["freq_controller_bin_default"] == "freq-controller-linespace-slo"
    ...
    assert "[sweep-qps-docker-power-clean-freq-ctrl-linespace-sweep-slo-dual]" in run_script_text
    assert "GPU_ID=2" in run_script_text
    assert 'GATEWAY_BASE_URL_VALUE="${GATEWAY_BASE_URL:-}"' in run_script_text
    assert (
        'FREQ_CONTROLLER_GATEWAY_IPC_SOCKET_PATH_VALUE="${FREQ_CONTROLLER_GATEWAY_IPC_SOCKET_PATH:-'
        '${DEFAULT_FREQ_CONTROLLER_GATEWAY_IPC_SOCKET_PATH}}"'
        in run_script_text
    )
    assert "/tmp/vllm-gateway-ctx-profile-${PORT_PROFILE_ID_VALUE}.sock" in run_script_text
