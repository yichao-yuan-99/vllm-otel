...
This workflow is the `freq-controller-linespace-slo` plus `gateway_ctx`
SLO-aware variant of
`experiments/sweep-qps-docker-power-clean-freq-ctrl-linespace-sweep-slo`.

It keeps the same clean split-plan lookup, QPS/SLO sweep shape, Zeus power
logging, and independent frequency-controller flow, but the generated runner
also enables gateway ctx-aware mode and gateway SLO-aware mode before each
`(qps, target_slo)` replay job.

For every run:

- the freq controller receives `--throughput-target <target_slo>`
- the freq controller also receives
  `--gateway-ipc-socket-path /tmp/vllm-gateway-ctx-profile-<port_profile_id>.sock`
  by default
- gateway ctx-aware mode starts first with `policy_mode = "throughput"`
- gateway SLO-aware mode then starts with
  `target_tokens_per_s = <target_slo>` and
...
```bash
python3 experiments/sweep-qps-docker-power-clean-freq-ctrl-linespace-sweep-slo-dual/generate_experiment.py \
...
```bash
bash experiments/sweep-qps-docker-power-clean-freq-ctrl-linespace-sweep-slo-dual/generated/<timestamp>/run_replay.sh
```

Override port profile at runtime:

```bash
bash experiments/sweep-qps-docker-power-clean-freq-ctrl-linespace-sweep-slo-dual/generated/<timestamp>/run_replay.sh 3
```
...
- `FREQ_CONTROLLER_GATEWAY_IPC_SOCKET_PATH=/tmp/vllm-gateway-ctx-profile-<port_profile_id>.sock`
...
If `FREQ_CONTROLLER_GATEWAY_IPC_SOCKET_PATH` is not set, the generated runner
defaults it to `/tmp/vllm-gateway-ctx-profile-<port_profile_id>.sock` so
`gateway_ctx` sweeps connect to the matching per-profile socket automatically.
...
- `results/replay/sweep-qps-docker-power-clean-freq-ctrl-linespace-sweep-slo-dual/<dataset>/<agent>/split/<split>/<qps>/<slo>/<timestamp>/`
...
python3 post-process/run_all.py \
  --root-dir results/replay/sweep-qps-docker-power-clean-freq-ctrl-linespace-sweep-slo-dual
```

## Tests

```bash
.venv/bin/pytest experiments/sweep-qps-docker-power-clean-freq-ctrl-linespace-sweep-slo-dual/test -q
```
