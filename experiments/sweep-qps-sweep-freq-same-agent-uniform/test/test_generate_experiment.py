...
def load_generator_module() -> object:
    ...
    spec = importlib.util.spec_from_file_location(
        "generate_sweep_qps_sweep_freq_same_agent_uniform_experiment",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise RuntimeError(f"Unable to load module from {module_path}")
    module = importlib.util.module_from_spec(spec)
    sys.modules["generate_sweep_qps_sweep_freq_same_agent_uniform_experiment"] = module
...
def test_main_generates_sweep_qps_sweep_freq_same_agent_uniform_bundle(tmp_path: Path) -> None:
    ...
    replay_config_qps025_low = tomllib.loads(
        (batch_dir / "qps0_25" / "freq" / "core-345-810" / "replay.toml").read_text(encoding="utf-8")
    )
    replay_qps025_low = replay_config_qps025_low["replay"]
    assert replay_qps025_low["randomize_seed"] == 11
    assert replay_qps025_low["time_constraint_s"] == 600.0
    assert replay_qps025_low["port_profile_id"] == 3
    assert replay_qps025_low["launch_policy_override"]["pattern"]["name"] == "uniform"
    assert replay_qps025_low["launch_policy_override"]["pattern_args"]["rate"] == 0.25
    assert "seed" not in replay_qps025_low["launch_policy_override"]
    assert replay_qps025_low["plan"] == module.path_for_config(expected_plan_copy)
    assert replay_qps025_low["output_dir"].endswith(
        "results/replay/sweep-qps-sweep-freq-same-agent-uniform/"
        f"dataset-a/agent-a/trail/profile-2_run_beta/qps0_25/{batch_timestamp}/core-345-810"
    )

    replay_config_qps03_high = tomllib.loads(
        (batch_dir / "qps0_3" / "freq" / "core-345-1305" / "replay.toml").read_text(encoding="utf-8")
    )
    replay_qps03_high = replay_config_qps03_high["replay"]
    assert replay_qps03_high["launch_policy_override"]["pattern"]["name"] == "uniform"
    assert replay_qps03_high["launch_policy_override"]["pattern_args"]["rate"] == 0.3
    assert replay_qps03_high["plan"] == module.path_for_config(expected_plan_copy)
    assert replay_qps03_high["output_dir"].endswith(
        "results/replay/sweep-qps-sweep-freq-same-agent-uniform/"
        f"dataset-a/agent-a/trail/profile-2_run_beta/qps0_3/{batch_timestamp}/core-345-1305"
    )
    ...
    assert len(manifest["qps_points"][0]["frequency_points"]) == 2
    assert manifest["qps_points"][1]["qps"] == 0.3
    assert manifest["qps_points"][1]["qps_slug"] == "qps0_3"
    assert len(manifest["qps_points"][1]["frequency_points"]) == 2

    run_script_path = batch_dir / "run_replay.sh"
    run_script_text = run_script_path.read_text(encoding="utf-8")
    assert "[sweep-qps-sweep-freq-same-agent-uniform]" in run_script_text
    assert "SOURCE_TRAIL_NAME=profile-2/run_beta" in run_script_text
    assert "starting qps=0.25 qps_slug=qps0_25 freq_points=2" in run_script_text
    assert (
        "run_one_frequency 0.25 qps0_25 345 810 core-345-810 qps0_25/freq/core-345-810/replay.toml"
        in run_script_text
    )
    assert (
        "run_one_frequency 0.3 qps0_3 345 1305 core-345-1305 qps0_3/freq/core-345-1305/replay.toml"
        in run_script_text
    )
