#!/usr/bin/env python3
"""Generate one derived-plan same-agent uniform QPS x frequency sweep bundle."""
...
EXPERIMENT_DIR_NAME = "sweep-qps-sweep-freq-same-agent-uniform"
...
def build_generation_command_raw(argv: list[str] | None) -> str:
    ...
    return shlex.join([str(sys.executable), str(GENERATOR_SCRIPT_PATH), *[str(token) for token in argv]])
...
def write_run_script(
    ...
    *,
    ...
    gpu_id: int,
    qps_jobs: list[QpsSweepJob],
) -> None:
    total_frequency_runs = sum(len(job.frequency_jobs) for job in qps_jobs)
    lines = [
        "#!/usr/bin/env bash",
        "set -euo pipefail",
        "",
        ...
        "  local max_mhz=\"$4\"",
        "  local freq_slug=\"$5\"",
        "  local replay_config_ref=\"$6\"",
        "  local replay_output_ref=\"$7\"",
        ...
        "",
        f"  echo \"[{EXPERIMENT_LOG_TAG}] trail=${{SOURCE_TRAIL_NAME}} qps=${{qps_value}} freq=${{min_mhz}}:${{max_mhz}} slug=${{qps_slug}}/${{freq_slug}} output=${{replay_output_ref}} port_profile=${{PORT_PROFILE_ID_VALUE}}\"",
        ...
        (
            ...
            f"total_frequency_runs={total_frequency_runs} "
            f"gpu_id={gpu_id} "
    ...
    for qps_job in qps_jobs:
        lines.append(
            f"echo \"[{EXPERIMENT_LOG_TAG}] starting qps={qps_job.qps_display} "
            f"qps_slug={qps_job.qps_slug} freq_points={len(qps_job.frequency_jobs)}\""
        )
        for frequency_job in qps_job.frequency_jobs:
            lines.append(
                "run_one_frequency "
                f"{_shell_quote(qps_job.qps_display)} "
                f"{_shell_quote(qps_job.qps_slug)} "
                f"{frequency_job.min_mhz} "
                f"{frequency_job.max_mhz} "
                f"{_shell_quote(frequency_job.slug)} "
                f"{_shell_quote(frequency_job.replay_config_relpath)} "
                f"{_shell_quote(path_for_config(frequency_job.replay_output_dir))}"
            )
        lines.append("")

    lines.extend(
        [
            f"echo \"[{EXPERIMENT_LOG_TAG}] completed {len(qps_jobs)} qps points and {total_frequency_runs} total replay runs\"",
            "",
        ]
    )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        description=(
            "Generate a same-agent uniform QPS x frequency sweep replay bundle from one "
            "precompiled derived single-trail replay plan and emit one sweep entrypoint script."
        ),
    )
    parser.add_argument(
        "--source-plan",
    ...
    parser.add_argument(
        "--qps-list",
        required=True,
        help="Comma-separated uniform QPS rates, e.g. 0.1,0.2,0.3",
    )
    parser.add_argument(
        "--time-constraint-s",
        required=True,
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "<dataset-lineage>/trail/<safe-source-trail>/<qps>/<timestamp>/<freq-slug> "
            f"under results/replay/{EXPERIMENT_DIR_NAME}/. If dataset-lineage cannot "
            "be derived from the plan, the path starts at trail/<safe-source-trail>/..."
        ),
    )
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        for qps in qps_values:
            ...
            replay_output_base_dir = (replay_output_root_base / qps_slug / batch_timestamp).resolve()
            frequency_jobs: list[FrequencyReplayJob] = []
            for frequency_range in frequency_ranges:
                ...
                replay_config_path = (
                    batch_dir / qps_slug / "freq" / frequency_range.slug / "replay.toml"
                ).resolve()
            ...
            qps_jobs.append(
                QpsSweepJob(
                    ...
                    qps_slug=qps_slug,
                    replay_output_base_dir=replay_output_base_dir,
        ...
        qps_summary = [
            {
                "qps": qps_job.qps,
                "qps_slug": qps_job.qps_slug,
                "replay_output_base_dir": path_for_config(qps_job.replay_output_base_dir),
                "frequency_points": [
                    {
                        "min_mhz": job.min_mhz,
                        "max_mhz": job.max_mhz,
                    ...
                    for job in qps_job.frequency_jobs
                ],
            }
            for qps_job in qps_jobs
        ]
