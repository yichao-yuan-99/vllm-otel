# Sweep QPS Docker Power Clean Gateway Multi Lowest Profile Without Pending Freq Ctrl Linespace SLO

This workflow is the `gateway_multi` + `lowest_profile_without_pending` +
independent `freq-controller-linespace-slo` variant of
`experiments/sweep-qps-docker-power-clean-gateway_multi-lowest_profile_without_pending_freq_ctrl_linespace`.
...
```bash
python3 experiments/sweep-qps-docker-power-clean-gateway_multi-lowest_profile_without_pending_freq_ctrl_linespace-slo/generate_experiment.py \
...
```bash
bash experiments/sweep-qps-docker-power-clean-gateway_multi-lowest_profile_without_pending_freq_ctrl_linespace-slo/generated/<timestamp>/run_replay.sh
```

Override the gateway_multi port-profile selection at runtime:

```bash
bash experiments/sweep-qps-docker-power-clean-gateway_multi-lowest_profile_without_pending_freq_ctrl_linespace-slo/generated/<timestamp>/run_replay.sh 13,14
```
...
`freq-controller-linespace-slo` per backend with the selected
`--throughput-target`, and after the replay finishes it stops the controllers
and calls `POST /ctx-aware/end`.
...
- `results/replay/sweep-qps-docker-power-clean-gateway_multi-lowest_profile_without_pending_freq_ctrl_linespace-slo/<dataset>/<agent>/split/<split>/<qps>/<slo>/<timestamp>/`
...
```bash
.venv/bin/pytest experiments/sweep-qps-docker-power-clean-gateway_multi-lowest_profile_without_pending_freq_ctrl_linespace-slo/test -q
```
