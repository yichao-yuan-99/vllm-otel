...
def load_generator_module() -> object:
    ...
    spec = importlib.util.spec_from_file_location(
        (
            "generate_sweep_qps_docker_power_clean_gateway_multi_"
            "lowest_profile_without_pending_freq_ctrl_linespace_slo_experiment"
        ),
        module_path,
    )
...
def test_main_generates_sweep_qps_slo_gateway_multi_bundle(tmp_path: Path) -> None:
    ...
    assert manifest["assignment_policy"] == "lowest_profile_without_pending"
    ...
    assert "TARGET_ASSIGNMENT_POLICY=lowest_profile_without_pending" in run_script_text
    assert (
    ...
    assert (
        'FREQ_CONTROLLER_BIN="${FREQ_CONTROLLER_BIN:-freq-controller-linespace-slo}"'
        in run_script_text
    )
    assert (
    ...
    assert (
        "DEFAULT_FREQ_CONTROLLER_THRESHOLD="
        f"{module.DEFAULT_FREQ_CONTROLLER_THRESHOLD}"
    ) in run_script_text
...
def test_main_supports_repeated_port_profile_flags_custom_thresholds_suffix_and_single_slo(
    tmp_path: Path,
) -> None:
