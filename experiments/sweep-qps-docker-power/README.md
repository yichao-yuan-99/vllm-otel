...
- `results/replay/sweep-qps-docker-power/split/<split>/<qps>/<timestamp>/`

## Generate One Sweep Bundle

```bash
python3 experiments/sweep-qps-docker-power/generate_experiment.py \
...
```bash
bash experiments/sweep-qps-docker-power/generated/<timestamp>/run_replay.sh
```

Override port profile at runtime:

```bash
bash experiments/sweep-qps-docker-power/generated/<timestamp>/run_replay.sh 3
```
