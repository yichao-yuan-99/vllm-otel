#!/usr/bin/env python3
"""Generate one local sweep-QPS + power logging replay experiment bundle.
...
DEFAULT_OUTPUT_CONFIG_DIR = REPO_ROOT / "experiments" / "sweep-qps-docker-power" / "generated"
DEFAULT_REPLAY_OUTPUT_ROOT = Path("results") / "replay" / "sweep-qps-docker-power" / "split"
...
def write_replay_config(path: Path, *, replay_payload: dict[str, Any]) -> None:
    lines: list[str] = [
        "# Auto-generated by experiments/sweep-qps-docker-power/generate_experiment.py",
        "",
    ]
...
def write_run_script(
    ...
    *,
    ...
    lines = [
        ...
        "trap 'echo \"[sweep-qps-docker-power] interrupted\" >&2; exit 130' INT TERM",
        "",
        "run_one_qps() {",
        "  local qps_value=\"$1\"",
        "  local qps_slug=\"$2\"",
        ...
        "",
        "  echo \"[sweep-qps-docker-power] qps=${qps_value} slug=${qps_slug} output=${replay_output_ref} port_profile=${PORT_PROFILE_ID_VALUE}\"",
        ...
        "",
        (
            f"echo \"[sweep-qps-docker-power] target_model={target_model} split={split} "
    ...
    lines.extend(
        [
            "",
            f"echo \"[sweep-qps-docker-power] completed {len(qps_jobs)} qps points\"",
            "",
        ]
    )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        description=(
            "Generate a sweep-QPS replay bundle with power logging by looking up an "
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "<split>/<qps>/<timestamp> under results/replay/sweep-qps-docker-power/split/."
        ),
    )
    return parser
