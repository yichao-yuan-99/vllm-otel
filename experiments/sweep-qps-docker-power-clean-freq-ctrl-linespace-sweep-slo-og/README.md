# Sweep QPS Docker Power Clean Freq Ctrl Linespace Sweep SLO OG
...
- gateway SLO-aware mode then starts with `target_tokens_per_s = <target_slo>`
  and `policy_mode = "push-back-half-slack"`
...
```bash
python3 experiments/sweep-qps-docker-power-clean-freq-ctrl-linespace-sweep-slo-og/generate_experiment.py \
...
```bash
bash experiments/sweep-qps-docker-power-clean-freq-ctrl-linespace-sweep-slo-og/generated/<timestamp>/run_replay.sh
```

Override port profile at runtime:

```bash
bash experiments/sweep-qps-docker-power-clean-freq-ctrl-linespace-sweep-slo-og/generated/<timestamp>/run_replay.sh 3
```
...
- `results/replay/sweep-qps-docker-power-clean-freq-ctrl-linespace-sweep-slo-og/<dataset>/<agent>/split/<split>/<qps>/<slo>/<timestamp>/`
...
python3 post-process/run_all.py \
  --root-dir results/replay/sweep-qps-docker-power-clean-freq-ctrl-linespace-sweep-slo-og
```

## Tests

```bash
.venv/bin/pytest experiments/sweep-qps-docker-power-clean-freq-ctrl-linespace-sweep-slo-og/test -q
```
