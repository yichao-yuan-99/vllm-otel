...
DEFAULT_OUTPUT_CONFIG_DIR = (
    REPO_ROOT
    / "experiments"
    / "sweep-qps-docker-power-clean-gateway_multi-lowest_profile_without_pending_freq_ctrl_linespace"
    / "generated"
)
...
def write_run_script(
    ...
    *,
    ...
    default_port_profile_ids_csv = ",".join(str(profile_id) for profile_id in default_port_profile_ids)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        description=(
            ...
            "split plan, forcing assignment policy "
            "lowest_profile_without_pending, enabling ctx-aware mode, and "
            "starting one freq-controller-linespace instance per backend GPU."
        ),
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        selected_plan_copy_path = (batch_dir / "plan" / lookup_result.selected_plan_path.name).resolve()
