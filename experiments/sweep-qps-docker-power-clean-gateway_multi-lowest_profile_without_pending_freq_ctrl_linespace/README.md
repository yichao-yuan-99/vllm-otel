# Sweep QPS Docker Power Clean Gateway Multi Lowest Profile Without Pending Freq Ctrl Linespace

This workflow is the `gateway_multi` + `lowest_profile_without_pending` +
independent `freq-controller-linespace` variant of
`experiments/sweep-qps-docker-power-clean-gateway_multi-lowest_profile_without_pending`.

It keeps the same clean split-plan lookup, multi-profile replay, Zeus power
logging, assignment-policy setup, and gateway ctx-aware flow, but the generated
runner also:

- starts one `freq-controller-linespace` process per backend profile/GPU pair
  before each replay job
...
This extra ctx-aware control flow is still required because
...
```bash
python3 experiments/sweep-qps-docker-power-clean-gateway_multi-lowest_profile_without_pending_freq_ctrl_linespace/generate_experiment.py \
...
```bash
bash experiments/sweep-qps-docker-power-clean-gateway_multi-lowest_profile_without_pending_freq_ctrl_linespace/generated/<timestamp>/run_replay.sh
```

Override the gateway_multi port-profile selection at runtime:

```bash
bash experiments/sweep-qps-docker-power-clean-gateway_multi-lowest_profile_without_pending_freq_ctrl_linespace/generated/<timestamp>/run_replay.sh 13,14
```
...
`freq-controller-linespace` instances are paired with backend GPUs by list
position.
...
to `POST /policy`. Before each replay job, it sends the selected ctx-aware
thresholds to `POST /ctx-aware/start`, launches one
`freq-controller-linespace` per backend, and after the replay finishes it stops
the controllers and calls `POST /ctx-aware/end`.
...
- `results/replay/sweep-qps-docker-power-clean-gateway_multi-lowest_profile_without_pending_freq_ctrl_linespace/<dataset>/<agent>/split/<split>/<qps>/<timestamp>/`
...
This variant writes nested per-profile single-controller logs. The shared
freq-control post-process currently recognizes the flat
`freq-control-linespace/` layout and the multi-controller
...
```bash
.venv/bin/pytest experiments/sweep-qps-docker-power-clean-gateway_multi-lowest_profile_without_pending_freq_ctrl_linespace/test -q
```
