...
def load_generator_module() -> object:
    ...
    spec = importlib.util.spec_from_file_location(
        "generate_sweep_qps_docker_power_clean_gateway_multi_lowest_usage_freq_ctrl_linespace_slo_experiment",
        module_path,
    )
...
def test_main_generates_sweep_qps_slo_gateway_multi_lowest_usage_bundle(
    tmp_path: Path,
) -> None:
    ...
    assert replay_qps025_slo8["port_profile_id_list"] == [3, 4]
    ...
    assert replay_qps025_slo8["output_dir"].endswith(
        "results/replay/sweep-qps-docker-power-clean-gateway_multi-lowest_usage_freq_ctrl_linespace-slo/"
    ...
    assert manifest["freq_controller_bin_default"] == module.DEFAULT_FREQ_CONTROLLER_BIN
    ...
    assert [
        point["target_slo"] for point in manifest["qps_points"][0]["slo_points"]
    ] == [8.0, 12.0]
    ...
    assert (
        manifest["qps_points"][0]["slo_points"][0]["freq_controller_backend_log_dirs"][0][
            "freq_controller_log_dir"
        ]
    ).endswith(
        "results/replay/sweep-qps-docker-power-clean-gateway_multi-lowest_usage_freq_ctrl_linespace-slo/"
        f"dataset-a/agent-a/split/rest/qps0_25/slo8/{batch_timestamp}/freq-control-linespace/profile-3"
    )
    assert (
        manifest["qps_points"][0]["slo_points"][0]["freq_controller_backend_log_dirs"][1][
            "freq_controller_log_dir"
        ]
    ).endswith(
        "results/replay/sweep-qps-docker-power-clean-gateway_multi-lowest_usage_freq_ctrl_linespace-slo/"
        f"dataset-a/agent-a/split/rest/qps0_25/slo8/{batch_timestamp}/freq-control-linespace/profile-4"
    )
    ...
    assert "[sweep-qps-docker-power-clean-gateway_multi-lowest_usage_freq_ctrl_linespace-slo]" in run_script_text
    ...
    assert 'FREQ_CONTROLLER_BIN="${FREQ_CONTROLLER_BIN:-freq-controller-linespace-slo}"' in run_script_text
    ...
    assert '  freq_controller_log_root_dir="${replay_output_dir}/freq-control-linespace"' in run_script_text
...
def test_main_supports_repeated_port_profile_flags_custom_threshold_suffix_and_single_slo(
    tmp_path: Path,
) -> None:
    ...
    assert manifest["target_slo_list"] == [6.5]
    ...
    assert (
        ...
        "results/replay/sweep-qps-docker-power-clean-gateway_multi-lowest_usage_freq_ctrl_linespace-slo/"
        f"dataset-a/agent-a/split/exclude-unranked/qps0_5/slo6_5/{batch_timestamp}/freq-control-linespace/profile-5"
    )
    ...
    assert "slo_values=(6.5)" in run_script_text
