...
MODULE_ROOT = PROJECT_ROOT / "freq-controller-linespace-multi"
if str(MODULE_ROOT) not in sys.path:
    sys.path.insert(0, str(MODULE_ROOT))

from freq_controller_linespace_multi import (
    FrequencyController,
    FrequencyControllerConfig,
    GatewayContextSnapshot,
    GatewayIPCConfig,
    ZeusdConfig,
    ZeusdMultiGPUFrequencyController,
...
def test_parse_gpu_index_tokens_accepts_comma_and_space_separated_values() -> None:
    assert parse_gpu_index_tokens(["2", "3,4"]) == (2, 3, 4)
...
def test_load_controller_config_rejects_gpu_targeting_in_toml(
    ...
    with pytest.raises(ValueError, match="use --gpu-indices instead"):
...
def test_zeusd_multi_gpu_frequency_controller_applies_and_resets_each_gpu() -> None:
    controller = ZeusdMultiGPUFrequencyController(
...
def test_frequency_controller_uses_linespace_policy_and_resets_on_exit(
    ...
    fake_gpu = FakeGroupGPUController()
    ...
    assert log_paths.query_path.name.startswith("freq-controller-ls-multi.query.")
    assert log_paths.decision_path.name.startswith("freq-controller-ls-multi.decision.")
    assert log_paths.control_error_path.name.startswith(
        "freq-controller-ls-multi.control-error."
    )
