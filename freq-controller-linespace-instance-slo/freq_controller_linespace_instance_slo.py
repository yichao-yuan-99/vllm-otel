#!/usr/bin/env python3
"""Linespace instance GPU frequency controller with throughput SLO precedence."""
...
DEFAULT_LOG_FILE_PREFIX = "freq-controller-ls-instance-slo"
...
@dataclass(frozen=True)
class GatewayOutputThroughputSnapshot:
    job_active: bool
    job_started_at: str | None
    agent_count: int
    throughput_agent_count: int
    min_output_tokens_per_s: float | None
    max_output_tokens_per_s: float | None
    avg_output_tokens_per_s: float | None
...
class FrequencyControllerConfig:
    ...
    target_output_throughput_tokens_per_s: float
    ...
    def __post_init__(self) -> None:
        ...
        if self.target_output_throughput_tokens_per_s <= 0:
            raise ValueError("target_output_throughput_tokens_per_s must be > 0")
        ...
        *,
        ...
        return {
            ...
            "target_output_throughput_tokens_per_s": (
                self.target_output_throughput_tokens_per_s
            ),
...
def load_controller_config(
    ...
    *,
    ...
    resolved_output_throughput_target = _parse_float(
        output_throughput_target_value,
        "target_output_throughput_tokens_per_s",
    )
    ...
    return FrequencyControllerConfig(
        ...
        target_output_throughput_tokens_per_s=resolved_output_throughput_target,
...
class GatewayContextClient:
    ...
    def _read_json_payload(self, path: str) -> dict[str, Any]:
        ...
        if not isinstance(payload, dict):
            raise RuntimeError("gateway IPC response must be a JSON object")
        return payload

    def read_context_snapshot(self) -> GatewayContextSnapshot:
        payload = self._read_json_payload("/ipc/context")
    ...
    def read_output_throughput_snapshot(self) -> GatewayOutputThroughputSnapshot:
        payload = self._read_json_payload("/ipc/output-throughput")
        ...
        if agent_count < 0:
            raise RuntimeError("gateway IPC response agent_count must be >= 0")
        throughput_agent_count = payload.get("throughput_agent_count")
        if isinstance(throughput_agent_count, bool) or not isinstance(
            throughput_agent_count,
            int,
        ):
            raise RuntimeError("gateway IPC response is missing throughput_agent_count")
        if throughput_agent_count < 0:
            raise RuntimeError(
                "gateway IPC response throughput_agent_count must be >= 0"
            )

        def parse_optional_metric(field_name: str) -> float | None:
            metric_value = payload.get(field_name)
            ...
            if isinstance(metric_value, bool) or not isinstance(
                ...
                raise RuntimeError(
                    f"gateway IPC response {field_name} must be a number or null"
                )
            parsed_value = float(metric_value)
            if not math.isfinite(parsed_value):
                raise RuntimeError(
                    f"gateway IPC response {field_name} must be finite when present"
                )
            return parsed_value

        min_output_tokens_per_s = parse_optional_metric("min_output_tokens_per_s")
        max_output_tokens_per_s = parse_optional_metric("max_output_tokens_per_s")
        avg_output_tokens_per_s = parse_optional_metric("avg_output_tokens_per_s")
        if throughput_agent_count == 0:
            if (
                min_output_tokens_per_s is not None
                or max_output_tokens_per_s is not None
                or avg_output_tokens_per_s is not None
            ):
                raise RuntimeError(
                    "gateway IPC response throughput summary must be null when "
                    "throughput_agent_count is 0"
                )
        elif (
            min_output_tokens_per_s is None
            or max_output_tokens_per_s is None
            or avg_output_tokens_per_s is None
        ):
            raise RuntimeError(
                "gateway IPC response throughput summary is incomplete"
            )
        ...
        return GatewayOutputThroughputSnapshot(
            ...
            job_started_at=job_started_at,
            agent_count=agent_count,
            throughput_agent_count=throughput_agent_count,
            min_output_tokens_per_s=min_output_tokens_per_s,
            max_output_tokens_per_s=max_output_tokens_per_s,
            avg_output_tokens_per_s=avg_output_tokens_per_s,
        )
...
class FrequencyControllerLogs:
    def __init__(self, log_dir: Path) -> None:
        ...
        self.slo_decision = JsonlLogFile(
            resolved_log_dir
            / f"{DEFAULT_LOG_FILE_PREFIX}.slo-decision.{started_at}.jsonl"
        )
        ...
        self.paths = FrequencyControllerLogPaths(
            ...
            slo_decision_path=self.slo_decision.path,
        )
...
def choose_next_frequency_index(
    *,
    ...
    target_context_usage_threshold: float,
    max_index: int,
    moving_average_min_output_tokens_per_s: float | None = None,
    target_output_throughput_tokens_per_s: float | None = None,
) -> tuple[int, str]:
    if (
        target_output_throughput_tokens_per_s is not None
        and moving_average_min_output_tokens_per_s is not None
        and moving_average_min_output_tokens_per_s
        < target_output_throughput_tokens_per_s
    ):
        return max_index, "increase_for_slo"
...
class FrequencyController:
    def __init__(
        ...
        *,
        ...
        self.context_window = MovingAverageWindow(config.control_interval_s)
        self.output_throughput_window = MovingAverageWindow(config.control_interval_s)
        self.current_frequency_index = config.initial_frequency_index
        self.last_context_snapshot: GatewayContextSnapshot | None = None
        self.last_output_throughput_snapshot: (
            GatewayOutputThroughputSnapshot | None
        ) = None
    ...
    def _read_context_snapshot_with_fallback(
        ...
        try:
            ...
            if self.last_context_snapshot is None:
                raise
            return self.last_context_snapshot, str(exc)
        self.last_context_snapshot = snapshot
        return snapshot, None

    def _read_output_throughput_snapshot_with_fallback(
        self,
    ) -> tuple[GatewayOutputThroughputSnapshot, str | None]:
        try:
            snapshot = self.gateway_client.read_output_throughput_snapshot()
        except Exception as exc:
            if self.last_output_throughput_snapshot is None:
                raise
            return self.last_output_throughput_snapshot, str(exc)
        self.last_output_throughput_snapshot = snapshot
        return snapshot, None

    def _sample_gateway_metrics(self, now_s: float) -> None:
        context_snapshot, context_read_error = self._read_context_snapshot_with_fallback()
        context_usage = context_snapshot.total_context_tokens
        self.context_window.add(now_s, context_usage)

        throughput_snapshot: GatewayOutputThroughputSnapshot | None = None
        throughput_read_error: str | None = None
        try:
            throughput_snapshot, throughput_read_error = (
                self._read_output_throughput_snapshot_with_fallback()
            )
        except Exception as exc:
            throughput_read_error = str(exc)
            self.output_throughput_window.prune(now_s)
        else:
            min_output_tokens_per_s = throughput_snapshot.min_output_tokens_per_s
        ...
        if throughput_read_error is not None:
            error_messages.append(f"output_throughput: {throughput_read_error}")
        query_fields: dict[str, Any] = {
            "phase": "active",
            "context_usage": context_usage,
            "job_active": context_snapshot.job_active,
            "agent_count": context_snapshot.agent_count,
            "sample_count_window": self.context_window.sample_count,
            "throughput_sample_count_window": self.output_throughput_window.sample_count,
            "throughput_agent_count": (
                None
                if throughput_snapshot is None
                else throughput_snapshot.throughput_agent_count
            ),
            "min_output_tokens_per_s": (
                None
                if throughput_snapshot is None
                else throughput_snapshot.min_output_tokens_per_s
            ),
            "max_output_tokens_per_s": (
                None
                if throughput_snapshot is None
                else throughput_snapshot.max_output_tokens_per_s
            ),
    ...
    def _control_once(self, now_s: float) -> None:
        ...
        if moving_average_context_usage is None:
            return
        moving_average_min_output_tokens_per_s = self.output_throughput_window.average(
            now_s
        )

        max_index = len(self.config.frequency_mhz_levels) - 1
        context_target_index = choose_target_frequency_index(
            moving_average_context_usage=moving_average_context_usage,
        ...
        next_index, action = choose_next_frequency_index(
            ...
            target_context_usage_threshold=self.config.target_context_usage_threshold,
            max_index=max_index,
            moving_average_min_output_tokens_per_s=(
                moving_average_min_output_tokens_per_s
            ),
            target_output_throughput_tokens_per_s=(
                self.config.target_output_throughput_tokens_per_s
            ),
        )
        slo_override_applied = (
            moving_average_min_output_tokens_per_s is not None
            and moving_average_min_output_tokens_per_s
            < self.config.target_output_throughput_tokens_per_s
        )
        ...
        decision_fields: dict[str, Any] = {
            ...
            "changed": changed,
            "decision_policy": (
                "throughput_slo_precedence"
                if slo_override_applied
                else "context_linespace"
            ),
            "slo_override_applied": slo_override_applied,
            "current_frequency_mhz": current_frequency_mhz,
            "target_frequency_mhz": target_frequency_mhz,
            "window_context_usage": moving_average_context_usage,
            "window_min_output_tokens_per_s": moving_average_min_output_tokens_per_s,
            "sample_count": self.context_window.sample_count,
            "throughput_sample_count": self.output_throughput_window.sample_count,
            "target_context_usage_threshold": (
                self.config.target_context_usage_threshold
            ),
            "target_output_throughput_tokens_per_s": (
                self.config.target_output_throughput_tokens_per_s
            ),
            ...
            "segment_width_context_usage": (
                self.config.segment_width_context_usage
            ),
        ...
        try:
            ...
            while True:
                ...
                if now_s >= next_sample_s:
                    try:
                        self._sample_gateway_metrics(now_s)
                    except Exception as exc:
                        self.logs.query.write(
                            phase="active",
                            error=str(exc),
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="freq-controller-linespace-instance-slo",
        description=(
            "Control GPU core frequency from gateway context usage with "
            "instance-style direct targets, throughput SLO precedence, and zeusd."
        ),
    )
    parser.add_argument(
        "--config",
    ...
    parser.add_argument(
        "--target-output-throughput-tokens-per-s",
        "--target-output-throughput",
        "--throughput-target",
        dest="target_output_throughput_tokens_per_s",
        type=float,
        help=(
            "Minimum moving-average min agent output throughput in tokens/s. "
            "If the observed value is lower, the controller jumps directly to "
            "the highest configured frequency for that decision. Required "
            "unless configured in TOML."
        ),
    )
    parser.add_argument(
        "--only-freq-list-geq",
    ...
    parser.add_argument(
        ...
        help=(
            "GPU index to control through zeusd. Pass a single integer for one GPU "
            "or a comma-separated list to apply the same frequency to multiple GPUs. "
    ...
    try:
        ...
        print(
            (
                "freq-controller-linespace-instance-slo logs: "
                ...
                f"slo_decision={log_paths.slo_decision_path}"
            ),
            file=sys.stderr,
        )
        return 0
