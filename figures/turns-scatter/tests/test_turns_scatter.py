...
MATERIALIZE_SCRIPT = REPO_ROOT / "figures" / "turns-scatter" / "materialize_turns_scatter.py"
PLOT_SCRIPT = REPO_ROOT / "figures" / "turns-scatter" / "plot_turns_scatter.py"
...
class TurnsScatterFigureTest(unittest.TestCase):
    def test_materialize_turns_scatter_builds_expected_panels(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            output_path = tmp_path / "turns-scatter.json"
            subprocess.run(
                [
                    sys.executable,
            ...
            self.assertEqual(payload["figure_name"], "turns-scatter")
            self.assertEqual(
    ...
    def test_plot_turns_scatter_writes_output(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            input_path = tmp_path / "turns-scatter.json"
            output_path = tmp_path / "turns-scatter.png"
            subprocess.run(
                [
                    sys.executable,
                    str(MATERIALIZE_SCRIPT),
