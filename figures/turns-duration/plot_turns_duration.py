#!/usr/bin/env python3
"""Render the turns-duration figure from materialized agent-duration distributions."""
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Render the turns-duration figure from the JSON output of "
            "materialize_turns_duration.py."
        )
    )
    parser.add_argument(
        "--input",
        required=True,
        help="Input JSON from materialize_turns_duration.py.",
    )
    parser.add_argument(
        "--output",
        default=None,
        help="Optional output path. Default: figures/turns-duration/output/<input-stem>.pdf",
    )
    parser.add_argument(
        "--y-scale",
        choices=("log", "linear"),
...
def _format_duration_tick(value: float, _position: float) -> str:
    if value <= 0.0:
        return ""
    if value >= 1000.0 and value.is_integer():
        return f"{int(value):,}"
    return f"{value:g}"
...
def main() -> int:
    ...
    figure_title = (
        ...
        or "Agent Duration Distribution by Benchmark and Agent"
    )
    metric_label = (
    ...
    if args.y_scale == "linear":
        lower_bound = min_duration * 0.85 if min_duration > 0.0 else 0.0
        upper_bound = max_duration * 1.18 if max_duration > 0.0 else 1.0
        if upper_bound <= lower_bound:
            upper_bound = lower_bound + 1.0
    else:
        lower_bound = min_duration * 0.9
        upper_bound = max_duration * 1.30
        if upper_bound <= lower_bound:
