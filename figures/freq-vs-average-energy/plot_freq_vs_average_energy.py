#!/usr/bin/env python3
"""Plot frequency vs average energy and throughput from a materialized CSV."""
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Plot the freq-vs-average-energy figure from a materialized CSV."
    )
    parser.add_argument(
        "--input",
        required=True,
    ...
    parser.add_argument(
        "--output",
        default=None,
        help="Optional output path. Default: figures/freq-vs-average-energy/output/<input-stem>.pdf",
    )
    parser.add_argument(
        "--title",
        default="Frequency vs Average Energy and Throughput",
        help="Figure title.",
    )
    parser.add_argument(
        "--figure-width",
    ...
    parser.add_argument(
        ...
        type=float,
        default=5.6,
        help="Figure height in inches (default: 5.6).",
    )
    parser.add_argument(
        "--dpi",
        type=int,
...
def main() -> int:
    ...
    left_axis.set_ylabel("Average energy per finished replay (kJ)", color=left_color)
