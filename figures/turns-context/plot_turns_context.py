#!/usr/bin/env python3
"""Render the turns-context figure from materialized max-context distributions."""
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Render the turns-context figure from the JSON output of "
            "materialize_turns_context.py."
        )
    )
    parser.add_argument(
        "--input",
        required=True,
        help="Input JSON from materialize_turns_context.py.",
    )
    parser.add_argument(
        "--output",
        default=None,
        help="Optional output path. Default: figures/turns-context/output/<input-stem>.pdf",
    )
    parser.add_argument(
        "--y-scale",
        choices=("log", "linear"),
    ...
    parser.add_argument(
        "--title",
        default="Maximum Context Usage Distribution by Benchmark and Agent",
        help="Figure title.",
    )
    parser.add_argument(
        "--figure-width",
