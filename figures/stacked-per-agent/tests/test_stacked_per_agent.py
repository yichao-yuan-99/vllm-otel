...
class StackedPerAgentFigureTest(unittest.TestCase):
    ...
    def test_materialize_stacked_per_agent_can_highlight_top_n_active_durations(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            tmp_path = Path(temp_dir)
            run_dir = tmp_path / "run"
            output_path = tmp_path / "stacked-per-agent.json"

            payload = _materialize_fixture(
                run_dir,
            ...
            self.assertEqual(
                payload["color_policy"],
                {
                    "unique_color_top_n_by_active_duration": 2,
                    "shared_color_hex": "#D0D7DE",
                    "highlighted_agents_stack_below_shared_color_agents": True,
                    "highlighted_agents_order": "first-active-descending",
                },
            )
            self.assertEqual(
                [agent["agent_key"] for agent in payload["agents"]],
            ...
            self.assertEqual(
                [agent["stack_rank"] for agent in payload["agents"]],
                [1, 0, 2],
            )
            self.assertEqual(
                [agent["analysis_active_duration_s"] for agent in payload["agents"]],
                [240.0, 120.0, 90.0],
            )
            self.assertNotEqual(payload["agents"][0]["color_hex"], "#D0D7DE")
            self.assertNotEqual(payload["agents"][1]["color_hex"], "#D0D7DE")
            self.assertNotEqual(
                payload["agents"][0]["color_hex"],
                payload["agents"][1]["color_hex"],
            )
            self.assertEqual(payload["agents"][2]["color_hex"], "#D0D7DE")
            self.assertEqual(
                [entry["agent_key"] for entry in payload["windows"][0]["contributions"]],
                ["agent-b", "agent-a"],
            )
            self.assertEqual(
                [entry["agent_key"] for entry in payload["windows"][1]["contributions"]],
                ["agent-b", "agent-a", "agent-c"],
            )

    def test_materialize_stacked_per_agent_orders_highlighted_agents_by_later_start(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            tmp_path = Path(temp_dir)
            run_dir = tmp_path / "run"
            ranges_path = (
