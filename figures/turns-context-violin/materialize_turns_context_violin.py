#!/usr/bin/env python3
"""Materialize per-job max-context distributions for the turns-context-violin figure."""
...
DEFAULT_OUTPUT_STEM = "turns-context-violin"
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Materialize per-job max-context distributions for the "
            "turns-context-violin figure from run-stats/run-stats-summary.json files."
        )
    )
    parser.add_argument(
        "--root-dir",
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "figures/turns-context-violin/data/"
            "turns-context-violin.<root>.benchmarks-<...>.agents-<...>.json"
        ),
    )
...
def _summarize_context_usages(context_usages: list[int]) -> dict[str, float | int]:
    if not context_usages:
        raise ValueError("At least one max-context value is required")
    sorted_contexts = sorted(context_usages)
    q1 = _quantile_from_sorted_values(sorted_contexts, 0.25)
    median = _quantile_from_sorted_values(sorted_contexts, 0.50)
    q3 = _quantile_from_sorted_values(sorted_contexts, 0.75)
    mean = sum(sorted_contexts) / len(sorted_contexts)
    return {
        "sample_count": len(sorted_contexts),
        "min": min(sorted_contexts),
        "q1": q1,
        "median": median,
        "q3": q3,
        "p90": _quantile_from_sorted_values(sorted_contexts, 0.90),
        "p95": _quantile_from_sorted_values(sorted_contexts, 0.95),
        "mean": mean,
        "max": max(sorted_contexts),
        "iqr": q3 - q1,
    }
...
def _materialize_panel(selection: PanelSelection) -> dict[str, Any]:
    ...
    context_usages: list[int] = []
    raw_lengths = payload.get("job_max_request_lengths")
    if isinstance(raw_lengths, list):
        for raw_length in raw_lengths:
            parsed_length = _int_or_none(raw_length)
            if parsed_length is None or parsed_length < 0:
                continue
            context_usages.append(parsed_length)

    if not context_usages:
        raw_jobs = payload.get("jobs")
        if not isinstance(raw_jobs, list):
            raise ValueError(
                "Missing both 'job_max_request_lengths' and 'jobs' in "
                f"{selection.summary_path}"
            )
        for raw_job in raw_jobs:
            if not isinstance(raw_job, dict):
                continue
            parsed_length = _int_or_none(raw_job.get("max_request_length"))
            if parsed_length is None or parsed_length < 0:
                continue
            context_usages.append(parsed_length)
    ...
    if not context_usages:
        raise ValueError(
            "No usable per-job max context values were found in "
            f"{selection.summary_path}"
        )
    ...
    stats = _summarize_context_usages(context_usages)
    return {
        ...
        "avg_job_max_request_length_reported": _float_or_none(
            payload.get("avg_job_max_request_length")
        ),
        "max_job_max_request_length_reported": _float_or_none(
            payload.get("max_job_max_request_length")
        ),
        "avg_job_max_request_length_computed": stats["mean"],
        "max_job_max_request_length_computed": stats["max"],
        "max_context_usages": context_usages,
        "stats": stats,
    }
...
def main() -> int:
    ...
    all_context_usages = [
        context_usage
        for panel in panels
        for context_usage in panel["max_context_usages"]
        if isinstance(context_usage, int)
    ]
    output_path = (
    ...
    materialized_payload = {
        ...
        "figure_title": (
            "Maximum Context Usage Distribution by Benchmark and Agent "
            "(Violin + Boxplot)"
        ),
        "metric_name": "max_context_usage",
        "metric_label": "Maximum context usage per job (tokens)",
        "metric_definition": (
            "max_context_usage = job_max_request_lengths or jobs[].max_request_length "
            "from run-stats-summary.json, where request length = prompt_tokens + "
            "generation_tokens"
        ),
        "context_source_fields": [
            "job_max_request_lengths",
            "jobs[].max_request_length",
        ],
        ...
        "total_job_count": len(all_context_usages),
        "global_min_context_usage": min(all_context_usages),
        "global_max_context_usage": max(all_context_usages),
        "panels": panels,
    }
