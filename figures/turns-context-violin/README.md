...
- each raw point is one job's max request length
- each violin shows a kernel-density estimate over the same per-job values
- each overlaid box summarizes the same per-job max-context distribution
- each diamond mark is the arithmetic mean of that panel's `max_context_usages`
...
- a log-scale y-axis by default, because max-context distributions are strongly
  heavy-tailed
...
```bash
python3 figures/turns-context-violin/materialize_turns_context_violin.py \
...
- `figures/turns-context-violin/data/turns-context-violin.qwen3-coder-30b.benchmarks-dabstep_swebench-verified_terminal-bench-2-0.agents-mini-swe-agent_terminus-2.json`
...
```bash
python3 figures/turns-context-violin/plot_turns_context_violin.py \
  --input figures/turns-context-violin/data/turns-context-violin.qwen3-coder-30b.benchmarks-dabstep_swebench-verified_terminal-bench-2-0.agents-mini-swe-agent_terminus-2.json
```
...
python3 figures/turns-context-violin/plot_turns_context_violin.py \
  --input figures/turns-context-violin/data/turns-context-violin.qwen3-coder-30b.benchmarks-dabstep_swebench-verified_terminal-bench-2-0.agents-mini-swe-agent_terminus-2.json \
  --output figures/turns-context-violin/output/turns-context-violin.qwen3-coder-30b.linear.png \
  --y-scale linear
```
