#!/usr/bin/env python3
"""Render the turns-context-violin figure from materialized max-context distributions."""
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Render the turns-context-violin figure from the JSON output of "
            "materialize_turns_context_violin.py."
        )
    )
    parser.add_argument(
        "--input",
        required=True,
        help="Input JSON from materialize_turns_context_violin.py.",
    )
    parser.add_argument(
        "--output",
        default=None,
        help=(
            ...
            "figures/turns-context-violin/output/<input-stem>.pdf"
        ),
    )
    parser.add_argument(
        "--y-scale",
...
def main() -> int:
    ...
    for fallback_index, raw_panel in enumerate(raw_panels):
        if not isinstance(raw_panel, dict):
            continue
        raw_contexts = raw_panel.get("max_context_usages")
        if not isinstance(raw_contexts, list):
            continue
        context_usages = [
            parsed
            for parsed in (_float_or_none(value) for value in raw_contexts)
        ...
        benchmark_label = _string_or_none(raw_panel.get("benchmark_label")) or benchmark
        agent_type = (
    ...
    min_context = min(min(panel["max_context_usages"]) for panel in panels)
    max_context = max(max(panel["max_context_usages"]) for panel in panels)
    if args.y_scale == "log" and min_context <= 0.0:
        raise ValueError(
            "Log y-scale requires strictly positive max-context values. "
