#!/usr/bin/env python3
"""Render the isolate-capacity 1x3 bar-chart figure from materialized JSON."""
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Plot the isolate-capacity figure from a materialized JSON dataset."
    )
    parser.add_argument(
        "--input",
        required=True,
        help="Input JSON from materialize_isolate_capacity.py.",
    )
    parser.add_argument(
        "--output",
        default=None,
        help="Optional output path. Default: figures/isolate-capacity/output/<input-stem>.pdf",
    )
    parser.add_argument(
        "--title",
        default="Capacity Isolation Comparison",
        help="Figure title.",
    )
    parser.add_argument(
        "--figure-width",
...
def main() -> int:
    ...
    qps_slug = payload.get("qps_slug") if isinstance(payload.get("qps_slug"), str) else None
    subtitle_parts = [f"{len(cases)} cases"]
    if qps_slug:
        subtitle_parts.append(f"source runs: {qps_slug}")
