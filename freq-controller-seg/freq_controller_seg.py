#!/usr/bin/env python3
"""Segmented GPU frequency controller driven by gateway context usage."""
...
class FrequencyControllerConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if self.low_freq_threshold > self.target_context_usage_lower_bound:
            raise ValueError(
                "low_freq_threshold must be <= target_context_usage_lower_bound"
            )
        if self.low_freq_cap_mhz not in self.frequency_mhz_levels:
            raise ValueError(
                "low_freq_cap_mhz must be one of frequency_mhz_levels"
            )
    ...
    @property
    def low_freq_cap_index(self) -> int:
        return self.frequency_mhz_levels.index(self.low_freq_cap_mhz)
    ...
    def to_log_payload(
        ...
        *,
        ...
        return {
            ...
            "low_freq_threshold": self.low_freq_threshold,
            "low_freq_cap_mhz": self.low_freq_cap_mhz,
...
def load_controller_config(
    ...
    *,
    ...
    low_freq_threshold: float | None = None,
    low_freq_cap_mhz: int | None = None,
    ...
    low_freq_threshold_value = (
        low_freq_threshold
        if low_freq_threshold is not None
        else _lookup_with_fallback(
            config_table,
            shared_config_table,
            ("low_freq_threshold", "lfth"),
        )
    )
    resolved_low_freq_threshold = _parse_float(
        low_freq_threshold_value,
        "low_freq_threshold",
    )
    low_freq_cap_value = (
        ...
        if low_freq_cap_mhz is not None
            ...
            config_table,
            shared_config_table,
            ("low_freq_cap_mhz", "low_freq_cap"),
        )
    )
    resolved_low_freq_cap_mhz = _parse_int(
        low_freq_cap_value,
        "low_freq_cap_mhz",
    )
    if resolved_low_freq_cap_mhz <= 0:
        raise ValueError("low_freq_cap_mhz must be > 0")
    ...
    return FrequencyControllerConfig(
        ...
        low_freq_threshold=resolved_low_freq_threshold,
        low_freq_cap_mhz=resolved_low_freq_cap_mhz,
...
def effective_min_frequency_index(
    *,
    moving_average_context_usage: float,
    low_freq_threshold: float,
    low_freq_cap_index: int,
) -> int:
    if moving_average_context_usage < low_freq_threshold:
        return 0
    return low_freq_cap_index
...
def choose_next_frequency_index(
    *,
    ...
    effective_min_index = effective_min_frequency_index(
        moving_average_context_usage=moving_average_context_usage,
        low_freq_threshold=low_freq_threshold,
        low_freq_cap_index=low_freq_cap_index,
    )
    if current_index < effective_min_index:
        return min(effective_min_index, current_index + 1), "increase_to_low_freq_cap"
    if moving_average_context_usage < lower_bound:
        return max(effective_min_index, current_index - 1), "decrease"
...
class FrequencyController:
    ...
    def _control_once(self, now_s: float) -> None:
        ...
        effective_min_index = effective_min_frequency_index(
            ...
            low_freq_threshold=self.config.low_freq_threshold,
            low_freq_cap_index=self.config.low_freq_cap_index,
        )
        ...
        self.logs.decision.write(
            ...
            upper_bound=self.config.target_context_usage_upper_bound,
            low_freq_threshold=self.config.low_freq_threshold,
            low_freq_cap_mhz=self.config.low_freq_cap_mhz,
            effective_min_frequency_mhz=self.config.frequency_mhz_levels[
                effective_min_index
            ],
        )
        if not changed:
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="freq-controller-seg",
        description=(
            "Control GPU core frequency from gateway context usage using a "
            "segmented low-frequency floor and zeusd."
        ),
    )
    parser.add_argument(
        "--config",
    ...
    parser.add_argument(
        "--low-freq-threshold",
        "--lfth",
        dest="low_freq_threshold",
        type=float,
        help=(
            "Context threshold below which the controller may use the true "
            "minimum frequency. Required unless configured in TOML."
        ),
    )
    parser.add_argument(
        "--low-freq-cap",
        "--low-freq-cap-mhz",
        dest="low_freq_cap_mhz",
        type=int,
        help=(
            "Frequency floor used whenever context usage is at or above the "
            "low-frequency threshold. Required unless configured in TOML."
        ),
    )
    parser.add_argument(
        "--port-profile-id",
    ...
    try:
        log_paths = run_controller(
            ...
            low_freq_cap_mhz=args.low_freq_cap_mhz,
            port_profile_id=args.port_profile_id,
            gpu_index=args.gpu_index,
        )
        print(
            (
                "freq-controller-seg logs: "
