...
from gateway_ctx.app import GatewayConfig, create_app, create_gateway_service
from gateway_ctx.model_configs import ModelRegistry, load_model_registry
from gateway_ctx.port_profiles import PortProfile, load_port_profile
from gateway_ctx.runtime_config import DEFAULT_CONFIG_PATH, load_runtime_settings
...
_BOOTSTRAP_ENV = "VLLM_OTEL_GATEWAY_CTX_BOOTSTRAPPED"
...
def _ensure_config_exists(config_path: Path) -> None:
    ...
    typer.secho(
        "hint: copy gateway_ctx/config.example.toml to gateway_ctx/config.toml and edit as needed",
        err=True,
...
def _default_ipc_socket_path(profile_id: int | str | None) -> Path:
    ...
    return DEFAULT_IPC_SOCKET_DIR / f"vllm-gateway-ctx-profile-{resolved_profile_id}.sock"
...
def start(
    config: Path = typer.Option(
        ...
        dir_okay=False,
        file_okay=True,
        help="Path to the gateway_ctx TOML config file.",
    ),
    port_profile_id: int | None = typer.Option(
        None,
        help="Port profile numeric ID from configs/port_profiles.toml. Overrides run.port_profile_id in gateway_ctx/config.toml.",
    ),
