...
from gateway_ctx.app import (
    CTX_AWARE_SCHEDULER_INTERVAL_S,
    ForwardResult,
    GatewayConfig,
    GatewayService,
    create_app,
...
def test_job_end_blocks_for_configured_wait(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("gateway_ctx.app.time.sleep", fake_sleep)
...
def test_ipc_context_reports_active_agent_context_usage(tmp_path: Path) -> None:
    ...
    assert initial_payload["agent_count"] == 2
    assert initial_payload["total_context_tokens"] == 0
    assert initial_payload["ctx_aware_enabled"] is False
    assert initial_payload["usage_threshold_tokens"] is None
    assert initial_payload["scheduling_threshold_tokens"] is None
    assert initial_payload["effective_total_context_tokens"] == 6000
    assert initial_payload["ongoing_agent_count"] == 2
    assert initial_payload["pending_agent_count"] == 0
    assert initial_payload["ongoing_effective_context_tokens"] == 6000
    assert initial_payload["pending_effective_context_tokens"] == 0
    ...
    assert payload["effective_total_context_tokens"] == 1700
    assert payload["ongoing_agent_count"] == 2
    assert payload["pending_agent_count"] == 0
    assert payload["ongoing_effective_context_tokens"] == 1700
    assert payload["pending_effective_context_tokens"] == 0
    assert payload["agents"][0]["api_token_hash"] == sha256_hex(token_a)
    assert payload["agents"][0]["trace_id"] == active_runs[token_a].trace_id
    assert payload["agents"][0]["run_start_time"] == active_runs[token_a].run_start_time
    assert payload["agents"][0]["schedule_state"] == "ongoing"
    assert payload["agents"][0]["context_tokens"] == 1200
    assert payload["agents"][0]["effective_context_tokens"] == 1200
    assert payload["agents"][0]["has_usable_context_usage"] is True
    assert payload["agents"][0]["pending_since"] is None
    assert payload["agents"][0]["ralexation_until"] is None
    assert payload["agents"][0]["output_tokens_per_s"] is not None
    assert payload["agents"][0]["slo_slack_s"] is None

    assert payload["agents"][1]["api_token_hash"] == sha256_hex(token_b)
    assert payload["agents"][1]["trace_id"] == active_runs[token_b].trace_id
    assert payload["agents"][1]["run_start_time"] == active_runs[token_b].run_start_time
    assert payload["agents"][1]["schedule_state"] == "ongoing"
    assert payload["agents"][1]["context_tokens"] == 500
    assert payload["agents"][1]["effective_context_tokens"] == 500
    assert payload["agents"][1]["has_usable_context_usage"] is True
    assert payload["agents"][1]["pending_since"] is None
    assert payload["agents"][1]["ralexation_until"] is None
    assert payload["agents"][1]["output_tokens_per_s"] is not None
    assert payload["agents"][1]["slo_slack_s"] is None
    ...
    assert payload["effective_total_context_tokens"] == 2100
    ...
    assert payload["effective_total_context_tokens"] == 1600
    assert payload["agents"][0]["api_token_hash"] == sha256_hex(token_a)
    assert payload["agents"][0]["trace_id"] == active_runs[token_a].trace_id
    assert payload["agents"][0]["run_start_time"] == active_runs[token_a].run_start_time
    assert payload["agents"][0]["schedule_state"] == "ongoing"
    assert payload["agents"][0]["context_tokens"] == 1600
    assert payload["agents"][0]["effective_context_tokens"] == 1600
    assert payload["agents"][0]["has_usable_context_usage"] is True
    assert payload["agents"][0]["pending_since"] is None
    assert payload["agents"][0]["ralexation_until"] is None
    assert payload["agents"][0]["output_tokens_per_s"] is not None
...
def test_ipc_context_keeps_last_known_value_when_usage_is_missing(tmp_path: Path) -> None:
    ...
    first_payload = client.get("/ipc/context").json()
    assert first_payload["total_context_tokens"] == 1200
    assert first_payload["effective_total_context_tokens"] == 1200
    assert first_payload["agents"][0]["has_usable_context_usage"] is True
    assert first_payload["agents"][0]["effective_context_tokens"] == 1200
    ...
    second_payload = client.get("/ipc/context").json()
    assert second_payload["total_context_tokens"] == 1200
    assert second_payload["effective_total_context_tokens"] == 1200
    assert second_payload["agents"][0]["has_usable_context_usage"] is True
    assert second_payload["agents"][0]["effective_context_tokens"] == 1200


def test_ctx_aware_control_endpoints_validate_and_persist_across_jobs(
    tmp_path: Path,
) -> None:
    client = build_client()

    status = client.get("/ctx-aware")
    assert status.status_code == 200
    assert status.json()["enabled"] is False
    assert status.json()["usage_threshold_tokens"] is None
    assert status.json()["scheduling_threshold_tokens"] is None
    assert status.json()["policy_mode"] == "age"

    invalid_order = client.post(
        "/ctx-aware/start",
        json={
            "usage_threshold_tokens": 6000,
            "scheduling_threshold_tokens": 6000,
        },
    )
    assert invalid_order.status_code == 400
    assert "smaller" in invalid_order.json()["detail"]

    invalid_floor = client.post(
        "/ctx-aware/start",
        json={
            "usage_threshold_tokens": 6000,
            "scheduling_threshold_tokens": 2999,
        },
    )
    assert invalid_floor.status_code == 400
    assert ">= 3000" in invalid_floor.json()["detail"]

    invalid_policy = client.post(
        "/ctx-aware/start",
        json={
            "usage_threshold_tokens": 6000,
            "scheduling_threshold_tokens": 4000,
            "policy_mode": "fastest",
        },
    )
    ...
    assert enabled.json()["policy_mode"] == "age"
    assert enabled.json()["ongoing_agent_count"] == 0
    assert enabled.json()["pending_agent_count"] == 0

    assert client.post(
        "/job/start",
        json={"output_location": str(tmp_path / "ctx-job-a")},
    ).status_code == 200

    start_while_active = client.post(
        "/ctx-aware/start",
        json={
            "usage_threshold_tokens": 15000,
            "scheduling_threshold_tokens": 12000,
        },
    )
    assert start_while_active.status_code == 409

    end_while_active = client.post("/ctx-aware/end")
    assert end_while_active.status_code == 409

    assert client.post("/job/end", json={"status": "completed"}).status_code == 200

    persisted = client.get("/ctx-aware")
    assert persisted.status_code == 200
    assert persisted.json()["enabled"] is True
    assert persisted.json()["usage_threshold_tokens"] == 12000
    assert persisted.json()["scheduling_threshold_tokens"] == 9000
    assert persisted.json()["policy_mode"] == "age"

    assert client.post(
        "/job/start",
        json={"output_location": str(tmp_path / "ctx-job-b")},
    ).status_code == 200
    assert client.get("/ctx-aware").json()["enabled"] is True
    assert client.post("/job/end", json={"status": "completed"}).status_code == 200

    disabled = client.post("/ctx-aware/end")
    assert disabled.status_code == 200
    assert disabled.json()["enabled"] is False
    assert disabled.json()["usage_threshold_tokens"] is None
    assert disabled.json()["scheduling_threshold_tokens"] is None
    assert disabled.json()["policy_mode"] == "age"


def test_ctx_aware_control_endpoints_accept_throughput_policy_mode(
    tmp_path: Path,
) -> None:
    client = build_client()
    ...
    assert enabled.status_code == 200
    assert enabled.json()["enabled"] is True
    assert enabled.json()["policy_mode"] == "throughput"

    context_payload = client.get("/ipc/context").json()
    assert context_payload["ctx_aware_enabled"] is True
    assert context_payload["ctx_aware_policy_mode"] == "throughput"

    assert client.post(
        "/job/start",
        json={"output_location": str(tmp_path / "ctx-throughput-policy")},
    ).status_code == 200
    assert client.post("/job/end", json={"status": "completed"}).status_code == 200
...
def test_slo_aware_control_endpoints_require_ctx_aware_and_persist_across_jobs(
    tmp_path: Path,
) -> None:
    client = build_client()

    status = client.get("/slo-aware")
    assert status.status_code == 200
    assert status.json()["enabled"] is False
    assert status.json()["target_tokens_per_s"] is None
    assert status.json()["policy_mode"] is None

    requires_ctx = client.post(
        "/slo-aware/start",
        json={
    ...
    assert enabled.status_code == 200
    assert enabled.json()["enabled"] is True
    assert enabled.json()["target_tokens_per_s"] == 25.0
    assert enabled.json()["policy_mode"] == "push-back-half-slack"

    context_payload = client.get("/ipc/context").json()
    assert context_payload["slo_aware_enabled"] is True
    assert context_payload["slo_target_tokens_per_s"] == 25.0
    assert context_payload["slo_policy_mode"] == "push-back-half-slack"

    assert client.post(
        "/job/start",
        json={"output_location": str(tmp_path / "slo-job-a")},
    ).status_code == 200
    ...
    end_while_active = client.post("/slo-aware/end")
    assert end_while_active.status_code == 409
    ...
    assert client.post(
        "/job/start",
        json={"output_location": str(tmp_path / "slo-job-b")},
    ).status_code == 200
    assert client.get("/slo-aware").json()["enabled"] is True
    assert client.post("/job/end", json={"status": "completed"}).status_code == 200
...
def test_ctx_aware_new_agent_admission_and_oldest_pending_promotion(
    ...
    tokens = ["agent-a", "agent-b", "agent-c", "agent-d", "agent-e"]
    ...
    assert client.post(
        "/job/start",
        json={"output_location": str(tmp_path / "ctx-admission")},
    ).status_code == 200
    ...
    assert status.status_code == 200
    payload = status.json()
    assert payload["ongoing_agent_count"] == 3
    assert payload["pending_agent_count"] == 2
    assert payload["ongoing_effective_context_tokens"] == 9000
    assert payload["pending_effective_context_tokens"] == 6000

    assert service.active_runs["agent-a"].schedule_state == "ongoing"
    assert service.active_runs["agent-b"].schedule_state == "ongoing"
    assert service.active_runs["agent-c"].schedule_state == "ongoing"
    assert service.active_runs["agent-d"].schedule_state == "pending"
    assert service.active_runs["agent-e"].schedule_state == "pending"
    assert service.active_runs["agent-d"].pending_since_iso is not None
    assert service.active_runs["agent-e"].pending_since_iso is not None

    context_payload = client.get("/ipc/context").json()
    assert context_payload["effective_total_context_tokens"] == 15000
    assert context_payload["ongoing_agent_count"] == 3
    assert context_payload["pending_agent_count"] == 2

    assert client.post(
        "/agent/end",
        json={"api_token": "agent-c", "return_code": 0},
    ).status_code == 200

    promoted = client.get("/ctx-aware").json()
    assert promoted["ongoing_agent_count"] == 3
    assert promoted["pending_agent_count"] == 1
    assert service.active_runs["agent-d"].schedule_state == "ongoing"
    assert service.active_runs["agent-d"].pending_since_iso is None
    assert service.active_runs["agent-e"].schedule_state == "pending"


def test_ctx_aware_demotes_youngest_ongoing_agent_after_context_growth(
    tmp_path: Path,
) -> None:
    def response_json_factory(request_json: dict[str, Any]) -> dict[str, Any]:
        return {
    ...
    assert client.post(
        "/ctx-aware/start",
        json={
            "usage_threshold_tokens": 10000,
            "scheduling_threshold_tokens": 9000,
        },
    ).status_code == 200
    assert client.post(
        "/job/start",
        json={"output_location": str(tmp_path / "ctx-demotion")},
    ).status_code == 200

    for token in ["agent-a", "agent-b", "agent-c"]:
        assert client.post("/agent/start", json={"api_token": token}).status_code == 200

    assert client.post(
        "/v1/chat/completions",
        headers={"x-api-key": "agent-a"},
        json={
            "model": "Qwen3-Coder-30B-A3B-Instruct",
            "messages": [{"role": "user", "content": "grow context"}],
            "test_usage": {"prompt_tokens": 5000, "completion_tokens": 0},
        },
    ).status_code == 200

    assert service.active_runs["agent-a"].schedule_state == "ongoing"
    assert service.active_runs["agent-b"].schedule_state == "ongoing"
    assert service.active_runs["agent-c"].schedule_state == "pending"

    status = client.get("/ctx-aware").json()
    assert status["ongoing_agent_count"] == 2
    assert status["pending_agent_count"] == 1
    assert status["ongoing_effective_context_tokens"] == 8000
    assert status["pending_effective_context_tokens"] == 3000


def test_ctx_aware_throughput_policy_demotes_highest_throughput_agent(
    tmp_path: Path,
) -> None:
    ...
    service.start_job(str(tmp_path / "ctx-throughput-demotion"))
    for token in ["agent-a", "agent-b", "agent-c"]:
        service.start_agent(token)

    with service._lock:
        service.active_runs["agent-a"].current_context_tokens = 7000
        service.active_runs["agent-a"].has_usable_context_usage = True
        service.active_runs["agent-a"].total_completion_tokens = 100
        service.active_runs["agent-a"].total_ctx_aware_request_duration_s = 10.0

        service.active_runs["agent-b"].current_context_tokens = 2000
        service.active_runs["agent-b"].has_usable_context_usage = True
        service.active_runs["agent-b"].total_completion_tokens = 500
        service.active_runs["agent-b"].total_ctx_aware_request_duration_s = 10.0

        service.active_runs["agent-c"].current_context_tokens = 2000
        service.active_runs["agent-c"].has_usable_context_usage = True
        service.active_runs["agent-c"].total_completion_tokens = 50
        service.active_runs["agent-c"].total_ctx_aware_request_duration_s = 10.0

        service._rebalance_ctx_aware_locked()

    status = service.get_ctx_aware_summary()
    assert status["policy_mode"] == "throughput"
    assert status["ongoing_agent_count"] == 2
    assert status["pending_agent_count"] == 1
    assert status["ongoing_effective_context_tokens"] == 9000
    assert status["pending_effective_context_tokens"] == 2000
    assert service.active_runs["agent-a"].schedule_state == "ongoing"
    assert service.active_runs["agent-b"].schedule_state == "pending"
    assert service.active_runs["agent-c"].schedule_state == "ongoing"


def test_ctx_aware_throughput_policy_promotion_uses_current_queue_time(
    tmp_path: Path,
) -> None:
    async def run_case() -> None:
        ...
        service.start_job(str(tmp_path / "ctx-throughput-promotion"))
        for token in ["agent-a", "agent-b", "agent-c", "agent-d"]:
            service.start_agent(token)

        assert service.active_runs["agent-c"].schedule_state == "pending"
        assert service.active_runs["agent-d"].schedule_state == "pending"
        assert (
            service.active_runs["agent-c"].pending_since_monotonic
            <= service.active_runs["agent-d"].pending_since_monotonic
        )

        with service._lock:
            now_monotonic = time.monotonic()
            service.active_runs["agent-c"].total_completion_tokens = 100
            service.active_runs["agent-c"].total_ctx_aware_request_duration_s = 10.0
            service.active_runs["agent-c"].queued_request_count = 1
            service.active_runs["agent-c"].queued_request_started_monotonic = (
                now_monotonic - 0.1
            )

            service.active_runs["agent-d"].total_completion_tokens = 100
            service.active_runs["agent-d"].total_ctx_aware_request_duration_s = 10.0
            service.active_runs["agent-d"].queued_request_count = 1
            service.active_runs["agent-d"].queued_request_started_monotonic = (
                now_monotonic - 5.0
            )

        assert await service.end_agent("agent-b", 0) == {"status": "ok"}
...
def test_ctx_aware_job_log_writes_periodic_samples_and_transition_counts(
    tmp_path: Path,
) -> None:
    def response_json_factory(request_json: dict[str, Any]) -> dict[str, Any]:
        return {
    ...
    with build_client(response_json_factory=response_json_factory) as client:
        output_dir = tmp_path / "ctx-job-log"
        ...
        response = client.post("/job/start", json={"output_location": str(output_dir)})
        assert response.status_code == 200
        job_log_path = Path(response.json()["ctx_aware_job_log_path"])
        slo_decision_log_path = Path(response.json()["slo_aware_decision_log_path"])
        assert job_log_path.exists()
        assert slo_decision_log_path.exists()

        for token in ["agent-a", "agent-b", "agent-c", "agent-d"]:
            assert client.post("/agent/start", json={"api_token": token}).status_code == 200

        time.sleep(CTX_AWARE_SCHEDULER_INTERVAL_S * 1.6)
        ...
        assert client.post(
            "/agent/end",
            json={"api_token": "agent-a", "return_code": 0},
        ).status_code == 200

        time.sleep(CTX_AWARE_SCHEDULER_INTERVAL_S * 1.6)

        for token in ["agent-b", "agent-c", "agent-d"]:
            assert client.post("/agent/end", json={"api_token": token, "return_code": 0}).status_code == 200

        response = client.post("/job/end", json={"status": "completed"})
        assert response.status_code == 200
        assert response.json()["ctx_aware_job_log_path"] == str(job_log_path)

        records = load_jsonl(job_log_path)
        assert len(records) >= 4
        assert records[0]["ongoing_agent_count"] == 0
        assert records[0]["pending_agent_count"] == 0
        assert records[0]["ralexation_agent_count"] == 0
        assert records[0]["ongoing_effective_context_tokens"] == 0
        assert records[0]["pending_effective_context_tokens"] == 0
        assert records[0]["ralexation_effective_context_tokens"] == 0
        ...
        for record in records:
            assert expected_keys.issubset(set(record))

        assert any(
            record["new_agents_added_as_ongoing"] == 3
            and record["new_agents_added_as_pending"] == 1
            and record["ongoing_agent_count"] == 3
            and record["pending_agent_count"] == 1
            for record in records
        )
        assert any(
            record["agents_turned_pending_due_to_context_threshold"] == 1
            and record["ongoing_agent_count"] == 2
            and record["pending_agent_count"] == 2
            for record in records
        )
        assert any(
            record["agents_turned_ongoing"] == 2
            and record["ongoing_agent_count"] == 3
            and record["pending_agent_count"] == 0
            for record in records
        )
        assert records[-1]["ongoing_agent_count"] == 0
        assert records[-1]["pending_agent_count"] == 0


def test_ctx_aware_pending_request_waits_and_logs_pending_duration(
    tmp_path: Path,
) -> None:
    async def run_case() -> None:
        ...
        service.start_job(str(tmp_path / "ctx-pending-wait"))
        ...
        record = service.active_runs["agent-c"].request_records[0]
        ...
        assert record["request_duration_ms"] >= record["span_duration_ms"]
        assert record["response"]["usage"]["prompt_tokens"] == 100

    asyncio.run(run_case())


def test_ctx_aware_pending_disconnect_returns_499_without_upstream_call(
    tmp_path: Path,
) -> None:
    async def run_case() -> None:
        observed = {"forward_calls": 0}
        ...
        service.start_job(str(tmp_path / "ctx-pending-disconnect"))
        service.start_agent("agent-a")
        service.start_agent("agent-b")
        service.start_agent("agent-c")
        ...
        assert observed["forward_calls"] == 0

        record = service.active_runs["agent-c"].request_records[0]
        assert record["forward_start_time"] is None
        assert record["pending_duration_ms"] > 0
        assert record["span_duration_ms"] is None
        assert record["response"]["error"] == "client_disconnected"

    asyncio.run(run_case())


def test_end_agent_cancels_pending_queued_request_without_upstream_call(
    tmp_path: Path,
) -> None:
    async def run_case() -> None:
        observed = {"forward_calls": 0}
        ...
        service.start_job(str(tmp_path / "ctx-pending-end"))
        service.start_agent("agent-a")
        service.start_agent("agent-b")
        service.start_agent("agent-c")
        ...
        proxy_task = asyncio.create_task(
            service.proxy_request(
                ...
                body=json.dumps(
                    {
                        "model": "Qwen3-Coder-30B-A3B-Instruct",
                        "messages": [{"role": "user", "content": "cancel queued request"}],
                    }
                ).encode("utf-8"),
            )
        )
        ...
        for _ in range(50):
            if service.active_runs["agent-c"].queued_request_count == 1:
                break
            await asyncio.sleep(0.01)
        assert service.active_runs["agent-c"].queued_request_count == 1
        assert observed["forward_calls"] == 0

        assert await service.end_agent("agent-c", 0) == {"status": "ok"}
        result = await proxy_task

        assert result.status_code == 409
        assert observed["forward_calls"] == 0
        assert "agent-c" not in service.active_runs
        assert len(service.completed_runs) == 1

        run = service.completed_runs[0]
        assert run.api_token == "agent-c"
        assert len(run.request_records) == 1
        record = run.request_records[0]
        assert record["forward_start_time"] is None
        assert record["pending_duration_ms"] > 0
        assert record["span_duration_ms"] is None
        assert record["response"]["error"] == "agent_ended_while_pending"

    asyncio.run(run_case())


def test_end_agent_rejects_while_forwarded_request_is_active(tmp_path: Path) -> None:
    async def run_case() -> None:
        ...
        service.start_job(str(tmp_path / "ctx-active-end"))
        service.start_agent("agent-a")
        ...
        proxy_task = asyncio.create_task(
            service.proxy_request(
                ...
                body=json.dumps(
                    {
                        "model": "Qwen3-Coder-30B-A3B-Instruct",
                        "messages": [{"role": "user", "content": "block end"}],
                    }
                ).encode("utf-8"),
            )
        )
        ...
        with pytest.raises(RuntimeError, match="cannot end agent while requests are active"):
            await service.end_agent("agent-a", 0)
        assert "agent-a" in service.active_runs

        allow_finish.set()
        await proxy_task
        assert await service.end_agent("agent-a", 0) == {"status": "ok"}
        assert "agent-a" not in service.active_runs
        assert len(service.completed_runs) == 1
...
def test_ipc_output_throughput_reports_summary_and_details(tmp_path: Path) -> None:
    ...
    assert payload["avg_output_tokens_per_s"] == pytest.approx(
...
def test_gateway_forward_to_vllm_uses_no_timeout(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("gateway_ctx.app.httpx.AsyncClient", FakeAsyncClient)
...
def test_default_ipc_socket_path_uses_port_profile_id() -> None:
    assert _default_ipc_socket_path(0) == Path("/tmp/vllm-gateway-ctx-profile-0.sock")
    assert _default_ipc_socket_path("3") == Path("/tmp/vllm-gateway-ctx-profile-3.sock")
...
def test_resolve_ipc_socket_path_defaults_to_profile_specific_path() -> None:
    assert _resolve_ipc_socket_path(
        ...
        profile_id=2,
    ) == Path("/tmp/vllm-gateway-ctx-profile-2.sock")
...
def test_slo_aware_push_back_half_slack_enters_ralexation(tmp_path: Path) -> None:
    async def run_case() -> None:
        async def fake_forward(
            method: str,
            path: str,
        ...
        service.start_job(str(tmp_path / "slo-ralexation-enter"))
        service.start_agent("agent-a")
        service.start_agent("agent-b")
        ...
        assert run.ready_event is not None
        assert run.ready_event.is_set() is False
        assert run.ralexation_until_iso is not None
        assert run.ralexation_until_monotonic is not None
...
def test_slo_aware_ready_ralexation_becomes_pending_when_pending_exists(
    tmp_path: Path,
) -> None:
    async def run_case() -> None:
        async def fake_forward(
        ...
        service.start_job(str(tmp_path / "slo-ralexation-to-pending"))
        for token in ["agent-a", "agent-b", "agent-c", "agent-d"]:
            service.start_agent(token)
        ...
        assert result.status_code == 200
        assert service.active_runs["agent-a"].schedule_state == "ralexation"

        with service._lock:
            service.active_runs["agent-a"].ralexation_until_monotonic = time.monotonic() - 0.1
            service._rebalance_ctx_aware_locked()

        assert service.active_runs["agent-a"].schedule_state == "pending"
        assert service.active_runs["agent-a"].pending_since_iso is not None
        assert service.active_runs["agent-a"].ralexation_until_iso is None

    asyncio.run(run_case())


def test_slo_aware_ready_ralexation_reenters_ongoing_without_pending(
    tmp_path: Path,
) -> None:
    async def run_case() -> None:
        async def fake_forward(
        ...
        service.start_job(str(tmp_path / "slo-ralexation-to-ongoing"))
        service.start_agent("agent-a")
        service.start_agent("agent-b")
        ...
        assert service.active_runs["agent-a"].schedule_state == "ongoing"
        assert service.active_runs["agent-a"].ready_event is not None
        assert service.active_runs["agent-a"].ready_event.is_set() is True

    asyncio.run(run_case())


def test_slo_aware_ralexation_request_waits_and_logs_pending_duration(
    tmp_path: Path,
) -> None:
    async def run_case() -> None:
        forward_calls: list[float] = []
        ...
        service.start_job(str(tmp_path / "slo-ralexation-wait"))
        service.start_agent("agent-a")

        with service._lock:
            service._mark_run_ralexation_locked(service.active_runs["agent-a"], duration_s=1.0)
        ...
        proxy_task = asyncio.create_task(
            service.proxy_request(
                ...
                body=json.dumps(
                    {
                        "model": "Qwen3-Coder-30B-A3B-Instruct",
                        "messages": [{"role": "user", "content": "wait in ralexation"}],
                    }
                ).encode("utf-8"),
            )
        )
        ...
        with service._lock:
            service._mark_run_ongoing_locked(service.active_runs["agent-a"])

        result = await proxy_task
        assert result.status_code == 200
        assert len(forward_calls) == 1

        record = service.active_runs["agent-a"].request_records[0]
        assert record["forward_start_time"] is not None
        assert record["pending_duration_ms"] > 0
        assert record["span_duration_ms"] is not None
        run = service.active_runs["agent-a"]
        request_duration_s = record["request_duration_ms"] / 1000.0
        span_duration_s = record["span_duration_ms"] / 1000.0
        assert run.total_llm_request_duration_s == pytest.approx(
            request_duration_s,
            abs=1e-6,
        )
        assert run.current_output_tokens_per_s == pytest.approx(
            12.0 / run.total_llm_request_duration_s,
            rel=1e-6,
        )
        assert run.current_output_tokens_per_s < (12.0 / span_duration_s)

    asyncio.run(run_case())


def test_slo_aware_decision_log_records_enter_and_wake_events(
    tmp_path: Path,
) -> None:
    async def run_case() -> None:
        async def fake_forward(
        ...
        start_payload = service.start_job(str(tmp_path / "slo-decision-log"))
        ...
        entered = records[0]
        assert entered["from_schedule_state"] == "ongoing"
        assert entered["to_schedule_state"] == "ralexation"
        assert entered["policy_mode"] == "push-back-half-slack"
        assert entered["ralexation_duration_s"] > 0
        assert entered["ralexation_until"] is not None

        left = records[1]
        assert left["from_schedule_state"] == "ralexation"
        assert left["to_schedule_state"] == "ongoing"
        assert left["wake_reason"] == "timer_expired"
        assert left["resume_disposition"] == "ctx_aware_admitted"
