...
BASE_POST_PROCESS_DIR = (THIS_DIR.parent / "post-process").resolve()
...
PIPELINE_STEPS = [
    ...
    "request-throughput/extract_run.py",
    ...
    "gateway/slo-aware-log/extract_run.py",
...
VISUALIZATION_STEPS = [
    ...
    "visualization/request-throughput/generate_all_figures.py",
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "runs under a root directory, preferring gateway_multi-aware overrides "
            "from post-process-multi/ when available."
        )
    )
...
def _script_path(relative_path: str) -> Path:
    override_path = (THIS_DIR / relative_path).resolve()
    if override_path.is_file():
        return override_path
    return (BASE_POST_PROCESS_DIR / relative_path).resolve()
...
def _step_list(*, skip_visualization: bool) -> list[str]:
...
def _build_step_commands_for_run_dir(
    ...
    *,
    ...
    for script_rel_path in _step_list(skip_visualization=skip_visualization):
        commands.append(
            [
                sys.executable,
