...
MODULE_ROOT = PROJECT_ROOT / "freq-controller-linespace-instance"
if str(MODULE_ROOT) not in sys.path:
    sys.path.insert(0, str(MODULE_ROOT))

import freq_controller_linespace_instance as controller_module

from freq_controller_linespace_instance import (
    FrequencyController,
...
def test_frequency_controller_uses_linespace_policy_and_resets_on_exit(
    ...
    assert log_paths.query_path.name.startswith("freq-controller-ls-instance.query.")
    assert log_paths.decision_path.name.startswith(
        "freq-controller-ls-instance.decision."
    )
    assert log_paths.control_error_path.name.startswith(
        "freq-controller-ls-instance.control-error."
    )
