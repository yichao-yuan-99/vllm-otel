#!/usr/bin/env python3
"""Smoke tests for the mi2104x embedded TP1 launcher script/docs."""
...
class StartServicesScriptTest(unittest.TestCase):
    ...
    def test_start_services_unrolls_all_mi2104x_tp1_profiles(self) -> None:
        script_text = START_SCRIPT.read_text(encoding="utf-8")

        self.assertIn("PROFILE_IDS=(0 1 2 3)", script_text)
        self.assertIn("VLLM_TENSOR_PARALLEL_SIZE=1", script_text)
        self.assertIn('local selector="${VLLM_MODEL_KEY:-${VLLM_MODEL_NAME:-qwen3_coder_30b_fp8}}"', script_text)
        ...
        self.assertIn('--env HIP_VISIBLE_DEVICES=0', script_text)
    ...
    def test_readme_documents_embedded_batch_workflow(self) -> None:
        ...
        self.assertIn("one TP=1 vLLM per port profile `0..3`, pinned to GPU `0..3`", readme_text)
        ...
        self.assertIn("python3 servers/servers-amdhpc-mi2104x-embedded-TP1/launch.py render", readme_text)
        self.assertIn("python3 servers/servers-amdhpc-mi2104x-embedded-TP1/launch.py submit", readme_text)
        self.assertIn("qwen3_coder_30b_fp8", readme_text)
