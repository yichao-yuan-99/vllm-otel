#!/usr/bin/env python3
"""Unit tests for the mi2104x embedded TP1 sbatch renderer."""
...
from launch import Mi2104xEmbeddedTp1Launcher, LaunchError  # type: ignore[import-not-found]


class Mi2104xEmbeddedTp1LauncherTest(unittest.TestCase):
    ...
    def test_render_writes_simple_mi2104x_tp1_sbatch_wrapper(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            start_script = self._write_start_script(root)
            launcher = Mi2104xEmbeddedTp1Launcher(
            ...
            self.assertEqual(rendered["partition"], "mi2104x")
            self.assertEqual(rendered["profile_list"], [0, 1, 2, 3])
            self.assertEqual(rendered["service_count"], 4)
            self.assertEqual(rendered["tensor_parallel_size"], 1)
            self.assertEqual(rendered["profiles"][0]["gpu_ids"], "0")
            self.assertEqual(rendered["profiles"][3]["gpu_ids"], "3")
            ...
            self.assertIn("#SBATCH --partition=mi2104x", script_text)
    ...
    def test_extract_sbatch_job_id_handles_cluster_banner_output(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            sbatch_output = textwrap.dedent(
                ...
                sbatch:    --> ok: partition provided mi2104x
                """
            )
