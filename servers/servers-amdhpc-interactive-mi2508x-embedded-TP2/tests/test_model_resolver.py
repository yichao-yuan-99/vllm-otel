#!/usr/bin/env python3
"""Unit tests for the interactive embedded TP2 model resolver."""
...
class ModelResolverTest(unittest.TestCase):
    def _write_model_config(self, root: Path) -> Path:
        ...
        config_path.write_text(
            textwrap.dedent(
                """\
                schema_version = 1
                default_model = "qwen3_coder_30b_fp8"
    ...
    def test_resolves_config_key_to_launch_values(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            resolved = resolve_model_launch_config(
                ...
                selector="qwen3_coder_30b_fp8",
            )

            self.assertEqual(resolved.resolved_model_key, "qwen3_coder_30b_fp8")
            self.assertEqual(
    ...
    def test_resolves_alias_and_normalizes_tp2_extra_args(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            config_path = self._write_model_config(Path(tmpdir))
