...
JOB_LOG_DIR="${JOB_LOG_DIR:-${REPO_ROOT}/servers/servers-amdhpc-interactive-mi3001x-embedded-TP1/logs}"
...
GATEWAY_LOG="${JOB_LOG_DIR}/gateway.${RUN_ID}.p${PROFILE_ID}.log"
...
cleanup() {
  set +e
  terminate_process "gateway profile=${PROFILE_ID}" "${GATEWAY_PID:-}"
...
wait_for_gateway_ready() {
  ...
  while (( SECONDS < deadline )); do
    if ! kill -0 "${GATEWAY_PID}" >/dev/null 2>&1; then
      echo "Gateway exited before readiness. See ${GATEWAY_LOG}" >&2
      return 1
    fi
  ...
  echo "Timed out waiting for gateway readiness. See ${GATEWAY_LOG}" >&2
  return 1
}
...
wait_for_service_phase() {
  while true; do
    ...
    if ! kill -0 "${VLLM_PID}" >/dev/null 2>&1; then
      ...
      echo "vLLM exited. See ${VLLM_LOG}" >&2
      return 92
    fi
    if ! kill -0 "${GATEWAY_PID}" >/dev/null 2>&1; then
      wait "${GATEWAY_PID}" >/dev/null 2>&1 || true
      echo "Gateway exited. See ${GATEWAY_LOG}" >&2
      return 93
    fi
    sleep 2
  done
...
echo "  gateway-parse: http://${GATEWAY_HOST}:${GATEWAY_PARSE_PORT}"
