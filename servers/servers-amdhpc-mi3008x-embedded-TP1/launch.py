#!/usr/bin/env python3
"""Render or submit a dedicated mi3008x embedded TP=1 Slurm job."""
...
DEFAULT_AMD_SMI_POWER_DAEMON_BIN = Path(
    "/home1/yichaoy/vllm-otel/.venv/bin/amd-smi-power-daemon"
)
DEFAULT_AMD_POWER_READER_BIN = Path("/home1/yichaoy/vllm-otel/.venv/bin/amd-power-reader")

PARTITION = "mi3008x"
MAX_TIME = "12:00:00"
JOB_NAME_PREFIX = "vllm_embedded_tp1_"
GPU_MEMORY_GB = 192.0
MAX_ALLOWED_WEIGHT_VRAM_GB = GPU_MEMORY_GB * 0.75

PROFILE_LAYOUTS = [
    {"profile_id": 0, "gpu_id": 0, "vllm_port": 11451, "gateway_port": 11457, "gateway_parse_port": 18171, "lmcache_port": 29411},
    {"profile_id": 1, "gpu_id": 1, "vllm_port": 24123, "gateway_port": 24157, "gateway_parse_port": 28171, "lmcache_port": 29437},
    {"profile_id": 2, "gpu_id": 2, "vllm_port": 31987, "gateway_port": 31955, "gateway_parse_port": 38171, "lmcache_port": 29459},
    {"profile_id": 3, "gpu_id": 3, "vllm_port": 40823, "gateway_port": 40857, "gateway_parse_port": 48171, "lmcache_port": 29483},
    {"profile_id": 4, "gpu_id": 4, "vllm_port": 52341, "gateway_port": 52357, "gateway_parse_port": 58171, "lmcache_port": 29507},
    {"profile_id": 5, "gpu_id": 5, "vllm_port": 59231, "gateway_port": 59257, "gateway_parse_port": 59171, "lmcache_port": 29531},
    {"profile_id": 6, "gpu_id": 6, "vllm_port": 60231, "gateway_port": 60257, "gateway_parse_port": 60171, "lmcache_port": 29557},
    {"profile_id": 7, "gpu_id": 7, "vllm_port": 61231, "gateway_port": 61257, "gateway_parse_port": 61171, "lmcache_port": 29579},
]
...
@dataclass
class Mi3008xEmbeddedTp1Launcher:
...
def _handle_render(args: argparse.Namespace) -> None:
    ...
    launcher = Mi3008xEmbeddedTp1Launcher(model_config_path=Path(args.model_config))
    try:
    ...
    _print_json(
        {
            "ok": True,
            "code": 0,
            "message": "rendered mi3008x embedded TP1 sbatch",
            "data": payload,
        }
    )
    ...
    _print_json(
        {
            "ok": True,
            "code": 0,
            "message": "submitted mi3008x embedded TP1 sbatch",
            "data": payload,
        }
    )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Render or submit a dedicated mi3008x embedded TP1 Slurm job. "
            "The job launches the explicit eight-profile service stack and then "
            "runs the supplied experiment script once for each profile id 0..7."
        )
    )
    ...
    def add_common_arguments(command_parser: argparse.ArgumentParser) -> None:
        ...
        command_parser.add_argument(
            ...
            "-e",
            required=True,
            help="Experiment script path. It will be invoked once per profile id 0..7.",
        )
        command_parser.add_argument(
            "--env",
            action="append",
