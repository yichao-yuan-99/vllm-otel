#!/usr/bin/env python3
"""Unit tests for the mi3008x embedded TP1 sbatch renderer."""
...
from launch import Mi3008xEmbeddedTp1Launcher, LaunchError  # type: ignore[import-not-found]


class Mi3008xEmbeddedTp1LauncherTest(unittest.TestCase):
    ...
    def test_render_writes_simple_mi3008x_sbatch_wrapper(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            start_script = self._write_start_script(root)
            launcher = Mi3008xEmbeddedTp1Launcher(
            ...
            self.assertEqual(rendered["service_count"], 8)
            self.assertEqual(rendered["profiles"][0]["profile_id"], 0)
            self.assertEqual(rendered["profiles"][7]["gpu_id"], 7)
