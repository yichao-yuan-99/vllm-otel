...
usage() {
  cat <<'USAGE'
Usage: servers/servers-amdhpc/run_services.sh [--env-file PATH] <command>
...
Examples:
  bash servers/servers-amdhpc/run_services.sh --env-file servers/servers-amdhpc/.env pull
  bash servers/servers-amdhpc/run_services.sh start
USAGE
}
...
encode_model_extra_args() {
  if [[ -n "${VLLM_MODEL_EXTRA_ARGS_B64}" ]]; then
    return 0
  fi
  VLLM_MODEL_EXTRA_ARGS_B64="$(
    VLLM_EXTRA_ARGS_JSON="${VLLM_EXTRA_ARGS_JSON}" python3 - <<'PY'
import base64
import json
import os

payload = os.environ["VLLM_EXTRA_ARGS_JSON"]
try:
    extra_args = json.loads(payload)
except json.JSONDecodeError as exc:
    raise SystemExit(f"error: VLLM_EXTRA_ARGS_JSON is not valid JSON: {exc}")

if not isinstance(extra_args, list) or not all(isinstance(item, str) for item in extra_args):
    raise SystemExit("error: VLLM_EXTRA_ARGS_JSON must be a JSON string array")

encoded = base64.b64encode(json.dumps(extra_args, separators=(",", ":")).encode("utf-8")).decode("ascii")
print(encoded)
PY
  )"
}
...
require_sif_files() {
  ...
  if [[ "${missing}" -ne 0 ]]; then
    echo "hint: run 'bash servers/servers-amdhpc/run_services.sh --env-file ${ENV_FILE} pull' first" >&2
    exit 1
  fi
}
