# Interactive AMD HPC Embedded TP1 MI3008X

This directory contains:

- a bash launcher for an already allocated interactive `mi3008x` node
...
The stack matches the rendered embedded-TP1 `mi3008x` sbatch job minus the
experiment phase:
...
Like the rendered `mi3008x` sbatch example, `start-services.sh` explicitly
unrolls the eight vLLM and `gateway_ctx` launches instead of deriving them from
a looped GPU list at runtime.
...
From an interactive allocation on a `mi3008x` node:

```bash
bash servers/servers-amdhpc-interactive-mi3008x-embedded-TP1/start-services.sh
```

The foreground launcher writes logs under
`servers/servers-amdhpc-interactive-mi3008x-embedded-TP1/logs/`.
It starts one `amd-smi-power-daemon`, waits for its Unix socket to become
ready, launches shared Jaeger, then starts all eight TP=1 vLLM and
`gateway_ctx` pairs for profiles `0..7`.
...
```bash
python3 servers/servers-amdhpc-interactive-mi3008x-embedded-TP1/client.py start
```
...
```bash
python3 servers/servers-amdhpc-interactive-mi3008x-embedded-TP1/client.py stop
```
...
```bash
python3 servers/servers-amdhpc-interactive-mi3008x-embedded-TP1/client.py status
```

The background wrapper keeps its pid record at
`servers/servers-amdhpc-interactive-mi3008x-embedded-TP1/logs/background-service.pid.json`
and writes launcher stdout/stderr to
`servers/servers-amdhpc-interactive-mi3008x-embedded-TP1/logs/launcher.<run_id>.log`.
...
- Profile `4`: vLLM `52341`, gateway `52357`, gateway parse `58171`
- Profile `5`: vLLM `59231`, gateway `59257`, gateway parse `59171`
- Profile `6`: vLLM `60231`, gateway `60257`, gateway parse `60171`
- Profile `7`: vLLM `61231`, gateway `61257`, gateway parse `61171`
...
- Defaults match the current `mi3008x` embedded-TP1 sbatch render where
  practical, including the same AMD power daemon, SIF paths, ports, and
  OTEL/`gateway_ctx` wiring.
- Service ports stay fixed to embedded port profiles `0..7`.
