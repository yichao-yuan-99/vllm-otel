...
JOB_LOG_DIR="${JOB_LOG_DIR:-${REPO_ROOT}/servers/servers-amdhpc-interactive-mi3008x-embedded-TP1/logs}"
...
echo "Interactive embedded TP1 service stack starting on $(hostname) at $(date)"
echo "Run ID: ${RUN_ID}"
resolve_model_launch_settings
...
launch_vllm 4 "${PROFILE_VLLM_PORTS[4]}" "${PROFILE_LMCACHE_PORTS[4]}" "${PROFILE_GPU_IDS[4]}"
launch_vllm 5 "${PROFILE_VLLM_PORTS[5]}" "${PROFILE_LMCACHE_PORTS[5]}" "${PROFILE_GPU_IDS[5]}"
launch_vllm 6 "${PROFILE_VLLM_PORTS[6]}" "${PROFILE_LMCACHE_PORTS[6]}" "${PROFILE_GPU_IDS[6]}"
launch_vllm 7 "${PROFILE_VLLM_PORTS[7]}" "${PROFILE_LMCACHE_PORTS[7]}" "${PROFILE_GPU_IDS[7]}"
...
wait_for_vllm_ready 4 "${PROFILE_VLLM_PORTS[4]}"
wait_for_vllm_ready 5 "${PROFILE_VLLM_PORTS[5]}"
wait_for_vllm_ready 6 "${PROFILE_VLLM_PORTS[6]}"
wait_for_vllm_ready 7 "${PROFILE_VLLM_PORTS[7]}"
...
launch_gateway 4 "${PROFILE_GATEWAY_PORTS[4]}" "${PROFILE_GATEWAY_PARSE_PORTS[4]}"
launch_gateway 5 "${PROFILE_GATEWAY_PORTS[5]}" "${PROFILE_GATEWAY_PARSE_PORTS[5]}"
launch_gateway 6 "${PROFILE_GATEWAY_PORTS[6]}" "${PROFILE_GATEWAY_PARSE_PORTS[6]}"
launch_gateway 7 "${PROFILE_GATEWAY_PORTS[7]}" "${PROFILE_GATEWAY_PARSE_PORTS[7]}"
...
wait_for_gateway_ready 4 "${PROFILE_GATEWAY_PORTS[4]}" "${PROFILE_GATEWAY_PARSE_PORTS[4]}"
wait_for_gateway_ready 5 "${PROFILE_GATEWAY_PORTS[5]}" "${PROFILE_GATEWAY_PARSE_PORTS[5]}"
wait_for_gateway_ready 6 "${PROFILE_GATEWAY_PORTS[6]}" "${PROFILE_GATEWAY_PARSE_PORTS[6]}"
wait_for_gateway_ready 7 "${PROFILE_GATEWAY_PORTS[7]}" "${PROFILE_GATEWAY_PARSE_PORTS[7]}"

echo "Services are ready."
echo "  amd-smi-power-socket: ${AMD_SMI_POWER_SOCKET_PATH}"
echo "  jaeger: http://127.0.0.1:${JAEGER_UI_LOCAL_PORT}"
for profile_id in "${PROFILE_IDS[@]}"; do
  echo "  profile ${profile_id}: gpu=${PROFILE_GPU_IDS[$profile_id]} vLLM=http://127.0.0.1:${PROFILE_VLLM_PORTS[$profile_id]} gateway=http://${GATEWAY_HOST}:${PROFILE_GATEWAY_PORTS[$profile_id]} gateway-parse=http://${GATEWAY_HOST}:${PROFILE_GATEWAY_PARSE_PORTS[$profile_id]}"
done
echo "Logs:"
