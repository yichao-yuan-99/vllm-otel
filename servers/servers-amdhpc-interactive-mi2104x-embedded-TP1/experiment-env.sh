...
# Shared helper for interactive mi2104x experiment scripts.
...
INTERACTIVE_CLIENT_SCRIPT="${INTERACTIVE_CLIENT_SCRIPT:-servers/servers-amdhpc-interactive-mi2104x-embedded-TP1/client.py}"
INTERACTIVE_START_SERVICES_SCRIPT="${INTERACTIVE_START_SERVICES_SCRIPT:-servers/servers-amdhpc-interactive-mi2104x-embedded-TP1/start-services.sh}"
...
INTERACTIVE_MI2104X_MIN_ID=0
INTERACTIVE_MI2104X_MAX_ID=3
...
interactive_embedded_tp1_normalize_port_profile_id() {
  ...
  local log_tag="${3:-interactive-embedded-tp1-mi2104x}"
  ...
  if (( normalized_value < INTERACTIVE_MI2104X_MIN_ID || normalized_value > INTERACTIVE_MI2104X_MAX_ID )); then
    echo "[${log_tag}] error: supported mi2104x port profile ids are 0..3" >&2
    return 1
  fi
  if [[ -n "${expected_value}" ]]; then
    ...
    if [[ "${normalized_value}" != "${normalized_expected}" ]]; then
      echo "[${log_tag}] error: this mi2104x workflow expects port profile ${normalized_expected}" >&2
      return 1
    fi
  fi
  printf '%s\n' "${normalized_value}"
...
interactive_embedded_tp1_normalize_gpu_index() {
  ...
  if (( normalized_value < INTERACTIVE_MI2104X_MIN_ID || normalized_value > INTERACTIVE_MI2104X_MAX_ID )); then
    echo "[${log_tag}] error: supported mi2104x gpu indices are 0..3" >&2
    return 1
  fi
  if [[ -n "${expected_value}" ]]; then
    local normalized_expected="${expected_value//[[:space:]]/}"
    ...
    if [[ "${normalized_value}" != "${normalized_expected}" ]]; then
      echo "[${log_tag}] error: this mi2104x workflow expects gpu index ${normalized_expected}" >&2
      return 1
    fi
  fi
  printf '%s\n' "${normalized_value}"
...
interactive_embedded_tp1_resolve_profile_ports() {
  ...
  local log_tag="${2:-interactive-embedded-tp1-mi2104x}"
  local python_bin="${PYTHON_BIN:-python3}"
