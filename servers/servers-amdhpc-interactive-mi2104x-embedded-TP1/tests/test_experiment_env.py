#!/usr/bin/env python3
"""Smoke tests for the shared interactive mi2104x experiment env helper."""
...
class ExperimentEnvScriptTest(unittest.TestCase):
    def test_helper_defines_shared_interactive_functions(self) -> None:
        ...
        self.assertIn('INTERACTIVE_CLIENT_SCRIPT="${INTERACTIVE_CLIENT_SCRIPT:-servers/servers-amdhpc-interactive-mi2104x-embedded-TP1/client.py}"', script_text)
        self.assertIn('INTERACTIVE_START_SERVICES_SCRIPT="${INTERACTIVE_START_SERVICES_SCRIPT:-servers/servers-amdhpc-interactive-mi2104x-embedded-TP1/start-services.sh}"', script_text)
        ...
        self.assertIn("supported mi2104x port profile ids are 0..3", script_text)
        self.assertIn("supported mi2104x gpu indices are 0..3", script_text)
