#!/usr/bin/env python3
"""Resolve standalone launcher model settings from configs/model_config.toml."""
...
DEFAULT_EXTRA_ARGS = ["--trust-remote-code"]
...
def load_model_specs(config_path: Path) -> tuple[dict[str, ModelSpec], str | None]:
...
def resolve_model_launch_config(
    *,
    ...
    cleaned_selector = _clean_optional_str(selector)
    ...
    specs, default_model = load_model_specs(config_path)

    effective_selector = cleaned_selector or cleaned_model_name or default_model or "qwen3_coder_30b_fp8"
    spec = resolve_model_spec(specs, effective_selector)

    if spec is None:
        if cleaned_model_name is None or cleaned_served_model_name is None:
            raise ValueError(
                "model selector was not found in configs/model_config.toml; "
                "set VLLM_MODEL_KEY to a known key or provide both "
                "VLLM_MODEL_NAME and VLLM_SERVED_MODEL_NAME"
            )

        return ResolvedModelLaunchConfig(
            resolved_model_key="",
            vllm_model_name=cleaned_model_name,
            served_model_name=cleaned_served_model_name,
            model_extra_args_b64=cleaned_extra_args_b64
            or _encode_model_extra_args(DEFAULT_EXTRA_ARGS),
        )
    ...
    return ResolvedModelLaunchConfig(
        ...
        model_extra_args_b64=cleaned_extra_args_b64
        or _encode_model_extra_args(_normalize_extra_args(spec.extra_args)),
    )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "used by the standalone vLLM launcher."
        )
    )
    parser.add_argument("--config", type=Path, required=True, help="Path to configs/model_config.toml.")
    parser.add_argument("--selector", default=None, help="Model key, alias, or raw model name.")
    parser.add_argument(
        "--model-name",
        default=None,
        help="Optional raw model path override when selector is not in config.",
    )
    parser.add_argument(
        "--served-model-name",
        default=None,
