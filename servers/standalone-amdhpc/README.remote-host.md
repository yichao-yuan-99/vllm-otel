# Standalone Remote Port Forwarding

This note documents the minimal standalone port-forward daemon in
`servers/standalone-amdhpc/port_forward_d.py`.

It is the standalone equivalent of the remote/local forwarding behavior used by
`servers/servers-amdhpc`, but trimmed down to one port profile and one vLLM
port per daemon.

## What It Does

Each standalone `sbatch` job reverse-tunnels its vLLM port from the compute
node back to the login node.
...
```bash
python3 servers/standalone-amdhpc/port_forward_d.py start \
  -P 0
```
...
```bash
python3 servers/standalone-amdhpc/port_forward_d.py status \
  -P 0
```
...
```bash
python3 servers/standalone-amdhpc/port_forward_d.py check-health \
  -P 0
```
...
```bash
python3 servers/standalone-amdhpc/port_forward_d.py stop \
  -P 0
```

## Example Flow

1. Start a standalone service job, for example:

```bash
sbatch servers/standalone-amdhpc/start-single-service-mi2508x.sbatch
```

2. Start the local forwarding daemon for the same port profile:

```bash
python3 servers/standalone-amdhpc/port_forward_d.py start -P 0
```

3. Check that the forwarded vLLM endpoint is healthy:

```bash
python3 servers/standalone-amdhpc/port_forward_d.py check-health -P 0
```
...
```bash
python3 servers/standalone-amdhpc/port_forward_d.py stop -P 0
```
...
- Runtime pid/log files live under `~/.cache/vllm-otel-apptainer/standalone-port-forward-d/` by default.
