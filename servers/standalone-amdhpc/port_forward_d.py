#!/usr/bin/env python3
"""Minimal SSH port-forward daemon for standalone AMD HPC vLLM jobs."""
...
DEFAULT_RUNTIME_DIR = Path.home() / ".cache" / "vllm-otel-apptainer" / "standalone-port-forward-d"
DEFAULT_PID_FILE_PREFIX = "standalone_port_forward_d"
DEFAULT_LOG_FILE_PREFIX = "standalone_port_forward_d"
DEFAULT_SSH_OPTIONS = [
    "-o",
...
app = typer.Typer(
    ...
    help="Minimal SSH tunnel daemon for standalone AMD HPC vLLM jobs.",
)
...
def start_port_forward_daemon(
    *,
    ...
    if isinstance(active_record, dict):
        if _record_matches_running_process(active_record):
            return _json_payload(
                ok=False,
                code=303,
                message="standalone port-forward daemon is already running",
    ...
    with log_file.open("a", encoding="utf-8") as handle:
        handle.write(f"[{_utc_now_iso()}] starting standalone port-forward-d: {shlex.join(ssh_cmd)}\n")
        handle.flush()
    ...
    while time.monotonic() < deadline:
        ...
        if rc is not None:
            ...
            return _json_payload(
                ok=False,
                code=304,
                message=f"failed to start standalone port-forward daemon (ssh exited with code {rc})",
                data={
    ...
    return _json_payload(
        ok=True,
        code=0,
        message="standalone port-forward daemon started",
        data={
            **record,
    ...
    *,
    ...
    if not isinstance(record, dict):
        ...
        return _json_payload(
            ok=True,
            code=0,
            message=f"standalone port-forward daemon is not running for port profile {port_profile_id}",
    ...
    try:
        ...
        return _json_payload(
            ok=True,
            code=0,
            message="removed invalid standalone port-forward daemon pid record",
    ...
    if not _pid_is_running(pid):
        ...
        return _json_payload(
            ...
            code=0,
            message=(
                "standalone port-forward daemon is not running for "
    ...
    pid_file.unlink(missing_ok=True)
    with log_file.open("a", encoding="utf-8") as handle:
        handle.write(f"[{_utc_now_iso()}] stopped standalone port-forward-d pid={pid} forced={forced}\n")

    return _json_payload(
        ok=True,
        code=0,
        message="standalone port-forward daemon stopped",
        data={
            "port_profile_id": port_profile_id,
            "pid": pid,
            "forced": forced,
    ...
    *,
    ...
    try:
        ...
        return _json_payload(
            ok=True,
            code=0,
            message="standalone port-forward daemon pid file is invalid",
            data={
                "running": False,
                "port_profile_id": port_profile_id,
                "pid_file": str(pid_file),
    ...
    return _json_payload(
        ok=True,
        code=0,
        message="standalone port-forward daemon status",
        data={
            "running": running,
            "port_profile_id": port_profile_id,
            "pid": pid,
...
def start(
    ...
    """Start a local SSH tunnel for one standalone vLLM port profile."""
...
def stop(
    ...
    """Stop the local standalone SSH tunnel daemon."""
...
def status_cmd(
    ...
    """Show standalone port-forward daemon status."""
