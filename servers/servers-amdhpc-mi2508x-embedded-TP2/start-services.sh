...
JOB_LOG_DIR="${JOB_LOG_DIR:-${REPO_ROOT}/servers/servers-amdhpc-mi2508x-embedded-TP2/logs}"
...
echo "MI2508X embedded TP2 service stack starting on $(hostname) at $(date)"
echo "Run ID: ${RUN_ID}"
resolve_model_launch_settings
load_vllm_extra_env_args
...
for profile_id in "${PROFILE_IDS[@]}"; do
  echo "  profile ${profile_id}: gpus=${PROFILE_GPU_IDS[$profile_id]} vLLM=http://127.0.0.1:${PROFILE_VLLM_PORTS[$profile_id]} gateway-ctx=http://${GATEWAY_HOST}:${PROFILE_GATEWAY_PORTS[$profile_id]} gateway-ctx-parse=http://${GATEWAY_HOST}:${PROFILE_GATEWAY_PARSE_PORTS[$profile_id]}"
done
echo "Logs:"
