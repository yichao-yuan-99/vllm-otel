#!/usr/bin/env python3
"""Render or submit a dedicated mi2508x embedded TP=2 Slurm job."""
...
PARTITION = "mi2508x"
MAX_TIME = "12:00:00"
JOB_NAME_PREFIX = "vllm_embedded_tp2_"
GPU_MEMORY_GB = 64.0
GPUS_PER_SERVICE = 2
...
def _normalize_tp2_extra_args(extra_args: list[str]) -> list[str]:
...
@dataclass
class Mi2508xEmbeddedTp2Launcher:
    ...
    def render(
        ...
        *,
        ...
        if model_spec.weight_vram_gb > MAX_ALLOWED_WEIGHT_VRAM_GB:
            raise LaunchError(
                message=(
                    ...
                    f"75% of one TP2 GPU-pair VRAM budget on partition '{PARTITION}' "
        ...
        effective_model_extra_args = _normalize_tp2_extra_args(
        ...
        script_path = self.run_dir / (
            f"sbatch-{timestamp}-{PARTITION}-{_safe_token(model_spec.key)}-embedded-tp2.sh"
        )
        script_text = textwrap.dedent(
            f"""\
            #!/usr/bin/env bash
            #SBATCH --job-name={_safe_token(f"{JOB_NAME_PREFIX}{PARTITION}_tp2")}
...
def _handle_render(args: argparse.Namespace) -> None:
    ...
    launcher = Mi2508xEmbeddedTp2Launcher(model_config_path=Path(args.model_config))
    try:
    ...
    _print_json(
        {
            "ok": True,
            "code": 0,
            "message": "rendered mi2508x embedded TP2 sbatch",
            "data": payload,
        }
    )
    ...
    _print_json(
        {
            "ok": True,
            "code": 0,
            "message": "submitted mi2508x embedded TP2 sbatch",
            "data": payload,
        }
    )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Render or submit a dedicated mi2508x embedded TP2 Slurm job. "
            "The job launches the explicit four-profile TP2 service stack and then "
