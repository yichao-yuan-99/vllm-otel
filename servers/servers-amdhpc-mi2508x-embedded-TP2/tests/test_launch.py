#!/usr/bin/env python3
"""Unit tests for the mi2508x embedded TP2 sbatch renderer."""
...
from launch import Mi2508xEmbeddedTp2Launcher, LaunchError  # type: ignore[import-not-found]


class Mi2508xEmbeddedTp2LauncherTest(unittest.TestCase):
    def _write_model_config(self, root: Path) -> Path:
        ...
        config_path.write_text(
            textwrap.dedent(
                ...
                [models.qwen3_70b]
                vllm_model_name = "Qwen/Qwen3-70B-Instruct"
                served_model_name = "Qwen3-70B-Instruct"
                weight_vram_gb = 100
                extra_args = []
                """
            ),
    ...
    def test_render_writes_simple_mi2508x_tp2_sbatch_wrapper(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            start_script = self._write_start_script(root)
            launcher = Mi2508xEmbeddedTp2Launcher(
            ...
            self.assertEqual(rendered["partition"], "mi2508x")
            self.assertEqual(rendered["profile_list"], [0, 1, 2, 3])
            self.assertEqual(rendered["service_count"], 4)
            self.assertEqual(rendered["tensor_parallel_size"], 2)
            self.assertEqual(rendered["profiles"][0]["gpu_ids"], "0,1")
            self.assertEqual(rendered["profiles"][3]["gpu_ids"], "6,7")
            ...
            self.assertIn("#SBATCH --partition=mi2508x", script_text)
            ...
            self.assertIn(
                'export FREQ_CONTROLLER_BIN="${FREQ_CONTROLLER_BIN:-'
                f"{shlex.quote(str((MODULE_ROOT.parents[1] / '.venv' / 'bin' / 'freq-controller-linespace-amd').resolve()))}"
                '}"',
                script_text,
            )
            self.assertIn(
                'export RESET_GPU_CORE_FREQ_BIN="${RESET_GPU_CORE_FREQ_BIN:-'
                f"{shlex.quote(str((MODULE_ROOT.parents[1] / '.venv' / 'bin' / 'amd-reset-gpu-core-freq').resolve()))}"
                '}"',
                script_text,
            )
    ...
    def test_rejects_models_that_do_not_fit_on_one_tp2_pair(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            launcher = Mi2508xEmbeddedTp2Launcher(
                model_config_path=self._write_model_config(root),
            ...
            with self.assertRaisesRegex(LaunchError, "TP2 GPU-pair VRAM"):
                launcher.render(
                    model="qwen3_70b",
                    experiment_script=experiment_script,
                )
    ...
    def test_extract_sbatch_job_id_handles_cluster_banner_output(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            sbatch_output = textwrap.dedent(
                ...
                sbatch:    --> ok: partition provided mi2508x
                """
            )
