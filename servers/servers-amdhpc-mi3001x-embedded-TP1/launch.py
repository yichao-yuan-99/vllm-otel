#!/usr/bin/env python3
"""Render or submit a dedicated mi3001x embedded TP=1 Slurm job."""
...
@dataclass
class Mi3001xEmbeddedTp1Launcher:
...
def _handle_render(args: argparse.Namespace) -> None:
    ...
    launcher = Mi3001xEmbeddedTp1Launcher(model_config_path=Path(args.model_config))
    try:
    ...
    _print_json(
        {
            "ok": True,
            "code": 0,
            "message": "rendered mi3001x embedded TP1 sbatch",
            "data": payload,
        }
    )
    ...
    _print_json(
        {
            "ok": True,
            "code": 0,
            "message": "submitted mi3001x embedded TP1 sbatch",
            "data": payload,
        }
    )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Render or submit a dedicated mi3001x embedded TP1 Slurm job. "
