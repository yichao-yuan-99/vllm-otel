# AMD HPC MI3001X Embedded TP1

This directory contains a dedicated `mi3001x` embedded TP=1 launcher with the
same runtime stack as `servers/servers-amdhpc-embedded-TP1`, but in a much
smaller layout modeled after
`servers/servers-amdhpc-interactive-mi3001x-embedded-TP1`.

The stack is fixed to port profile `0` on one `mi3001x` GPU and includes:
...
```bash
python3 servers/servers-amdhpc-mi3001x-embedded-TP1/launch.py render \
...
`servers/servers-amdhpc-mi3001x-embedded-TP1/run/` and Slurm logs land under
`servers/servers-amdhpc-mi3001x-embedded-TP1/logs/`.

The experiment invocation writes combined stdout/stderr to:

- `servers/servers-amdhpc-mi3001x-embedded-TP1/logs/experiment.<run_id>.p0.log`

## Direct Node Run

If you already have a `mi3001x` allocation and want to run the job logic
...
bash servers/servers-amdhpc-mi3001x-embedded-TP1/start-services.sh
```
...
and forwarded into the vLLM `apptainer exec` environment, matching the old
...
- This workflow is intentionally fixed to `mi3001x` and port profile `0`.
- The default Slurm time limit is `04:00:00`, and you can override it with
  `--time-limit`.
- The launcher still validates model fit against `75%` of one MI300 GPU's VRAM.
- Compared with `servers/servers-amdhpc-embedded-TP1`, the render path is much
  simpler because the runtime logic lives in a static `start-services.sh`
  instead of a large generated script.
