#!/usr/bin/env python3
"""Render or submit a dedicated mi3008x embedded TP=1 profile 0 Slurm job."""
...
@dataclass
class Mi3008xEmbeddedTp1Profile0Launcher:
...
def _handle_render(args: argparse.Namespace) -> None:
    ...
    launcher = Mi3008xEmbeddedTp1Profile0Launcher(model_config_path=Path(args.model_config))
    try:
    ...
    _print_json(
        {
            "ok": True,
            "code": 0,
            "message": "rendered mi3008x embedded TP1 profile 0 sbatch",
            "data": payload,
        }
    )
    ...
    _print_json(
        {
            "ok": True,
            "code": 0,
            "message": "submitted mi3008x embedded TP1 profile 0 sbatch",
            "data": payload,
        }
    )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "that only launches port profile 0 on GPU 0. "
            "The job launches the service stack for port profile 0 and then "
            "runs the supplied experiment script with positional argument 0."
        )
    )
    ...
    def add_common_arguments(command_parser: argparse.ArgumentParser) -> None:
        ...
        command_parser.add_argument(
            ...
            "-e",
            required=True,
            help="Experiment script path. It will be invoked as: <runner> <script> 0.",
        )
        command_parser.add_argument(
            "--env",
            action="append",
