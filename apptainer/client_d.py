...
def _build_ssh_command(
    *,
    ...
    cmd.extend(["-L", f"{vllm_port}:127.0.0.1:{vllm_port}"])
    cmd.extend(["-L", f"{jaeger_otlp_port}:127.0.0.1:{jaeger_otlp_port}"])
    cmd.extend(["-L", f"{jaeger_ui_port}:127.0.0.1:{jaeger_ui_port}"])
...
def start_client_d(
    *,
    ...
    jaeger_otlp_port: int = DEFAULT_JAEGER_OTLP_PORT,
    jaeger_ui_port: int = DEFAULT_JAEGER_UI_PORT,
    ...
    for port_name, port_value in {
        ...
        if not isinstance(port_value, int) or port_value <= 0 or port_value > 65535:
            return _json_payload(ok=False, code=302, message=f"{port_name} must be in range 1..65535")
    ...
    return _json_payload(
        ...
        data={
            **record,
            ...
            "jaeger_ui_url": f"http://127.0.0.1:{jaeger_ui_port}",
            "jaeger_otlp_endpoint": f"grpc://127.0.0.1:{jaeger_otlp_port}",
        },
    )
